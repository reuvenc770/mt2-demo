...
$sql="select email_addr from unsub_log where client_id =1025 and unsub_date >= date_sub(curdate(),interval 1 day) and unsub_date < curdate()";
$sth=$dbhu->prepare($sql);
$sth->execute();
...
if ($ftp)
{
    $ftp->login('P49FRunsub','mgPBJXZF') or print "Cannot login ", $ftp->message;
    $ftp->ascii();
	$ftp->cwd("XLUnsubs");
