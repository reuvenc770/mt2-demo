...
if ($nid > 0)
{
$sql = "select client_id,first_name,campaign_cnt,3rdparty_cnt,aol_cnt,chunk_cnt from network_schedule, user where client_id=user_id and client_id=$nid order by client_id";
}
else
{
$sql = "select client_id,first_name,campaign_cnt,3rdparty_cnt,aol_cnt,chunk_cnt from network_schedule, user where client_id=user_id and client_id not in (287,288,289,290,291) order by client_id";
}
$sth1 = $dbhq->prepare($sql);
$sth1->execute();
while (($client_id,$company,$camp_cnt,$third_cnt,$aol_cnt,$chunk_cnt) = $sth1->fetchrow_array())
{
$cday=$cday1;
$tday=$cday1;
	...
	while ($i <= $camp_cnt)
	{
		...
		while ($j < 7)
		{
			if ($j == 0)
			{
				...
				if ($stype ne "3")
				{
					$sql="select profile_name,schedule_info.status,brand_name,schedule_info.schedule_time from list_profile,schedule_info,client_brand_info where list_profile.profile_id=schedule_info.profile_id and slot_type=? and slot_id=? and schedule_info.client_id=? and schedule_info.brand_id=client_brand_info.brand_id";
					$sth = $dbhq->prepare($sql);
			...
			if (($cname,$temp_aid,$camp_id,$camp_brand,$auto_lock) = $sth->fetchrow_array())
			{
				...
				if ($diffcnt >= 0)
				{
					if ($stype ne "3")
					{
						print "<td><font face=\"Verdana\" size=1><a href=\"/cgi-bin/advertiser_disp2.cgi?pmode=U&puserid=$temp_aid\" target=\"_blank\">$cname</a>&nbsp;<font color=\"#FF00000\" face=\"Verdana\" size=\"1\"><b><a href=\"/cgi-bin/show_campaign.cgi?campaign_id=$camp_id&aid=$temp_aid&mode=U\" target=_blank>E</a></b>&nbsp;<font color=\"#FF00000\" face=\"Verdana\" size=\"1\"><b><a href=\"/cgi-bin/camp_del.cgi?campaign_id=$camp_id\" target=_blank>D</a></b>&nbsp;&nbsp;<b><a href=\"/cgi-bin/camp_lock.cgi?campaign_id=$camp_id&stype=$stype&lock_flag=$auto_lock\" target=\"hidden\">$auto_lock_str</a></b>"; 
					}
					else
					{
						my $temp_cnt;
						print "<td><font face=\"Verdana\" size=1><a href=\"/cgi-bin/advertiser_disp2.cgi?pmode=U&puserid=$temp_aid\" target=\"_blank\">$cname</a>&nbsp;<font color=\"#FF00000\" face=\"Verdana\" size=\"1\"><b><a href=\"/cgi-bin/3rdparty_show_camp.cgi?campaign_id=$temp_cnt\" target=_blank>E</a></b>&nbsp;<font color=\"#FF00000\" face=\"Verdana\" size=\"1\"><b><a href=\"/cgi-bin/camp_del.cgi?campaign_id=$camp_id\" target=_blank>D</a></b>&nbsp;&nbsp;<b><a href=\"/cgi-bin/camp_lock.cgi?campaign_id=$camp_id&stype=$stype&lock_flag=$auto_lock\" target=\"hidden\">$auto_lock_str</a></b>"; 
					}
				}
				else
				{
...
#    			}
			...
			else
			{
				...
				{
					print "<td width=\"171\"><font size=\"1\"><input type=\"checkbox\" value=\"${client_id}_${i}_${j}\" name=\"chkbox\"></font>&nbsp;&nbsp;<a href=\"schedule_advertiser.cgi?stype=$stype&xid=${client_id}_${i}_${j}&startdate=$cday_sav\" target=\"_blank\"><font size=1 face=\"Verdana\">Schedule</font></td>\n";
				}
				else
				{
