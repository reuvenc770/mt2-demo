...
my $cid= $query->param('id');
$sql="delete from company_info_contact where contact_id=$cid";
$sth = $dbhu->do($sql);
if ($dbhu->err() != 0)
{
	my $errmsg = $dbhu->errstr();
    util::logerror("Deleting company_info_contact record for $sql $user_id: $errmsg");
}
