...
if (!$hrCookie->{id}) {
	print "Location: third_party_login.cgi\n\n";
}
else {
	my $infoHR={};
#	$infoHR->{cookie}=$cookieID;
	$infoHR->{thirdID}=$hrCookie->{id};
...
sub display_available_deployed_campaign {
	...
	my $quer=qq|SELECT 3c.id, 3c.third_party_id, company, brand_name, advertiser_name, profile_name, 3c.client_id, |
			...
			.qq|date(3c.scheduled_datetime) as date FROM campaign c, 3rdparty_campaign 3c, advertiser_info a, |
			...
			.qq|c.deleted_date IS NULL AND 3c.third_party_id='$hrCookie->{id}' AND $date_clause |
	...
	foreach my $info (@info) {
		...
		print qq^
		<tr>
		  <td><a href="third_party_deployed.cgi?$href&id=$info->{id}&thirdID=$info->{third_party_id}&cID=$info->{client_id}&bID=$info->{brand_id}&catID=$info->{catid}&aID=$info->{advertiser_id}&campID=$info->{campaign_id}&mailer=$hrCookie->{id}#info">$dp_name</a></td>
		</tr>
		^;
	}
...
##        my $color=($count%2==0) ? "#E9E9E9" : "#FFFFFF";
		...
		my $report_purpose="";
		if ($hrCookie->{id}==5 || $hrCookie->{id}==9) {
...
##		my $footer_str = $infoHR->{DBID} . "_" . $infoHR->{sid} . "_" . $infoHR->{url_id};
		...
		if ($cwc3 ne "")
		{
			$html_code =~ s/{{URL}}/http:\/\/$infoHR->{redir_domain}\/cgi-bin\/redir1.cgi?eid={{EMAIL_USER_ID}}&cid={{CID}}&em={{EMAIL_ADDR}}&id=$infoHR->{DBID_linkid}&f=&s=&c=$cid&cwc3=$cwc3&cwcid=$cwcid&cwprogid=$cwprogid&cr=$cr&l=$landing_page/g;
		}
		else
		{
			$html_code =~ s/{{URL}}/http:\/\/$infoHR->{redir_domain}\/cgi-bin\/redir1.cgi?eid={{EMAIL_USER_ID}}&cid={{CID}}&em={{EMAIL_ADDR}}&id=$infoHR->{DBID_linkid}&f=&s=&c=$cid&cwcid=$cwcid&cwprogid=$cwprogid&cr=$cr&l=$landing_page/g;
		}
            ...
            <tr>
                ...
                <table cellSpacing="0" cellPadding="0" width="100%" border="0" id="table4">
                    <tr>
						...
						<td align="right"><b>
							<a style="TEXT-DECORATION: none" href="third_party_login.cgi?action=logout">
...
sub retrieve_cookie {
	...
	if ($hCookies{deploy}) {
		my %hCookie=$hCookies{deploy}->value;
		if ($hCookie{id}) {
  ...
  <table align=center border=0 width=100% bgcolor=#509c10>
    ...
    <tr>
      <td align=center bgcolor=#ffffff>
        <form method=post action="third_party_deployed.cgi">
        <table border=0 width=100%>
		  $mailer
          <tr height=20>
