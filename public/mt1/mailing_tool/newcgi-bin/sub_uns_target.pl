...
sub uns_upd_list_member
{
	...
	$email_addr =~ s/'/''/g;
	$email_exists = 0 ;
	$sql = "select email_user_id,email_list.status,user_id from email_list,list where email_addr= '$email_addr' and email_list.list_id=list.list_id and list.user_id=18";
