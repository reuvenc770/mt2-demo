...
if ($args->{submit}) {
	...
	if ($err_flag==1) {
		...
		if ($hrInfo->{user_id} eq 'slks') {
			$args->{redir}||='slks_deployed_view.cgi';
		}
		else {
			$args->{redir}||='slks_deployed_view_new.cgi';
		}
		my $cookie = "slkslogin=$hrInfo->{user_id}; path=/;";
	...
	clear_cookie();
	print "Location: slks_login.cgi?redir=$args->{redir}\n\n";
}
else {
	display_header();
...
sub validate_data {
        ...
        if (!$hrArgs->{userID}) {
			...
			my $dataHR={'slks'=>'creative1', 'dave'=>'dave'};
...
sub display_login_form {
	...
	<form method="post" action="slks_login.cgi">
