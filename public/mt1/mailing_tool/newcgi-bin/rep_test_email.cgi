...
end_of_html
	...
	{
		$sql = "select hostname,ip_addr,email_addr,campaign_name,result_msg from test_emails_log,campaign where test_emails_log.campaign_id=campaign.campaign_id and test_date >= date_sub(curdate(),interval $cday day) and test_date < curdate() order by test_id"; 
	}
