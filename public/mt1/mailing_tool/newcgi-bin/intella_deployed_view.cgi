...
if (!$cookieID) {
	print "Location: intella_login.cgi?redir=intella_deployed_view.cgi\n\n";
}
else {
	my $infoHR={};
...
warn "$quer\n";
	...
	foreach (sort {$hr->{$a}{deploy_name} cmp $hr->{$b}{deploy_name}} keys %$hr) {
		print qq^
		<tr>
		  <td><a href="intella_deployed_view.cgi?$href&id=$_&thirdID=$hr->{$_}{third_party_id}&cID=$hr->{$_}{client_id}&catID=$hr->{$_}{catid}&aID=$hr->{$_}{advertiser_id}&campID=$hr->{$_}{campaign_id}&mailer=$args->{mailer}&sch_date=$hr->{$_}{sch_date}#info">$hr->{$_}{deploy_name} ($hr->{$_}{date})</a></td>
		</tr>
		^;
	}
            ...
            <tr>
                ...
                <table cellSpacing="0" cellPadding="0" width="100%" border="0" id="table4">
                    <tr>
						...
						<td align="right"><b>
							<a style="TEXT-DECORATION: none" href="intella_login.cgi?action=logout&redir=intella_deployed_view.cgi">
...
sub display_login_form {
	...
	<form method='post' action='intella_deployed_view.cgi'>
	  <tr>
		<td>
...
sub retrieve_cookie {
    ...
    if ($cookies{'intellalogin'} ne "0") {
        $login_ok = $cookies{'intellalogin'};
    }
    else {
        $login_ok = 0;
    }
