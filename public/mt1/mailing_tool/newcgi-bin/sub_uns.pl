...
sub uns_upd_list_member
{
	...
	$email_addr =~ s/'/''/g;
	$email_exists = 0 ;
	$sql = "select email_user_id,status,client_id from email_list where email_addr= '$email_addr'";
	...
	while (($emailid,$cstatus,$user_id) = $sth2->fetchrow_array())
	{
		if (($cstatus eq "A") || ($cstatus eq "P") || ($cstatus eq "G") || ($cstatus eq "L"))
		{
			...
			$sql = "update $emailList set status = 'U', unsubscribe_date=curdate(),unsubscribe_time=curtime() where email_user_id=$emailid and status in ('A','P')"; 
			$rows = $dbhu->do($sql) ;
