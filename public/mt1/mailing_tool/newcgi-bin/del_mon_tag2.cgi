...
<TD vAlign=top align=left bgColor=#FFFFFF>
    ...
    <TD vAlign=top align=left bgColor=#ffffff colSpan=10>
		...
		<FORM action="emailreach.cgi" method=post>
...
end_of_html
$sql="select distinct url from brand_url_info where brand_id=? order by url";
$sth=$dbhu->prepare($sql);
$sth->execute($bid);
my $url;
while (($url)=$sth->fetchrow_array())
{
	print "<tr><td>$url</td><TD vAlign=center align=left><FONT face=\"verdana,arial,helvetica,sans serif\" color=#509C10 size=2><INPUT maxLength=50 size=50 name=h_$url></FONT></TD></TR>\n";
}
$sth->finish();
print<<"end_of_html";
                    <TR>
