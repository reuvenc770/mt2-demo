...
$util->db_connect();
$dbh = $util->get_dbh;
$dbh1=DBI->connect('DBI:mysql:new_mail:suppress.routename.com', 'db_user', 'sp1r3V') or die "can't connect to db: $!";
#my $emailInterface = Lib::Database::Perl::Interface::Email->new(('write' => 1));
...
sub mail_send
{
	...
	while (($client_id,$tab) = $sth1->fetchrow_array())
	{
		...
		$sql = "select list_id,list_name from list where status='A' and user_id=$client_id and list_id not in ($confirmed_list_id,$unconfirmed_list_id) order by list_id"; 
		$sth2 = $dbh->prepare($sql);
		$sth2->execute();
		my $lstr="";
		...
		chop($lstr);
		$move_cnt = 0;
		$sql = "select list_id,email_user_id from daily_click_log where list_id in ($lstr) and date_clicked < curdate()"; 
		$sth = $dbh1->prepare($sql);
		$sth->execute();
		while (($list_id,$email_user_id) = $sth->fetchrow_array())
		{
			print "moving $email_user_id to $click_list_id\n";
				
			if ($email_user_id < 1111832975)
			{
				$sql = "update email_list set list_id=$click_list_id,subscribe_date=curdate(),subscribe_time=curtime() where email_user_id=$email_user_id and status='A'";
			}
			else
			{
				$sql = "update email_list_new set list_id=$click_list_id,subscribe_date=curdate(),subscribe_time=curtime() where email_user_id=$email_user_id and status='A'";
			}
			$rows = $dbh->do($sql);
				
			if ($list_id != $click_list_id)
			{
				$move_cnt++;
			...
			#
			move_email_addr($email_user_id,$client_id,$CLICK);
		}
		$sth->finish();
		$sql = "delete from daily_click_log where date_clicked < curdate() and list_id in ($lstr)";			
		$rows = $dbh1->do($sql);
		if ($move_cnt > 0)
		{
			#update list to have get_data process list
			updateListStatus($click_list_id);
		}
	}
	$sql = "delete from daily_click_log where date_clicked < curdate() and list_id=$confirmed_list_id";			
	$rows = $dbh1->do($sql);
	$sql = "delete from daily_click_log where date_clicked < curdate() and list_id=$unconfirmed_list_id";			
	$rows = $dbh1->do($sql);
	$sth1->finish();
	foreach (keys %{$MOVE})
	{
    	...
    	{
        	$list_id=$_;
			$sql="insert into move_click_log(client_id,date_processed,list_id,reccnt) values($client_id,curdate(),$list_id,$MOVE->{$client_id}{$list_id})";
			$rows = $dbh->do($sql);
...
sub move_email_addr
{
	...
	where 
		...
		and l.list_name not in ('Confirmed','Unconfirmed')
	union
	select 
	...
	{
		$sql = "update $table set list_id=$CLICK->{$user_id},subscribe_date=curdate(),subscribe_time=curtime() where email_user_id=$teid and status='A'";
		$rows=$dbh->do($sql);
		$sql = "delete from daily_click_log where date_clicked < curdate() and list_id=$list_id and email_user_id=$teid";			
		$rows = $dbh1->do($sql);
	}
	$sth2a->finish();
}
