...
$sql="select date_format(date_sub(curdate(),interval 1 day),'%m-%d')";
$sth=$dbhu->prepare($sql);
$sth->execute();
($cdate)=$sth->fetchrow_array();
$sth->finish();
my $filename="XLUnsub_".$cdate.".txt";
my $infile="VirtumundoUnsub_".$cdate.".txt";
open(LOG,">XLUnsub_$cdate.txt");
$sql="select email_addr from unsub_log where client_id in (751,912,911,750,752) and unsub_date >= date_sub(curdate(),interval 1 day) and unsub_date < curdate()";
$sth=$dbhu->prepare($sql);
$sth->execute();
...
if ($ftp)
{
    $ftp->login('virtunsub','qpaLs)@') or print "Cannot login ", $ftp->message;
    $ftp->ascii();
	$ftp->get($infile);
	$ftp->cwd("XLUnsubs");
...
sub uns_upd_list_member
{
	...
	$email_addr =~ s/'/''/g;
	$email_exists = 0 ;
	$sql = "select email_user_id,status,client_id from email_list where email_addr= '$email_addr' and client_id in (751,912,911,750,752)";
