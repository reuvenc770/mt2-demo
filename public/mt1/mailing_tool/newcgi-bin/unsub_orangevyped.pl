...
my $username="vypunsubs";
my $password="8haspAhu";
...
sub uns_upd_list_member
{
	...
	$email_addr =~ s/'/''/g;
	$email_exists = 0 ;
	$sql = "select email_user_id,email_list.status,client_id from email_list where email_addr= '$email_addr' and client_id=2645"; 
