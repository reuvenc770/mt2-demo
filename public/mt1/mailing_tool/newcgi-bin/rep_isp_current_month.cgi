...
#
# this page displays the ISP Open/Clicks Current Month Report 
#
# History
...
$cdate = $year_str . "-" . $month_str . "-01";
    if ($networkopt > 0)
    {
$sql = "(select campaign_id,payout,campaign_name,category_name,date_format(sent_datetime,'%m/%d/%Y'),substr(dayname(sent_datetime),1,3),unsubscribe_cnt,bounce_cnt,fullmbx_cnt,format(unsubscribe_cnt,0),format(bounce_cnt,0),format(fullmbx_cnt,0),id,notdelivered_cnt,format(notdelivered_cnt,0),list_profile.aol_flag,list_profile.hotmail_flag,list_profile.yahoo_flag,list_profile.other_flag,campaign.profile_id,sent_datetime,brand_id from campaign,advertiser_info,category_info,list_profile where sent_datetime >= '$cdate' and sent_datetime <= curdate() and campaign.advertiser_id=advertiser_info.advertiser_id and advertiser_info.category_id=category_info.category_id and campaign.profile_id=list_profile.profile_id and list_profile.client_id=$networkopt) order by sent_datetime";
    }
	else
	{
$sql = "select campaign_id,payout,campaign_name,category_name,date_format(sent_datetime,'%m/%d/%Y'),substr(dayname(sent_datetime),1,3),unsubscribe_cnt,bounce_cnt,fullmbx_cnt,format(unsubscribe_cnt,0),format(bounce_cnt,0),format(fullmbx_cnt,0),id,notdelivered_cnt,format(notdelivered_cnt,0),aol_flag,hotmail_flag,yahoo_flag,other_flag,campaign.profile_id,sent_datetime,brand_id from campaign,advertiser_info,category_info where sent_datetime >= '$cdate' and sent_datetime <= curdate() and campaign.advertiser_id=advertiser_info.advertiser_id and advertiser_info.category_id=category_info.category_id order by sent_datetime"; 
	}
$sth = $dbhq->prepare($sql);
$sth->execute();
open(LOG,">/tmp/log.a");
