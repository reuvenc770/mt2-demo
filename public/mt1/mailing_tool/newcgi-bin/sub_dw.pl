...
sub uns_upd_list_member
{
	...
	$email_addr =~ s/'/''/g;
	$email_exists = 0 ;
	$sql = "select email_user_id,status from email_list where email_addr= '$email_addr' and list_id in (select list_id from list where user_id=22)";
	...
	if (($emailid,$cstatus) = $sth2->fetchrow_array())
	{
		if (($cstatus eq "A") || ($cstatus eq "P") || ($cstatus eq "G") || ($cstatus eq "L"))
		{
			...
			$rows = $dbh->do($sql) ;
			$sql = "insert into unsub_log(email_addr,unsub_date,client_id) values('$email_addr',curdate(),22)";
			$rows = $dbh->do($sql);
			$total_unscnt++;
	...
	{
		$sql="delete from email_chunk_list_22 where email_user_id=$emailid";
		$rows = $dbh->do($sql);
	}
	$sth2->finish();
