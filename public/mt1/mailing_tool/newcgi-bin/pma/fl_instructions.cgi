...
#----- check for login --------
my $user_id = master::check_security();
if ($user_id == 0)
{
...
master::header("CURRENT FILE LAYOUT");    #--- Print HTML Header ---
...
<TR>
	...
	<TD colspan=3 width="95%" valign="top" align="left">
		...
		is blank then you may enter any dmasterred data.  If there are values you may
...
sub get_user_file_layout
{
	...
	$sql = qq{select 
		...
		from tranzact_file_layout where user_id = $user_id};
	$sth = $dbh->prepare($sql) ;
	$sth->execute();
