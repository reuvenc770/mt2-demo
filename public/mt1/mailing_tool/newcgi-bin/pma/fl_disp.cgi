...
pma::header("Add/Update FILE LAYOUT");    # Print HTML Header
...
sub disp_file_layout
{
	...
	$sql = qq{select 
		...
		date_capture_pos,	member_source_pos,	 phone_pos, url_pos 
		from tranzact_file_layout where client_name = '$client_name'};
	$sth = $dbh->prepare($sql) ;
	$sth->execute();
	...
	<input type=hidden name=puserid value='$client_name'>
	...
	<TD align=middle>
		...
		<TBODY>
			<TR bgColor="$light_table_bg" height=15>
				...
				<br>
				Define the 'File Layout' for batch uploads by selecting from the dpmared 
