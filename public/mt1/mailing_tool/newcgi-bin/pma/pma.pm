#################################################################
####   pma.pm  - utility package for PMS					 ####
#################################################################
...
sub initialize 
{
	...
	$self->{'_db_user'} = "leaddog";		# MySQL database username
	$self->{'_db_password'} = "mymail";    	# MySQL database password
	$self->{'_db_database'} = "new_mail";		# MySQL database name
	$self->{'_ads_db_database'} = "ads";	# MySQL Webtraxx database name
	$self->{'_test_db_database'} = "test";	# MySQL Test database name
	$self->{'_pms_db_database'} = "pms";	# MySQL Test database name
	$self->{'_pms_db_ip'} = ":";					# MySQL db ip address, blank for local machine
	$self->{'_supp_db_database'} = "supp";	# MySQL Test database name
	$self->{'_supp_db_ip'} = ":";					# MySQL db ip address, blank for local machine
	$self->{'_supp2_db_ip'} = ":";					# MySQL db ip address, blank for local machine
...
sub test_db_connect
{
	my $self = shift;
	use DBI;
	my $db_database = $self->{'_test_db_database'};
	my $db_ip = $self->{'_db_ip'};
	my $db_user = $self->{'_db_user'};
	my $db_password = $self->{'_db_password'};
	my $dbh = DBI->connect("DBI:mysql:$db_database$db_ip",$db_user,$db_password);
...
sub pms_db_connect
{
	my $self = shift;
	use DBI;
	my $db_database = $self->{'_pms_db_database'};
	my $db_ip = $self->{'_pms_db_ip'};
	my $db_user = $self->{'_db_user'};
	my $db_password = $self->{'_db_password'};
	my $dbh = DBI->connect("DBI:mysql:$db_database$db_ip",$db_user,$db_password);
	$self->{'_dbh1'} = $dbh;
...
sub supp_db_connect
{
	my $self = shift;
	use DBI;
	my $db_database = $self->{'_supp_db_database'};
...
sub supp2_db_connect
{
	my $self = shift;
	use DBI;
	my $db_database = $self->{'_supp_db_database'};
	my $db_ip = $self->{'_supp2_db_ip'};
	my $db_user = $self->{'_db_user'};
	my $db_password = $self->{'_db_password'};
	my $dbh = DBI->connect("DBI:mysql:$db_database$db_ip",$db_user,$db_password);
	$self->{'_dbh3'} = $dbh;
...
sub ads_db_connect
{
	my $self = shift;
	use DBI;
	my $db_database = $self->{'_ads_db_database'};
	my $db_ip = $self->{'_db_ip'};
	my $db_user = $self->{'_db_user'};
	my $db_password = $self->{'_db_password'};
	my $dbh = DBI->connect("DBI:mysql:$db_database$db_ip",$db_user,$db_password);
...
<!--	<tr>
	<TD width=248 bgColor=#FFFFFF rowSpan=2>
		<img border="0" src="/mail-images/top.jpg"></TD>
...
<head>
<meta http-equiv="Content-Type" content="text/html; charset=windows-1252">
<title>Master Email System</title>
</head>
<body link="#000000" vlink="#000000" alink="#000000">
<TABLE cellSpacing=0 cellPadding=0 align=left bgColor=#ffffff border=0>
<TBODY>
...
<!--		<tr>
		<td align="center">
			<br><iframe src="/adcgi-bin/pms_dispban.cgi?campaign_id=1&client_id=1&delivery=iframe"
