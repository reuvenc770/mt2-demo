...
($dbhq,$dbhu)=$util->get_dbh();
	...
	while (($server) = $sth->fetchrow_array()) {
			print "Sending redir.dbm to $server\n";
		$ftp = Net::FTP->new("$server", Timeout => 120, Debug => 0, Passive => 0) or print "Cannot connect to $server: $@\n";
		print "After connect\n";
		if ($ftp) {
			$ftp->login($username,'18years') or print "Cannot login ", $ftp->message;
			$ftp->ascii();
			print "After login\n";
			$ftp->put("${some_dir}/redir.dbm") or print "put failed ", $ftp->message;
			print "After put\n";
			$ftp->quit;
			print "Sent redir.dbm to $server\n";
		}
	}
	$sth->finish();
$dbhq->disconnect;
