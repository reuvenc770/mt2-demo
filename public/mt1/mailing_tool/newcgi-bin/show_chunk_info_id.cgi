#!/usr/bin/perl
#===============================================================================
# Purpose: Display chunk list info for an id
# Name   : show_chunk_info_id.cgi 
#
#--Change Control---------------------------------------------------------------
# 09/27/06  Jim Sobeck  Creation
#===============================================================================
...
end_of_html
$sql="select email_addr from master_email_chunk_list where email_user_id=$tid";
my $sth2a=$dbh2->prepare($sql);
$sth2a->execute();
if (($cemail) = $sth2a->fetchrow_array())
{
...
{
	$sql = "select first_name,last_name,address,city,state,zip,source_url,capture_date,member_source,unsubscribe_date,dob,gender,status,subscribe_date from email_chunk_list_${user_id} where email_user_id=$tid"; 
	print LOG "<$sql>\n";
	$sth = $dbh2->prepare($sql);
	print LOG "After prepare\n";
	$sth->execute();
	print LOG "After execute \n";
	if (($fname,$lname,$addr,$city,$state,$zip,$source_url,$cdate,$cip,$unsub_date,$birth_date,$gender,$cstatus,$sub_date) = $sth->fetchrow_array())
	{
...
print<<"end_of_html";
			<td align="center"> <a href="remove_user_chunk.cgi?eid=$tid&global=Y">Add to Global Suppression</A></td>
<!--			<td align="center"> <A HREF="unsub.cgi?eid=$tid&global=D" onClick="return confirm('Are you sure you want to remove <$cdomain>?\\nClick OK to remove');">Add domain to Global Suppression</A></td>
			<td align="center"><a href="/cgi-bin/show_band.cgi?eid=$tid">Show Info. for Bandwidth Providers</a></td> -->
			<td align="center">
				<A HREF="/cgi-bin/show_chunk_info_id.cgi">Return to Search</a></td>
			</tr>
			</table>
