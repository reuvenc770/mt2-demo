...
<TD vAlign=top align=left bgColor=white>
    ...
    <TD vAlign=top align=left bgColor=#ffffff colSpan=10><!-- doing ct-table-open -->
        ...
        <TBODY>
        <TR>
        <TD colSpan=11><FONT face="verdana,arial,helvetica,sans serif" color=#509C10 
...
<form method=post action="/cgi-bin/camp_save_sent.cgi"><input type=hidden name=campaign_id value=$campaign_id><input type=hidden name=sent_cnt value=$sent_cnt>
                ...
                <TD align=middle width=50 height=15><FONT face=Verdana,Arial,Helvetica,sans-serif 
                    color=white size=1><B>User Sent #</B> </FONT></TD>
...
#
$sql = "select sum(sent_cnt),sum(open_cnt),sum(click_cnt),sum(action_cnt),sum(user_sent_cnt) from campaign_log where campaign_id=$campaign_id";
$sth = $dbhq->prepare($sql);
$sth->execute();
($sent_cnt,$opened_cnt,$click_cnt,$total_conversions,$user_sent_cnt) = $sth->fetchrow_array();
$sth->finish();
if ($user_sent_cnt == 0)
{
	$user_sent_cnt=$sent_cnt;
}
#
...
print "<TD align=center>";
print "<FONT face=\"verdana,arial,helvetica,sans serif\" color=#000000 size=2>\n";
print "<input type=text name=user_cnt size=10 maxlenght=10 value=$user_sent_cnt></font></TD>\n";
...
print<<"end_of_html";
            ...
            <TD colspan=2 align="left">
				<a href="mainmenu.cgi"><IMG src="$images/home_blkline.gif" border=0></a> &nbsp;&nbsp;<input type=image src="/images/save.gif"></TD>
			</TR>
			</TBODY>
			</TABLE>
			</form>
