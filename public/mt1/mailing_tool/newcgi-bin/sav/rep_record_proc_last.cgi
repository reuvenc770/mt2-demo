...
<center>
<h3>Record Processing Report - Last Month</h3>
end_of_html
#
...
$sth->finish();
open(LOG,"> /tmp/k2.");
print LOG "<$month_str> <$year_str> <$sql>\n";
close(LOG);
...
while ($client_id <= $set_id)
{
	...
	$sth->finish();
$sql = "select date_format(date_processed,'%m/%d/%Y'),sum(records_received),sum(duplicates),sum(bad_format),sum(bad_domain),sum(supp_cnt),sum(yahoo_cnt),sum(others_cnt),sum(aol_cnt),sum(hotmail_cnt) from record_processing where client_id=$client_id and date_processed >= '$cdate1' and date_processed < '$cdate' group by 1 order by 1";
open(LOG,">/tmp/k1.");
print LOG "<$sql>\n";
close(LOG);
$sth = $dbhq->prepare($sql);
