...
sub process_file 
{
	...
	$sql = "select SQL_BUFFER_RESULT email_user_id,email_addr from member_list where email_user_id > 30000000 and email_user_id <= 36404548 and status='A'";
...
sub chk_user()
{
	...
	$sql = "select email_user_id from user_state_info2 where email_user_id=$email_user_id";
	$sth = $dbh->prepare($sql);
	$sth->execute();
	if (($tmp_id) = $sth->fetchrow_array())
	{
		$sth->finish();
		return;
	}
