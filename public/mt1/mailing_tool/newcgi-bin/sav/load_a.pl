...
$sql = "select list_id from list where list_id=73 or list_id=79 order by list_id";
$sth = $dbh->prepare($sql);
$sth->execute();
while (($list_id) = $sth->fetchrow_array())
{
	if ($list_id == 73)
	{
		$sql = "select SQL_BUFFER_RESULT email_addr,subscribe_datetime from member_list where list_id=$list_id and email_user_id >= 34655945 and status='A' order by email_user_id";
	}
	else
	{
	...
	{
		$sql = "select email_user_id from member_list1 where email_addr='$email_addr'";
		$sth2 = $dbh->prepare($sql);
		$sth2->execute();
		if (($tid) = $sth2->fetchrow_array())
		{
			$sth2->finish();
		}
		else
