...
#
$sql = "delete from schedule_info where property='$property' and scheduled_datetime >= curdate()";
my $rows = $dbh1->do($sql);
#
$sql = "select campaign_name,campaign_id, emails_sent,status,scheduled_date,max_emails from campaign where scheduled_date >= curdate() and scheduled_date <= date_add(curdate(),interval
7 day) and deleted_date is null order by scheduled_date";
$sth = $dbh->prepare($sql);
$sth->execute();
