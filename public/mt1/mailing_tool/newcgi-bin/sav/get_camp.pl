...
my $bin_dir_http;
	...
	$user_id = 2;
	$csite=$user_arr[$user_id][$0];
	$csite1=$csite;
	$csite2=$user_arr[$user_id][$1];
	$footer_color="Black";
	...
	open (MAIL,">/var/www/html/new_templates/camp_${camp_id}.txt");
   	print MAIL "From: {{FROMADDR}}\n";
   	print MAIL "To: {{EMAIL_ADDR}}\n";
   	print MAIL "Reply-To: campaign\@{{DOMAIN}}\n";
   	print MAIL "Subject: {{SUBJECT}}\n";
	print MAIL "Date: {{MAILDATE}}\n";
	print MAIL "Message-ID: <{{MSGID}}>\n";
	print MAIL "X-Email: {{EMAIL_ADDR}}:{{CID}}:\n";
...
#   	print MAIL "Content-Type: text/html; charset=us-ascii\n\n";
    ...
    $the_email =~ s/\x97//g;
    $the_email =~ s/\x82//g;
	print MAIL "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\" \"http://www.w3.org/TR/html4/loose.dtd\">\n";
...
#	$template_text =~ s/{{CID}}/$camp_id/g;
	...
	$tracking_str = "<IMG SRC=\"http://{{DOMAIN}}/cgi-bin/open_email1.cgi?id={{EMAIL_USER_ID}}&amp;cid={{CID}}&amp;f={{FID}}&amp;s={{SID}}&amp;c={{CRID}}\" border=0 height=1 width=1 alt=\"tracking img\">";
	$template_text =~ s\<body>\<body><center><p STYLE="font-size:10pt; font-family:arial">{{SNAME}}, If you cannot view the images in this message, enable images or <a href="{{URL}}">visit here</a> to see this epromo.</p></center><p>${tracking_str}\;
...
{{UNSUBSCRIBE}} 
		...
		$template_text .= "<p STYLE=\"font-size:10pt; font-family:arial\">Intended recipient: {{EMAIL_ADDR}}<br>If you prefer not to receive these epromos from us, <a href=\"http://{{DOMAIN}}/cgi-bin/redir1.cgi?eid={{EMAIL_USER_ID}}&amp;cid={{CID}}&amp;em={{EMAIL_ADDR}}&amp;id=42&amp;nid={{NID}}\" target=_blank>follow this link</a>.</p></body></html>";
...
system(@args) == 0 or die "system @args failed: $?";
                        	...
                        	$temp_str = "<a href=\"http://{{DOMAIN}}/cgi-bin/redir1.cgi?eid={{EMAIL_USER_ID}}&amp;cid=0&amp;em={{EMAIL_ADDR}}&amp;id=$link_id\" target=\"_blank\"><img src=\"http://www.{{IMG_DOMAIN}}/images/unsub/$unsub_img\" border=0 alt=\"Unsub\"></a><br><br>";
                    	}
					}
			if ($footer_color eq "WHITE")
			{
                $template_text =~ s\{{UNSUBSCRIBE}}\$temp_str<a href="http://{{DOMAIN}}/cgi-bin/redir1.cgi?eid={{EMAIL_USER_ID}}&amp;cid={{CID}}&amp;em={{EMAIL_ADDR}}&amp;id=42&amp;nid={{NID}}" target=_blank><img src="http://www.{{IMG_DOMAIN}}/images/footer/{{FOOTER_DOMAIN}}_1.jpg" border=0 alt="If this image does not load and you would rather not receive these messages, simply reply to this e-mail with 'remove' in the body of the message."></a><br><a href="http://{{DOMAIN}}/cgi-bin/redir1.cgi?cid=0&amp;eid={{EMAIL_USER_ID}}&amp;id=51&amp;nid={{NID}}" target=_blank><img src="http://www.{{IMG_DOMAIN}}/images/footer/{{FOOTER_DOMAIN}}_2.jpg" border=0 alt="Privacy"></a>\g;
			}
			else
			{
                $template_text =~ s\{{UNSUBSCRIBE}}\$temp_str<a href="http://{{DOMAIN}}/cgi-bin/redir1.cgi?eid={{EMAIL_USER_ID}}&amp;cid={{CID}}&amp;em={{EMAIL_ADDR}}&amp;id=42&amp;nid={{NID}}" target=_blank><img src="http://www.{{IMG_DOMAIN}}/images/footer/{{FOOTER_DOMAIN}}_1.jpg" border=0 alt="If this image does not load and you would rather not receive these messages, simply reply to this e-mail with 'remove' in the body of the message"></a><br><a href="http://{{DOMAIN}}/cgi-bin/redir1.cgi?cid=0&amp;eid={{EMAIL_USER_ID}}&amp;id=51&amp;nid={{NID}}" target=_blank><img src="http://www.{{IMG_DOMAIN}}/images/footer/{{FOOTER_DOMAIN}}_2.jpg" border=0 alt="privacy"></a>\g;
...
sub cb 
{
	...
	my $ext;
     ...
     if (($tag eq "img") or ($url1 eq "background"))
     {
        $_ = $url2;
