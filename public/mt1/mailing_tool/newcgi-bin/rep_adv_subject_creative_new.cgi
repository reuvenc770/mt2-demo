...
while (($creative_name,$tcid,$click_percent) = $sth->fetchrow_array())
{
	...
	while ($i < $cidcnt)
	{
		$sql = "select (sum(click_cnt)/sum(open_cnt))*100 from email_log where creative_id=? and campaign_id = ? and open_cnt > 0 and sent_cnt > 0";
		$sth1 = $dbhq->prepare($sql);
		$sth1->execute($tcid, $CIDARR[$i]);
...
end_of_html
$i=0;
##$sql="select creative_name,creative.creative_id,(sum(click_cnt)/sum(sent_cnt)*100000) from creative,email_log where creative.creative_id=email_log.creative_id and campaign_id in ($cid_str) and open_cnt > 0 and sent_cnt > 0 group by creative_name,creative.creative_id order by creative_name";
