...
sub process_file 
{
	...
	while (<SAVED>) 
	{
		...
		$total_cnt++;
		&uns_upd_list_member($email_addr, $user_id);
	} 
...
sub uns_upd_list_member
{
	...
	$email_addr =~ s/'/''/g;
	$email_exists = 0 ;
	$sql = "select email_user_id,status from email_list where email_addr= '$email_addr' and list_id in (select list_id from list where user_id= $user_id)";
