...
# *****************************************************************************************
# category_triggera.cgi
#
# this page display main page for setting up triggers for category 
#
# History
# Jim Sobeck, 10/03/05, Creation
# Jim Sobeck, 11/16/07, Added checkboxes on whether to calculate creative or not
# *****************************************************************************************
...
my $cday;
#
$sql="select calc_trigger1,calc_trigger2,calc_alt from category_trigger where category_id=$cid and client_id=$client_id and trigger_type='$trigger_type'";
$sth=$dbhu->prepare($sql);
$sth->execute();
($calc_trigger1,$calc_trigger2,$calc_alt)=$sth->fetchrow_array();
$sth->finish();
if ($calc_trigger1 eq "")
{
	$calc_trigger1="Y";
...
print<<"end_of_html";
    ...
    var selObj2 = document.getElementById('alt_trigger');
    var selIndex2 = selObj2.selectedIndex;
	setTimeout("chkwait()",100);
	parent.frames[1].location = "/newcgi-bin/chk_category_trigger.cgi?cid=$cid&trigger_type=$trigger_type&trigger1="+selObj.options[selIndex].value+"&trigger2="+selObj1.options[selIndex1].value+"&alt_trigger="+selObj2.options[selIndex2].value;
end_of_html
}
else
{
...
function addTriggerOption3(value,text)
{
    var newOpt = document.createElement("OPTION");
    newOpt.text=text;
    newOpt.value=value;
    campform.alt_trigger.add(newOpt);
}
...
function addOption3(value,text)
{
	...
	newOpt.text=text;
	newOpt.value=value;
	campform.advertiser_id3.add(newOpt);
}
...
function update_advertiser3(tid)
{
	var selObj = document.getElementById('catid3');
	var selIndex = selObj.selectedIndex;
	var selLength = campform.advertiser_id3.length;
	while (selLength>0)
	{
		campform.advertiser_id3.remove(selLength-1);
		selLength--;
	}
	campform.advertiser_id3.length=0;
	parent.frames[1].location="/newcgi-bin/upd_advertiser_list4.cgi?cid="+selObj.options[selIndex].value;
}
function update_subject()
{
	parent.frames[1].location="/newcgi-bin/upd_cat_creative_list.cgi?cid=$cid&client_id=$client_id";
}
function set_fields(trigger_creative,trigger_creative2,alt_trigger,catid1,catid2,catid3,advertiser_id1,advertiser_id2,advertiser_id3)
{
  	var i;
  	var selObj = document.getElementById('catid1');
  	...
  	var selObj = document.getElementById('catid3');
  	for (i=0; i<selObj.options.length; i++) {
    	if (selObj.options[i].value == catid3) { selObj.selectedIndex = i; break; }
	}
  	...
  	var selObj = document.getElementById('advertiser_id3');
  	for (i=0; i<selObj.options.length; i++) {
    	if (selObj.options[i].value == advertiser_id3) { selObj.selectedIndex = i; break; }
	}
  	var selObj = document.getElementById('trigger_creative');
  	for (i=0; i<selObj.options.length; i++) {
  	...
  	var selObj = document.getElementById('alt_trigger');
  	for (i=0; i<selObj.options.length; i++) {
    	if (selObj.options[i].value == alt_trigger) { selObj.selectedIndex = i; break; }
	}
}
...
function update_creative3()
{
	var selObj = document.getElementById('advertiser_id3');
	var selIndex = selObj.selectedIndex;
	var selLength = campform.alt_trigger.length;
	while (selLength>0)
	{
		campform.alt_trigger.remove(selLength-1);
		selLength--;
	}
	parent.frames[1].location="/newcgi-bin/upd_creative_list3.cgi?aid="+selObj.options[selIndex].value;
}
</script>
</head>
	...
	<tr>
		...
		<table cellSpacing="0" cellPadding="10" width="100%" bgColor="#999999" border="0" id="table4">
			<tr>
				...
				<form name="campform" method="post" action="/cgi-bin/category_trigger_save.cgi" target=_top>
...
&nbsp;</td>
									...
									<table cellSpacing="0" cellPadding="0" width="455" bgColor="#e3fad1" border="0" id="table11">
										<tr bgColor="#509c10">
											...
											<b>Default Trigger By Category</b></font></td>
...
$sth->finish;
print "</select>";
if ($calc_trigger1 eq "Y")
{
	print "&nbsp;&nbsp;<input type=checkbox value=Y name=calc_trigger1 checked>Use Best Performing Creative\n";
}
else
{
	print "&nbsp;&nbsp;<input type=checkbox value=Y name=calc_trigger1>Use Best Performing Creative\n";
}
print<<"end_of_html";
</td>
										</tr>
...
$sth->finish;
print "</select>";
if ($calc_trigger2 eq "Y")
{
	print "&nbsp;&nbsp;<input type=checkbox value=Y name=calc_trigger2 checked>Use Best Performing Creative\n";
}
else
{
	print "&nbsp;&nbsp;<input type=checkbox value=Y name=calc_trigger2>Use Best Performing Creative\n";
}
print<<"end_of_html";
</td>
										</tr>
...
<option value="0" selected>NONE</option>
										...
										<tr>
											<td colSpan="3">&nbsp;&nbsp;
											<font face="verdana,arial,helvetica,sans serif" color="#509c10" size="2">
											<b>Trigger Email Alternate (If advertiser selected is the same as a trigger)</b></font></td>
										</tr>
										</tr>
										<tr>
											<td colSpan="3">&nbsp;&nbsp;
										...
										<tr>
											<td colSpan="3">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
											<select name="catid3" onChange="update_advertiser3(0);">
											<option value="-1">ALL CATEGORIES</option>
end_of_html
$sql = "select category_id,category_name from category_info order by category_name";
$sth = $dbhq->prepare($sql);
...
if ($calc_alt eq "Y")
{
	print "&nbsp;&nbsp;<input type=checkbox value=Y name=calc_alt checked>Use Best Performing Creative\n";
}
else
{
	print "&nbsp;&nbsp;<input type=checkbox value=Y name=calc_alt>Use Best Performing Creative\n";
}
print<<"end_of_html";
</td>
										</tr>
										...
										<tr>
											<td colSpan="3">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
											<select name="advertiser_id3" onChange="update_creative3();">
											<option value="-1" selected>NONE
											</option>
end_of_html
if ($alt_trigger > 0)
{
	$sql = "select advertiser_id,advertiser_name from advertiser_info where category_id in (select category_id from advertiser_info,creative where creative_id=$alt_trigger and creative.advertiser_id=advertiser_info.advertiser_id) and status='A' and test_flag='N' order by advertiser_name";
}
else
{
	$sql = "select advertiser_id,advertiser_name from advertiser_info where status='A' and test_flag='N' order by advertiser_name"; 
...
print<<"end_of_html";
										...
										<tr>
											<td colSpan="3">&nbsp;&nbsp;&nbsp;&nbsp;<font color="#509c10"> </font>
											<font face="verdana,arial,helvetica,sans serif" color="#509c10" size="2">
											<b>&nbsp;</b></font>&nbsp;<select name="alt_trigger">
<option value="0" selected>NONE</option>
											</select> </td>
										</tr>
										<tr>
										...
										<tr>
											...
											<table cellSpacing="0" cellPadding="0" width="100%" border="0" id="table12">
												<tr>
													...
													<img height="22" src="/images/save_rev.gif" width="81" border="0" onClick="ProcessForm()"> <a href="/cgi-bin/category_trigger_list.cgi?userid=$client_id&trigger_type=$trigger_type" target=_top><img src="/images/cancel.gif" border=0></a>
													</td>
												</tr>
											</table>
											</td>
