...
#		$email_addr =~ s/\s//g ;     # remove all white space global
	...
	print "Total Actived Count = $total_addcnt\n";
	print "Total Already Activated/Unsubscribed Count = $total_alreadycnt\n";
...
sub uns_upd_list_member
{
	...
	$email_addr =~ s/'/''/g;
	$email_addr =~ tr/A-Z/a-z/;
	$email_exists = 0 ;
	$sql = "update member_list1 set status = 'A' where email_addr = '$email_addr' and status = 'P'";
