...
$sql="select third_party_id,campaign_id,advertiser_id,brand_id, vsgID,deploy_name,date_format(scheduled_datetime,'%m/%d/%Y'),date_format(scheduled_datetime,'%h'),date_format(scheduled_datetime,'%p'),client_id from 3rdparty_campaign where id=$camp_id"; 
$sth = $dbhq->prepare($sql);
$sth->execute();
($id,$temp_cid,$aid,$bid,$vsgID, $cname,$tid0,$tid1,$am_pm,$client_id)=$sth->fetchrow_array();
$sth->finish();
#
#	Get information about the mailer
#
$sql = "select mailer_name from third_party_defaults where third_party_id=?";
$sth = $dbhq->prepare($sql);
$sth->execute($id);
...
$sql = "select substr(profile_name,1,8) from list_profile,campaign where campaign.profile_id=list_profile.profile_id and campaign_id=?"; 
$sth = $dbhq->prepare($sql);
$sth->execute($temp_cid);
($pname)=$sth->fetchrow_array();
$sth->finish();
my $dir1a = $mailer_name . $network_name . $pname . $aname . $tid0 . $t_tid1 . $am_pm;
$dir1 =~ s/ //g;
$dir1 =~ s/\(//g;
$dir1 =~ s/\)//g;
$dir1 =~ s/\$//g;
	...
	<tr vAlign="top">
		...
		<table cellSpacing="0" cellPadding="10" width="100%" bgColor="#ffffff" border="0" id="table5">
			<tr>
				...
				<table cellSpacing="0" cellPadding="0" width="800" bgColor="#ffffff" border="0" id="table6">
					<tr>
...
rif" color="#509c10" size="3"><b>$cname</b></font><table border="0" width="100%" id="table9">
							...
							<tr>
								<td><b>Download:</b></td>
								<td><a href="/downloads/$dir1.zip">
								$dir1.zip</a></b><br>
								<b><a href="/downloads/$dir1a.zip">
								$dir1a.zip</a></b>
								</td>
							</tr>
							<tr>
