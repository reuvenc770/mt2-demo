#!/usr/bin/perl
#===============================================================================
# Purpose: Displays the HTML page to add seeds to delivery test seeds table 
...
<TD vAlign=top align=left bgColor=#FFFFFF>
	...
	<TD vAlign=top align=left bgColor=#ffffff colSpan=10>
		...
		<TD vAlign=center align=left><FONT face="verdana,arial,helvetica,sans serif" 
			color=#509C10 size=3> <B> Unique Deploy Seedlist</B> </FONT></TD>
		</TR>
		<TR>
...
end_of_html
my $seedlist_freq;
$sql="select parmval from sysparm where parmkey='SEEDLIST_FREQ'";
$sth = $dbhq->prepare($sql) ;
$sth->execute();
($seedlist_freq)=$sth->fetchrow_array();
$sth->finish();

$sql = "select email_addr from delivery_test_seeds where type='UNIQUE' order by email_addr";
$sth = $dbhq->prepare($sql) ;
$sth->execute();
my $cemail;
...
<tr><td>Seedlist Frequency: <input type=text name=seedlist_freq value="$seedlist_freq" size=7 maxlength=7> 
