...
while (($url,$aid) = $sth->fetchrow_array())
{
	...
	{
		# Remove old URLS
		$sql = "delete from advertiser_tracking where client_id =$ARGV[0] and advertiser_id=$aid and daily_deal='$ARGV[1]'";
		$rows = $dbhu->do($sql);
		#
...
#
			$sql="insert into advertiser_tracking(advertiser_id,url,code,date_added,client_id,link_id,daily_deal) values($aid,'$temp_url','$mid',curdate(),$client_id,$lid,'$ARGV[1]')";
			$rows = $dbhu->do($sql);
		}
		$sth2->finish();
	}
