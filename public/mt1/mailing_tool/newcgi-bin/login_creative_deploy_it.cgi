...
if (!$cookie) {
	print "Location: login_list_advertiser.cgi\n\n";
}
else {
#
...
#
my $sid;
$sql="select advertiser_name,vendor_supp_list_id,unsub_link,unsub_image,advertiser_url from advertiser where advertiser_id=?";
$sth = $dbhq->prepare($sql);
$sth->execute($aid);
($aname,$sid,$unsub_url,$unsub_img,$advertiser_url)=$sth->fetchrow_array();
$sth->finish();

$sql = " select list_name from vendor_supp_list_info where link_id=?"; 
$sth = $dbhq->prepare($sql);
$sth->execute($sid);
...
end_of_html
$got_it=0;
$sql="select advertiser_subject,sum(el.open_cnt)/sum(el.sent_cnt) from advertiser_subject,email_log el, campaign c where advertiser_subject.subject_id=el.subject_id and advertiser_subject.status='A' and advertiser_subject.advertiser_id=$aid and advertiser_subject.approved_flag='Y' and el.campaign_id=c.campaign_id and c.scheduled_date >= date_sub(curdate(),interval 60 day) and c.deleted_date is null and c.advertiser_id=$aid group by advertiser_subject order by 2 desc"; 
$sth = $dbhq->prepare($sql);
$sth->execute();
...
end_of_html
$got_it=0;
$sql="select advertiser_from,sum(el.open_cnt)/sum(el.sent_cnt) from advertiser_from,email_log el, campaign c where advertiser_from.from_id=el.from_id and advertiser_from.advertiser_id=$aid and advertiser_from != '{{FOOTER_SUBDOMAIN}}' and advertiser_from.approved_flag='Y' and el.campaign_id=c.campaign_id and c.scheduled_date >= date_sub(curdate(),interval 60 day) and c.deleted_date is null and c.advertiser_id=$aid group by advertiser_from order by 2 desc";
$sth = $dbhq->prepare($sql);
$sth->execute();
while (($subject_str,$temp_val)=$sth->fetchrow_array())
{
...
$got_it=0;
my $reccnt;
$sql="select count(*) from email_log el,creative_id,campaign c where el.creative_id=creative.creative_id and trigger_flag='N' and creative.status='A' and creative.advertiser_id=$aid and creative.approved_flag='Y' and el.campaign_id=c.campaign_id and c.scheduled_date >= date_sub(curdate(),interval 60 day) and c.deleted_date is null and c.advertiser_id=$aid";
$sth =$dbhq->prepare($sql);
$sth->execute();
...
if ($reccnt > 0)
{
	$sql="select el.creative_id,((sum(el.click_cnt)/sum(el.sent_cnt))*100000) from email_log el,creative,campaign c where el.creative_id=creative.creative_id and trigger_flag='N' and creative.status='A' and creative.advertiser_id=$aid and creative.approved_flag='Y' and el.campaign_id=c.campaign_id and c.scheduled_date >= date_sub(curdate(),interval 60 day) and c.deleted_date is null and c.advertiser_id=$aid group by email_log.creative_id order by 2 desc";
}
else
{
...
#
	...
	print "<b><u>Creative $camp_cnt: $creative_name ($cclick% Click - $temp_val Index - $temp_str</u></b><br>\n";
...
#
##	my $footer_str = $bid . "_" . $sid . "_" . $url_id;
	my $footer_str = $bid;
	...
	$html_code =~ s/{{IMG_DOMAIN}}/buyube.com/g;
...
print<<"end_of_html";
							<a href="/cgi-bin/login_camp_preview.cgi?campaign_id=$cid&format=H&cdeploy=Y" target=_blank>
...
print<<"end_of_html";
					...
					&nbsp;<table id="table8" cellPadding="5" width="66%" bgColor="white">
						<tr>
							...
							<a href="/cgi-bin/login_advertiser_list.cgi">
