...
<TD align='center'>
	...
	<td colspan=3>
		...
		<tr>
		<td align='center'><b><font face="Arial" size="2" color='#FFFFFF'>&nbsp;Append EIDs</FONT></b></td>
		</tr>
		<tr>
		<td>
		</td>
...
<center>
<h3>$ctype</h3>
<a href=appendEID_add.cgi?pid=0>Add Append EID</a>
<br><a href="mainmenu.cgi"><img src="$images/home_blkline.gif" border=0></a><br>
	...
	<TD vAlign=top align=left bgColor=#ffffff>
		...
		<th>ID</th><th>Filename</th><th>Append Client</th><th>Status</th></tr>
end_of_html
$sql="select ID,fileName,status,appendClient from AppendEID where lastUpdated >= date_sub(curdate(),interval 1 day) order by ID"; 
$sth=$dbhu->prepare($sql);
$sth->execute();
while (($ID,$fileName,$status,$appendClient) = $sth->fetchrow_array())
{
	$reccnt++;
    if ( ($reccnt % 2) == 0 )
    {
	...
	print "<tr bgcollor=$bgcolor><td>$ID</td><td>$fileName</td><td align=middle>$appendClient</td><td>$status</td></tr>\n";
}
