#!/usr/bin/perl
#===============================================================================
# Purpose: Schedule an individual slot 
# Name   : schedule_advertiser_save.cgi 
#
#--Change Control---------------------------------------------------------------
# 02/05/08  Jim Sobeck  Creation
#===============================================================================
...
#
my $chkbox= $query->param('xid');
if ($stype eq "N")
{
   ($nl_id,$slot_id,$cday) = split('_',$chkbox);
	$client_id=0;
}
else
{
   ($client_id,$slot_id,$cday) = split('_',$chkbox);
	...
	#
	my $exflag;
	$sql = "select substr('$exclude_days',dayofweek(date_add('$startdate',interval $cday-1 day)),1)"; 
	$sth = $dbhq->prepare($sql);
	$sth->execute();
    ...
    if ($reccnt == 0)
    {
		if ($stype eq "N")
		{
        	$sql = "select hour(schedule_time),nl_slot_info.profile_id,profile_name from nl_slot_info,list_profile where nl_slot_info.nl_id=$nl_id and slot_id=$slot_id and nl_slot_info.profile_id=list_profile.profile_id";
        	$sth = $dbhq->prepare($sql);
        	$sth->execute();
		...
		{
			$sql = "select hour(schedule_time),profile_id,brand_id,third_party_id,mta_id,performance,log_campaign from schedule_info where client_id=$client_id and slot_id=$slot_id and slot_type='$stype'";
			$sth = $dbhq->prepare($sql);
			$sth->execute();
			($stime,$profile_id,$brand_id,$third_id,$mta_id,$priority,$log_camp) = $sth->fetchrow_array();
			$sth->finish();
		}
...
#
		if ($stype eq "N")
		{
			...
			my $STHQ=$dbhq->prepare($sql);
			$STHQ->execute($profile_name,$nl_id);
			my $cpid;
			while (($cpid,$client_id) = $STHQ->fetchrow_array())
			{
				...
				{	
					...
					#
					$sql="select campaign_id from camp_schedule_info where nl_id=$nl_id and slot_id=$slot_id and slot_type='N' and schedule_date='$sdate' and client_id=$client_id and status='A'";
					...
					else
					{
						...
						$sth->finish();
				   		...
				   		$rows=$dbhu->do($sql);
						$sql="insert into camp_schedule_info(client_id,slot_id,slot_type,schedule_date,campaign_id,nl_id) values($client_id,$slot_id,'N','$sdate',$camp_id,$nl_id)";
						$rows=$dbhu->do($sql);
					}
				}
				$STHQ1->finish();
		...
		elsif (($stype eq "3") or ($third_id == $STRONGMAIL_ID))
		{
			$sql = "select mailer_name from third_party_defaults where third_party_id=$third_id";
			my $sthq = $dbhq->prepare($sql); 
			$sthq->execute(); 
...
#
		...
		{
			my $tcid;
			$sql="select campaign_id from camp_schedule_info where client_id=? and slot_id=? and slot_type=? and schedule_date=?";
			my $sthc=$dbhu->prepare($sql);
			$sthc->execute($client_id,$slot_id,$stype,$sdate);
			if (($tcid)=$sthc->fetchrow_array())
			{
				$sql="update campaign set deleted_date=now() where campaign_id=$tcid";
				$rows=$dbhu->do($sql);
				$sql="delete from camp_schedule_info where client_id=$client_id and slot_id=$slot_id and slot_type='$stype' and schedule_date='$sdate' and campaign_id=$tcid";
				$rows=$dbhu->do($sql);
			}
			$sthc->finish();
...
{
	print "<a href=\"/cgi-bin/newsletter_schedule.cgi\" target=_top>Newsletter Schedule</a>\n";
}
else
{
