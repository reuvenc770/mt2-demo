...
$sql = "select header_name,status,header_code from TraceHeaders where $userDataRestrictionWhereClause header_id= $template_id";
$sth=$dbhu->prepare($sql);
$sth->execute();
($template_name,$status,$templateCode)=$sth->fetchrow_array();
$sth->finish();
$template_name="Copy of ".$template_name;
$sql="insert into TraceHeaders(userID, header_name,dateAdded,status,header_code) values($user_id, '$template_name',now(),'A','$templateCode')";
$rows = $dbhu->do($sql);
if ($dbhu->err() != 0)
{
	$errmsg = $dbhu->errstr();
