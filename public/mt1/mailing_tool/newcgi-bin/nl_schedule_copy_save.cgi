...
#
	$sql="select campaign_id,slot_id,schedule_date from camp_schedule_info where nl_id=$network and slot_type='$stype' and schedule_date >= '$tdate' and schedule_date <= date_add('$tdate',interval $daycnt day)";
	$sth = $dbhq->prepare($sql);
	$sth->execute();
print LOG "<$sql>\n";
	while (($camp_id,$slot_id,$temp_date) = $sth->fetchrow_array())
	{
		$sql = "update campaign set deleted_date=now() where campaign_id=$camp_id and deleted_date is null";
		$rows = $dbhu->do($sql);
		$sql = "delete from camp_schedule_info where nl_id=$network and campaign_id=$camp_id and slot_id=$slot_id and schedule_date='$temp_date'";
		$rows = $dbhu->do($sql);
	}
	$sth->finish();
#
	$sql="select camp_schedule_info.campaign_id,slot_id,schedule_date,advertiser_info.advertiser_id,advertiser_info.category_id,advertiser_info.advertiser_name,camp_schedule_info.client_id,hour(campaign.scheduled_time) from camp_schedule_info,campaign,advertiser_info where camp_schedule_info.nl_id=$network and slot_type='$stype' and schedule_date >= '$sdate' and schedule_date <= '$edate' and slot_id in (select slot_id from nl_slot_info where nl_id=$network and status='A') and camp_schedule_info.campaign_id=campaign.campaign_id and campaign.advertiser_id=advertiser_info.advertiser_id and advertiser_info.status='A' and deleted_date is null"; 
	$sth = $dbhq->prepare($sql);
	$sth->execute();
print LOG "<$sql>\n";
	while (($camp_id,$slot_id,$temp_date,$aid,$catid,$aname,$client_id,$stime) = $sth->fetchrow_array())
	{
		#
...
#
			$sql = "select profile_id,brand_id from campaign where campaign_id=$camp_id"; 
			$sth1 = $dbhq->prepare($sql);
			$sth1->execute();
			...
			if ($tparty == $STRONGMAIL_ID)
			{
				...
				$sth1->finish();
				$sql = "insert into camp_schedule_info(client_id,slot_id,slot_type,schedule_date,campaign_id,nl_id) values($client_id,$slot_id,'$stype',date_add('$temp_date',interval $daycnt1 day),$new_camp_id,$network)";
				$rows = $dbhu->do($sql);
			}
#
