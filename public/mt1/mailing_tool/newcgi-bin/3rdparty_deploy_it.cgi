...
# *****************************************************************************************
# 3rdparty_deploy_it.cgi
#
# this page displays the 3rd party deployed page 
#
# History
...
my $id= $query->param('mailer_id');
my $bid = $query->param('brand_id');
my $vsgID=$query->param('vsgID');
...
if ($old_id > 0)
{
	$sql="update 3rdparty_campaign set third_party_id=$id, client_id=$client_id, vsgID='$vsgID', brand_id=$bid,catid=$catid, advertiser_id=$aid, deploy_name='$cname', scheduled_datetime='$sdate' where id=$old_id";
	$new_camp_id = $old_id;
}
else
{
	$old_id = 0;
	$sql="insert into 3rdparty_campaign(third_party_id,client_id,brand_id,catid,advertiser_id,deploy_name,scheduled_datetime,campaign_id, vsgID) values($id,$client_id,$bid,$catid,$aid,'$cname','$sdate',0, '$vsgID')";
}
my $rows=$dbhu->do($sql);
if ($old_id == 0)
{
...
if ($old_id > 0)
{
	...
	$sth->finish();
	$sql="update campaign set brand_id=$bid,scheduled_datetime='$sdate',campaign_name='$cname',advertiser_id=$aid,sent_datetime='$sdate',scheduled_date=date('$sdate'),scheduled_time=time('$sdate') where campaign_id=$temp_cid"; 
	$rows=$dbhu->do($sql);

	$sql="select substr(profile_name,1,8) from list_profile,campaign where campaign_id=$temp_cid and list_profile.profile_id=campaign.profile_id";
	$sth = $dbhq->prepare($sql);
	$sth->execute();
...
{
	$sql="select profile_id,substr(profile_name,1,8) from list_profile where client_id=$client_id and (profile_name='All' or (yahoo_flag='Y' and other_flag='Y')) and status='A'"; 
	$sth = $dbhq->prepare($sql);
	$sth->execute();
	($profile_id,$pname)=$sth->fetchrow_array();
	$sth->finish();
#
	my $ctype;
	if ($id == 10)
    ...
    $sql = "insert into campaign(campaign_name,user_id,status,created_datetime,advertiser_id,creative1_id,brand_id,scheduled_datetime,sent_datetime,profile_id,scheduled_date,scheduled_time,campaign_type) values('$cname',1,'C',now(),$aid,0,$bid,'$sdate','$sdate',$profile_id,date('$sdate'),time('$sdate'),'$ctype')";
	$rows=$dbhu->do($sql);
	...
	($temp_cid)=$sth->fetchrow_array();
	$sth->finish();
	$sql = "update 3rdparty_campaign set campaign_id=$temp_cid,deploy_name='$cname' where id=$new_camp_id";
	$rows=$dbhu->do($sql);
	#
...
if (($id eq "") or ($id == 10))
{
	$sql = "select link_id from advertiser_tracking where advertiser_id=$aid and client_id=$client_id and daily_deal='$id'";
}
else
{
...
#
my $dir1a = $mailer_name . $network_name . $pname . $aname . $tid0 . $tid1 . $am_pm;
#
$dir1a =~ s/ //g;
$dir1a =~ s/\(//g;
$dir1a =~ s/\)//g;
...
rif" color="#509c10" size="3"><b>$cname</b></font><table border="0" width="100%" id="table9">
							...
							<tr>
								<td><b>Download:</b></td>
								<td><b><a href="/downloads/$dir1a.zip">
								$dir1a.zip</a></b></td>
							</tr>
							<tr>
...
end_of_html
$sql="select advertiser_subject,sum(email_log.open_cnt)/sum(email_log.sent_cnt) from advertiser_subject,email_log,campaign where advertiser_subject.subject_id=email_log.subject_id and advertiser_subject.status='A' and advertiser_subject.advertiser_id=$aid and email_log.campaign_id=campaign.campaign_id and campaign.scheduled_date >= date_sub(curdate(),interval 60 day) and deleted_date is null and campaign.advertiser_id=$aid group by advertiser_subject order by 2 desc limit $num_subject";
$sth = $dbhq->prepare($sql);
$sth->execute();
...
print<<"end_of_html";
					...
					<b><u>From Line(s):</u></b>
end_of_html
$sql="select advertiser_from,sum(email_log.open_cnt)/sum(email_logsent_cnt) from advertiser_from,email_log,campaign where advertiser_from.from_id=email_log.from_id and advertiser_from.advertiser_id=$aid and email_log.campaign_id=campaign.campaign_id and campaign.advertiser_id=$aid and deleted_date is null and campaign.scheduled_date >= date_sub(curdate(),interval 60 day) group by advertiser_from order by 2 desc limit $num_from";
$sth = $dbhq->prepare($sql);
$sth->execute();
...
print<<"end_of_html";
					<br><br>
end_of_html
$sql="select email_log.creative_id,creative.creative_name,((sum(email_log.click_cnt)/sum(email_log.sent_cnt))*100000) from email_log,creative,campaign where email_log.creative_id=creative.creative_id and trigger_flag='N' and creative.status='A' and creative.advertiser_id=$aid and campaign.advertiser_id=$aid and email_log.campaign_id=campaign.campaign_id and campaign.scheduled_date >= date_sub(curdate(),interval 60 day) and campaign.deleted_date is null group by 1,2 order by 3 desc limit $num_creative";
$sth = $dbhq->prepare($sql);
$sth->execute();
...
##    $sql = "select (sum(open_cnt)/sum(sent_cnt)*100),(sum(click_cnt)/sum(open_cnt)*100),((sum(click_cnt)/sum(sent_cnt))*100000) from creative_log where creative_id=$cid";
    $sql = "select (sum(el.open_cnt)/sum(el.sent_cnt)*100),(sum(el.click_cnt)/sum(el.open_cnt)*100),((sum(el.click_cnt)/sum(el.sent_cnt))*100000) from email_log el,campaign c where creative_id=? and el.campaign_id=c.campaign_id and c.scheduled_date >= date_sub(curdate(),interval 60 day) and c.deleted_date is null";
    $sth1 = $dbhq->prepare($sql) ;
    $sth1->execute($cid);
    ($copen,$cclick,$cindex) = $sth1->fetchrow_array();
    $sth1->finish();
#
	$sql="select approved_flag,original_flag,html_code,comm_wizard_c3,comm_wizard_cid,comm_wizard_progid,cr,landing_page from creative where creative_id=$cid";
	$sth1 = $dbhq->prepare($sql) ;
	$sth1->execute();
	($aflag,$oflag,$html_code,$cwc3,$cwcid,$cwprogid,$cr,$landing_page) = $sth1->fetchrow_array();
	$sth1->finish();
...
#           system(@args) == 0 or die "system @args failed: $?";
            ...
            unlink($tfilename);
            $sql="update vendor_supp_list_info set pexicom_last_upload=curdate()
 where list_id=$vid";
            my $rows=$dbhu->do($sql);
        }
    }
...
print<<"end_of_html";
					</b><a href="/cgi-bin/3rdparty_deploy_main.cgi?id=$new_camp_id">Modify Settings</a><b><br>
