...
my $infile="Flatiron_unsub_".$cdate1.".txt";
open(LOG,">XLUnsub_$cdate.txt");
$sql="select email_addr from unsub_log where client_id =1089  and unsub_date >= date_sub(curdate(),interval 1 day) and unsub_date < curdate()";
$sth=$dbhu->prepare($sql);
$sth->execute();
...
if ($ftp)
{
    $ftp->login('flatironunsub','T6aNbKg9') or print "Cannot login ", $ftp->message;
    $ftp->ascii();
	$ftp->cwd("Incoming");
	$ftp->get($infile);
	$ftp->cwd("../Outgoing");
...
sub uns_upd_list_member
{
	...
	$email_addr =~ s/'/''/g;
	$email_exists = 0 ;
	$sql = "select email_user_id,status,client_id from email_list where email_addr= '$email_addr' and client_id = 1089";
