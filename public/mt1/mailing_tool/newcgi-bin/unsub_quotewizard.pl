...
$sql="select email_addr from unsub_log where client_id in (1262,1381) and unsub_date >= date_sub(curdate(),interval 7 day) and unsub_date < curdate()";
$sth=$dbhu->prepare($sql);
$sth->execute();
...
if ($ftp)
{
    $ftp->login('quotewizardunsub','D8tz99ts') or print "Cannot login ", $ftp->message;
    $ftp->ascii();
