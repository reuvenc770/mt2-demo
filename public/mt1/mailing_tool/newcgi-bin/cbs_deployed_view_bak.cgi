...
sub display_available_deployed_campaign {
	...
	foreach (sort {$hr->{$a}{deploy_name} cmp $hr->{$b}{deploy_name}} keys %$hr) {
		print qq^
		<tr>
		  <td><a href="cbs_deployed_view.cgi?$href&id=$_&thirdID=$hr->{$_}{third_party_id}&cID=$hr->{$_}{client_id}&catID=$hr->{$_}{catid}&aID=$hr->{$_}{advertiser_id}&campID=$hr->{$_}{campaign_id}&mailer=$args->{mailer}&sch_date=$hr->{$_}{sch_date}#info">$hr->{$_}{deploy_name} ($hr->{$_}{date})</a></td>
		</tr>
		^;
	}
...
sub display_info {
    ...
    elsif ($args->{thirdID}==14) {
        $med_link='S';
    }
	elsif ($args->{thirdID}==1) {
		$med_link='D';
	}
    else {
        $med_link='N';
...
sub display_subject_from_supp {
	...
	my @sub=();
	my @from=();
##	my $sql=qq|select advertiser_subject,sum(open_cnt)/sum(sent_cnt) from advertiser_subject,email_log where advertiser_subject.subject_id=email_log.subject_id and campaign_id in (select campaign_id from campaign where advertiser_id='$args->{aID}' and deleted_date is null) and advertiser_subject.status='A' and advertiser_subject.advertiser_id='$args->{aID}' group by advertiser_subject order by 2 desc limit $infoHR->{num_subject}|;
	my $sql=qq|select advertiser_subject,sum(open_cnt)/sum(sent_cnt) from advertiser_subject,email_log where advertiser_subject.subject_id=email_log.subject_id and advertiser_subject.status='A' and advertiser_subject.advertiser_id='$args->{aID}' group by advertiser_subject order by 2 desc limit $infoHR->{num_subject}|;
	...
	$sth->finish;
##	my $sqlF=qq|select advertiser_from,sum(open_cnt)/sum(sent_cnt) from advertiser_from,email_log where advertiser_from.from_id=email_log.from_id and campaign_id in (select campaign_id from campaign where advertiser_id='$args->{aID}' AND deleted_date is null) and advertiser_from.advertiser_id='$args->{aID}' group by advertiser_from order by 2 desc limit $infoHR->{num_from}|;
	my $sqlF=qq|select advertiser_from,sum(open_cnt)/sum(sent_cnt) from advertiser_from,email_log where advertiser_from.from_id=email_log.from_id and advertiser_from.advertiser_id='$args->{aID}' group by advertiser_from order by 2 desc limit $infoHR->{num_from}|;
...
sub display_creative {
	...
	my $sql=qq|select email_log.creative_id,creative.creative_name,((sum(click_cnt)/sum(sent_cnt))*100000) from email_log, creative where email_log.creative_id=creative.creative_id and trigger_flag='N' and creative.status='A' and advertiser_id='$args->{aID}' group by 1,2 order by 3 desc limit $infoHR->{num_creative}|;
	my $sth=$DBHQ->prepare($sql);
	$sth->execute;
	my $camp_cnt = 1;
	while (my ($cid,$creative_name,$temp_val)=$sth->fetchrow_array()) {
		my $querNum=qq|select (sum(open_cnt)/sum(sent_cnt)*100),(sum(click_cnt)/sum(open_cnt)*100),|
				   .qq|((sum(click_cnt)/sum(sent_cnt))*100000) from email_log where creative_id=?|;
...
##		my $footer_str = $infoHR->{DBID} . "_" . $infoHR->{sid} . "_" . $infoHR->{url_id};
		...
		$html_code =~ s/{{URL}}/http:\/\/$infoHR->{redir_domain}\/cgi-bin\/redir1.cgi?eid={{EMAIL_USER_ID}}&cid={{CID}}&id=$infoHR->{DBID_linkid}&f=&s=&c=$cid/g;
...
sub print_nav_form {
  ...
  <tr height=20>
    ...
    <select name=mailer>
  ^;
  my $quer=qq|select third_party_id,mailer_name from third_party_defaults where third_party_id in ('1','11') order by mailer_name|;
  my $sth=$DBHQ->prepare($quer);
  $sth->execute;
