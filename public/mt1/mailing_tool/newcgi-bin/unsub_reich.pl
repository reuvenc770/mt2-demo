...
my $infile="Reichweite_".$cdate.".csv";
open(LOG,">XLUnsub_$cdate.txt");
$sql="select email_addr from unsub_log where client_id in (1040,1041) and unsub_date >= date_sub(curdate(),interval $dayint day) and unsub_date < curdate()";
$sth=$dbhu->prepare($sql);
$sth->execute();
...
if ($ftp)
{
    $ftp->login('ReichUnsub','CAnAerD3') or print "Cannot login ", $ftp->message;
    $ftp->ascii();
	$ftp->get($infile);
	$ftp->cwd("XLUnsubs");
...
sub uns_upd_list_member
{
	...
	$email_addr =~ s/'/''/g;
	$email_exists = 0 ;
	$sql = "select email_user_id,status,client_id as emailTable from email_list where email_addr= '$email_addr' and client_id in (1040,1041)";
