...
if (!$cookie) {
	if ($args->{login}) {
		...
		if ($err==1) {
			...
			print "Location: login_advertiser_list2.cgi\n\n";
		}
	}
	else {
		display_header();
	...
	while (my $hr=$sth->fetchrow_hashref) {
		if ($args->{last_run1} ne '') {
			...
			if ($args=>{last_run} eq ">") {
				...
				if ($args->{last_run2}>0) {
				my $q=qq|SELECT COUNT(*) FROM campaign WHERE advertiser_id=? GROUP BY advertiser_id HAVING MAX(sent_datetime)=|
					 .qq|DATE_SUB(curdate(), INTERVAL $args->{last_run2} DAY)|;
				my $s=$DBHQ->prepare($q);
				$s->execute;
				my $exist=$s->fetchrow;
				$s->finish;
...
sub display_login_form {
    ...
    <form method="post" action="login_advertiser_list2.cgi">
      <tr>  
        <td>    
