...
if ($email_addr ne "")
{
	...
	{
		$sql = "insert into test_emails(campaign_id,email_addr,hostname,ip_addr,submit_datetime) values($campaign_id,'$email_addr','$hostname','$c1',now())";
		$rows = $dbhu->do($sql);
		if ($dbhu->err() != 0)
		{
			$errmsg = $dbhu->errstr();
...
if ($profile_id > 0)
{
	...
	for (my $i=1; $i <= 3; $i++) 
	{
        ...
        my $sthDom=$dbhq->prepare($qSelDom);
		$sthDom->execute($i);
