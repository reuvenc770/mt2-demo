#!/usr/bin/perl
# *****************************************************************************************
# unique_attribution_add.cgi
#
# this page adds a new UniqueAttribution
#
# History
# *****************************************************************************************
...
$sql = "insert into UniqueAttribution(client_id,level) values ($client_id,$level)";
$rows = $dbhu->do($sql);
if ($dbhu->err() != 0)
{
	$errmsg = $dbhu->errstr();
...
$sql="update UniqueAttribution set level=level+1 where level >= $level and client_id != $client_id";
$rows = $dbhu->do($sql);
