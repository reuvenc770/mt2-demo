...
#
my $cfrom = $query->param('cheadline');
    $cfrom =~ s/\//g;
    $cfrom =~ s/\xc2//g;
    $cfrom =~ s/\xa0//g;
    $cfrom =~ s/\xb7//g;
    ...
    $cfrom =~ s/\x85//g;
#
# Insert record into advertiser_headline
#
$sql = "update article_headline set headline='$cfrom' where headline_id=$sid and article_id=$aid";
$sth = $dbhu->do($sql);
if ($dbhu->err() != 0)
{
	my $errmsg = $dbhu->errstr();
    util::logerror("Updating article headline info record for $user_id: $errmsg");
}
#
