...
# Jim Sobeck, 11/21/06, Change to send to only clients where brand defined
# *****************************************************************************************
...
$dl_id=6;
open(LOG,">/tmp/jimbo1.");
print LOG "Starting it \n";
my $two=$query->param('two');
$sql="select third_party_id,default_client from third_party_defaults where third_party_id=$two";
$sth = $dbhq->prepare($sql);
$sth->execute();
($id,$dl_id)=$sth->fetchrow_array();
$sth->finish();
$link_type=$id;
...
#$sth->finish();
##$sql="select brand_id,profile_id from schedule_info where slot_type='3' and third_party_id=$id and client_id=$dl_id"; 
$sql="select brand_id from client_brand_info where brand_type='3rd Party' and third_party_id=$id and client_id=$dl_id and exclude_thirdparty='N'"; 
$sth = $dbhq->prepare($sql);
$sth->execute();
($dl_bid)=$sth->fetchrow_array();
$sth->finish();
$sql="select substr(profile_name,1,8) from list_profile where master='Y' and client_id=$dl_id"; 
$sth = $dbhq->prepare($sql);
$sth->execute();
...
$sth->finish();
#
$sql = "select advertiser_name,category_id,unsub_link,unsub_image,vendor_supp_list_id,tracking_pixel from advertiser_info where advertiser_id=$aid"; 
$sth = $dbhq->prepare($sql);
print LOG "<$sql>\n";
$sth->execute();
($aname,$catid,$unsub_url,$unsub_img,$vid,$tracking_pixel)=$sth->fetchrow_array();
$sth->finish();
$sql = "select company from user where user_id=?"; 
$sth = $dbhq->prepare($sql);
$sth->execute($dl_id);
...
#
my $dir1 = $mailer_name . $network_name . $pname . $aname . $tid0 . $tid1 . $am_pm;
$dir1 =~ s/ //g;
$dir1 =~ s/\(//g;
$dir1 =~ s/\)//g;
$dir1 =~ s/\$//g;
...
if ($old_id > 0)
{
	$sql="select client_id,brand_id from client_brand_info where brand_type='3rd Party' and third_party_id=? and status='A' and exclude_thirdparty='N'";
	$sth1 = $dbhq->prepare($sql);
	$sth1->execute($id);
	print LOG "<$sql>\n";
	while (($client_id,$bid)=$sth1->fetchrow_array())
	{
...
else
{
	...
	print OUTFILE "Network,Brand,Profile,Advertiser,URL,Advertiser Unsub,Mailer Unsub,Privacy,Tracking Pixel\n";
	$sql="select client_brand_info.client_id,profile_id,brand_id from client_brand_info,list_profile where client_brand_info.third_party_id=? and client_brand_info.client_id=list_profile.client_id and list_profile.master='Y' and brand_type='3rd Party' and exclude_thirdparty='N' and client_brand_info.status='A' and list_profile.status='A'";
	$sth1 = $dbhq->prepare($sql);
	print LOG "<$sql>\n";
	$sth1->execute($id);
	CLIENT: while (($client_id,$profile_id,$bid)=$sth1->fetchrow_array())
	{
		### top 10 check? ###
		#
		# Add logic to check to see if shouldnt mail deal for client based on
		# Advertiser or category id
		#
		my $sth1a;
		my $reccnt;
	...
	print LOG "<$sql>\n";
		...
		if ($reccnt == 0)
		{		
    		$sql = "insert into campaign(campaign_name,user_id,status,created_datetime,advertiser_id,creative1_id,brand_id,scheduled_datetime,sent_datetime,profile_id,scheduled_date,scheduled_time,campaign_type) values('$cname',1,'C',now(),$aid,0,$bid,'$sdate','$sdate',$profile_id,date('$sdate'),time('$sdate'),'3RDPARTY')";
	print LOG "<$sql>\n";
			$rows=$dbhu->do($sql);
			$sql="select max(campaign_id) from campaign where campaign_name='$cname' and status='C' and scheduled_date=date('$sdate') and scheduled_time=time('$sdate')"; 
			$sth = $dbhq->prepare($sql);
	print LOG "<$sql>\n";
			$sth->execute();
	...
	print LOG "<$sql>\n";
			...
			#
			$sql = "select link_id from advertiser_tracking where advertiser_id=$aid and client_id=$client_id and daily_deal='$link_type'";
			$sth = $dbhq->prepare($sql);
			$sth->execute();
			...
			{
				# No link generate it for the client and advertiser
				$link_id = gen_link($client_id,$link_type,$aid);
			}
			my $tlink_id;
			...
			$adv_unsub_link="http://${redir_domain}/cgi-bin/redir1.cgi?eid=$remailid&cid=0&em=$remail&id=$tlink_id"; 
			$client_unsub_link="http://${redir_domain}/cgi-bin/redir1.cgi?eid=$remailid&cid=${temp_cid}&em=$remail&id=42&nid=$client_id";
			$privacy_unsub_link="http://${redir_domain}/cgi-bin/redir1.cgi?eid=$remailid&cid=0&em=$remail&id=51&nid=$client_id";
			print OUTFILE "$company_name,$bname,$profile_name,$aname,$adv_link,$adv_unsub_link,$client_unsub_link,$privacy_unsub_link,$tracking_pixel\n";
		}
		else
		{
			my $company_name;
...
if ($old_id > 0)
{
	...
	($temp_cid)=$sth->fetchrow_array();
	$sth->finish();
	$sql="update campaign set scheduled_datetime='$sdate',campaign_name='$cname',advertiser_id=$aid,sent_datetime='$sdate',scheduled_date=date('$sdate'),scheduled_time=time('$sdate') where campaign_id=$temp_cid"; 
	$rows=$dbhu->do($sql);
}
#
...
$supp_name =~ s/\!//g;
#
$sql = "select link_id from advertiser_tracking where advertiser_id=$aid and client_id=$dl_id and daily_deal='$link_type'";
$sth = $dbhq->prepare($sql);
$sth->execute();
...
end_of_html
$sql="select subject_id from advertiser_subject where advertiser_id=? and status='A'";
$sth = $dbhq->prepare($sql);
$sth->execute($aid);
$temp_str="";
while (($subject_id) = $sth->fetchrow_array())
{
	$temp_str=$temp_str.$subject_id.",";
}
$sth->finish();
chop($temp_str);

$sql="select email_log.subject_id,sum(email_log.open_cnt)/sum(email_log.sent_cnt) from email_log,campaign where email_log.subject_id in ($temp_str) and email_log.campaign_id=campaign.campaign_id and campaign.scheduled_date >= date_sub(curdate(),interval 60 day) and deleted_date is null group by subject_id order by 2 desc limit $num_subject";
$sth = $dbhq->prepare($sql);
print LOG "<$sql>\n";
$sth->execute();
open(OUTFILE,">$dir2/subject.txt");
while (($subject_id,$temp_val)=$sth->fetchrow_array())
{
	$sql="select advertiser_subject from advertiser_subject where subject_id=?";
	my $sth1q = $dbhq->prepare($sql);
	$sth1q->execute($subject_id);
	($subject_str)=$sth1q->fetchrow_array();
	$sth1q->finish();			
...
print<<"end_of_html";
					...
					<b><u>From Line(s):</u></b>
end_of_html
$sql="select from_id from advertiser_from where advertiser_id=? and status='A'";
$sth = $dbhq->prepare($sql);
$sth->execute($aid);
$temp_str="";
while (($subject_id) = $sth->fetchrow_array())
...
$sth->finish();
chop($temp_str);
$sql="select email_log.from_id,sum(email_log.open_cnt)/sum(email_log.sent_cnt) from email_log,campaign where email_log.from_id in ($temp_str) and email_log.campaign_id=campaign.campaign_id and campaign.scheduled_date >= date_sub(curdate(),interval 60 day) and deleted_date is null group by from_id order by 2 desc limit $num_from";
$sth = $dbhq->prepare($sql);
print LOG "<$sql>\n";
$sth->execute();
...
close (OUTFILE);
print LOG "Done with from line\n";
$sth->finish();
print<<"end_of_html";
					<br><br>
end_of_html
$sql="select creative_id from creative where advertiser_id=? and status='A' and trigger_flag='N'";
$sth = $dbhq->prepare($sql);
$sth->execute($aid);
$temp_str="";
while (($subject_id) = $sth->fetchrow_array())
...
$sth->finish();
chop($temp_str);
$sql="select email_log.creative_id,sum(email_log.click_cnt)/sum(email_log.sent_cnt) from email_log,campaign where email_log.creative_id in ($temp_str) and email_log.campaign_id=campaign.campaign_id and campaign.scheduled_date >= date_sub(curdate(),interval 60 day) and deleted_date is null group by creative_id order by 2 desc limit $num_from";
$sth = $dbhq->prepare($sql);
print LOG "<$sql>\n";
$sth->execute();
...
{
	$sql="select creative_name,approved_flag,original_flag,html_code,comm_wizard_c3,comm_wizard_cid,comm_wizard_progid,cr,landing_page from creative where creative_id=$cid";
	$sth1 = $dbhq->prepare($sql) ;
	$sth1->execute();
	($creative_name,$aflag,$oflag,$html_code,$cwc3,$cwcid,$cwprogid,$cr,$landing_page) = $sth1->fetchrow_array();
	$sth1->finish();
    $sql = "select (sum(el.open_cnt)/sum(el.sent_cnt)*100),(sum(el.click_cnt)/sum(el.open_cnt)*100),((sum(el.click_cnt)/sum(el.sent_cnt))*100000) from email_log el, campaign c where el.creative_id=? and el.campaign_id=c.campaign_id and c.scheduled_date >= date_sub(curdate(),interval 60 day) and deleted_date is null";
print LOG "<$sql>\n";
    $sth1 = $dbhq->prepare($sql) ;
    $sth1->execute($cid);
...
##	my $footer_str = $dl_bid . "_" . $sid . "_" . $url_id;
	my $footer_str = $dl_bid;
	$html_code =~ s/{{HEADER_TEXT}}/$header_text/g;
	$html_code =~ s/{{FOOTER_TEXT}}/$footer_text/g;
	if ($cwc3 ne "")
	{
    	$html_code =~ s/{{URL}}/http:\/\/$redir_domain\/cgi-bin\/redir1.cgi?eid={{EMAIL_USER_ID}}&cid={{CID}}&em={{EMAIL_ADDR}}&id=$link_id&f=&s=&c=$cid&cwc3=$cwc3&cwcid=$cwcid&cwprogid=$cwprogid&cr=$cr&l=$landing_page/g;
	}
	else
	{
    	$html_code =~ s/{{URL}}/http:\/\/$redir_domain\/cgi-bin\/redir1.cgi?eid={{EMAIL_USER_ID}}&cid={{CID}}&em={{EMAIL_ADDR}}&id=$link_id&f=&s=&c=$cid&cwcid=$cwcid&cwprogid=$cwprogid&cr=$cr&l=$landing_page/g;
	}
...
print LOG "Before stuff\n";
my @args = ("/var/www/html/newcgi-bin/stuff_3rdparty.sh $dir1");
system(@args) == 0 or die "system @args failed: $?";
print LOG "After stuff\n";
#
# Check to see if need to copy creative and advertiser suppression file
#
print LOG "Supp < $suppression_path>\n";
print LOG "Creative < $creative_path>\n";
if ($suppression_path ne "")
{
	my $tfilename=${supp_name} . ".zip";
	my $mesg=ftp_file($ftp_ip,$ftp_username,$ftp_password,$suppression_path,"/data5/supp/",$tfilename);
	if ($mesg ne "")
	{
		print "<script language=JavaScript>alert('$mesg');</script>\n";
	}
}
print LOG "Done with suppression file\n";
if ($creative_path ne "")
{
	my $tfilename=$dir1 . ".zip";
	print LOG "$tfilename\n";
	print LOG "<$ftp_ip> <$ftp_username> <$ftp_password>\n";
...
sub gen_link
{
	...
	$sql = "select mediactivate_id from client_thirdparty where user_id=1 and third_party_id=$ctype";
	$sth = $dbhq->prepare($sql);
	$sth->execute();
	...
	$sth->finish();
	$sql="select hitpath_id from third_party_defaults where third_party_id=$ctype";
	$sth = $dbhq->prepare($sql);
	$sth->execute();
	($thirdparty_hitpath_id) = $sth->fetchrow_array();
	$sth->finish();
#
	...
	{
		$url=~s/&c3=lm//g;
		$url=~s/$hitpath_id/$thirdparty_hitpath_id/g;
		$url=~s/{{CID}}/$hitpath_id/g;
		$url=~s/c2={{EMAIL_USER_ID}}/c2={{CID}}/g;
		$sql = "delete from advertiser_tracking where client_id=$client_id and advertiser_id=$aid and daily_deal='$ctype'";
		$rows = $dbhu->do($sql);
		$sql = "select hitpath_id from user where hitpath_id!= '' and user_id =$client_id and status='A'";
		$sth2 = $dbhq->prepare($sql);
		$sth2->execute();
		while (($mid) = $sth2->fetchrow_array())
		{
			my $temp_url = $url;
			...
			#
			$sql="insert into advertiser_tracking(advertiser_id,url,code,date_added,client_id,link_id,daily_deal) values($aid,'$temp_url','$mid',curdate(),$client_id,$link_id,'$ctype')";
			$rows = $dbhu->do($sql);
		}
		$sth2->finish();
   		$sql = "update advertiser_info set url_count=(select count(*) from advertiser_tracking where advertiser_tracking.advertiser_id=$aid) where advertiser_id=$aid";
   		$rows = $dbhu->do($sql);
	}
	$sth->finish();
	return $link_id;
