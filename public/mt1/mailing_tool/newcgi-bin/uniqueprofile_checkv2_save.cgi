...
# ******************************************************************************
# uniqueprofile_checkv2_save.cgi 
#
...
{
	$sql="insert into UniqueCheckCountry(check_id,countryID) values($pid,$c)";
	$rows=$dbhu->do($sql);
}
foreach my $c (@ua)
{
	$sql="insert into UniqueCheckUA(check_id,userAgentStringLabelID) values($pid,$c)";
	$rows=$dbhu->do($sql);
}
if ($cgroup_id == 0)
{
