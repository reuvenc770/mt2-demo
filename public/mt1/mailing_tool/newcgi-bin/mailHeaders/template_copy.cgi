...
$sql = "select templateName,status,templateCode from mailingHeaderTemplate where $userDataRestrictionWhereClause templateID = $template_id";
$sth=$dbhu->prepare($sql);
$sth->execute();
($template_name,$status,$templateCode)=$sth->fetchrow_array();
$sth->finish();
$template_name="Copy of ".$template_name;
$sql="insert into mailingHeaderTemplate(userID, templateName,dateAdded,status,templateCode) values($user_id, '$template_name',now(),'A','$templateCode')";
$rows = $dbhu->do($sql);
if ($dbhu->err() != 0)
{
	$errmsg = $dbhu->errstr();
