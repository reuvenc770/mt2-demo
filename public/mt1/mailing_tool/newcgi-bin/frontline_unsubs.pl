...
$sth=$dbhu->prepare($sql);
$sth->execute();
my $filename="/tmp/unsubs_frontline_".$cdate.".csv";
my $outfile="unsubs_frontline_".$cdate.".csv";
open(LOG,">$filename");
...
$sth->finish();
close(LOG);
my $host = "ftp.frontlinedm.com";
$ftp = Net::FTP->new("$host", Timeout => 20, Debug => 0, Passive => 0) or print "Cannot connect to $host: $@\n";
if ($ftp)
{
	$ftp->login('a1539_SVunsub','19qp2w') or print "Cannot login ", $ftp->message;
    $ftp->ascii();
