...
$DBHQ=DBI->connect('DBI:mysql:new_mail:updatep.routename.com','db_user','sp1r3V', \%attr) or die "Can't connect to DB: $!\n";
...
if (!$cookieID) {
	print "Location: cbs_login.cgi?redir=cbs_deployed_view.cgi\n\n";
}
	...
	#get user_id for user defined company
	if (($args->{company}) && ($args->{company} ne 'all')){
		$sqlUID=qq|SELECT user_id FROM user WHERE company = '$args->{company}'|;
	}
	
	#this is the default company
	else{
		$sqlUID=qq|SELECT user_id FROM user WHERE company='Domain Lanes'|;
	}
	...
	my $sqlBID=qq|SELECT brand_id FROM client_brand_info WHERE third_party_id=$infoHR->{thirdID} AND client_id=$infoHR->{uid}|;
	...
	if ($args->{aID}) {
		...
		my $qLINKID=qq|select link_id from advertiser_tracking where advertiser_id='$args->{aID}' and client_id='$infoHR->{uid}' |
				   .qq|and daily_deal='$infoHR->{thirdID}'|;
	...
	#user picked a company so display this data
	if ($args->{company}) {
...
sub display_available_deployed_campaign {
	...
	#if user picked a company used the old values
	if ($args->{company}){
		$brand_id = $args->{DBID};
		$client_id = 6;
	}
	...
	#this is a new query
		...
		$brand_id = $infoHR->{DBID};
		$client_id = $infoHR->{uid}
	...
	my $quer=qq|SELECT 3c.id, third_party_id, 3c.client_id, a.category_id as catid, 3c.advertiser_id, 3c.deploy_name, |
			...
			.qq|c.advertiser_id=a.advertiser_id AND 3c.advertiser_id=a.advertiser_id AND 3c.client_id='$client_id' |
			.qq|AND 3c.third_party_id='$infoHR->{thirdID}' |
			.qq|AND 3c.brand_id='$brand_id' AND $date_clause ORDER BY 3c.id ASC|;
	warn "$quer\n";
	...
	#if deploy_name is set
	if($args->{deploy_name}){
		
		#these are values passed in from long value of deploy name
		my @form_params = split(/&/,$args->{deploy_name});
		my @values;
	
		#now split on = to get hash key name and value
		foreach my $key(@form_params){
		
			@values = split(/=/,$key);
			$args->{$values[0]} = $values[1];
			
			$company .= "<input type='hidden' name='$values[0]' value='$values[1]'>\n";
			
		} #end foreach
				
		my $company_query = qq|SELECT company FROM brand_url_info bui, client_brand_info cbi, 3rdparty_campaign 3c, campaign c, list_profile lp, user u |
		 	...
		 	.qq|date(3c.scheduled_datetime)='$args->{sch_date}' order by company|;
		warn "$company_query\n";
		
		my $com_sth=$DBHQ->prepare($company_query);
		$com_sth->execute;
		$company_results=$com_sth->fetchall_arrayref();
		$com_sth->finish;
		
		$company .= "<p><SELECT NAME='company'>\n";
		$company .= "\t<OPTION VALUE='all'> - ALL - </OPTION>\n";
		
		for (my $i=0 ; $i <= $#$company_results ; $i++) {
			
			$selected = "";
			if($args->{'company'} eq $company_results->[$i][0]){
				$selected = "SELECTED"	
			}
	
			$company .= "\t<OPTION $selected VALUE='$company_results->[$i][0]'> $company_results->[$i][0] </OPTION>\n";
	...
	print qq^<form method=post action="cbs_deployed_view.cgi">
	<SELECT NAME='deploy_name'>^;
	...
	foreach (sort {$hr->{$a}{deploy_name} cmp $hr->{$b}{deploy_name}} keys %$hr) {
		...
		print qq^\t<OPTION $selected VALUE='id=$_&thirdID=$hr->{$_}{third_party_id}&cID=$hr->{$_}{client_id}&catID=$hr->{$_}{catid}&aID=$hr->{$_}{advertiser_id}&campID=$hr->{$_}{campaign_id}&mailer=$args->{mailer}&sch_date=$hr->{$_}{sch_date}'> $hr->{$_}{deploy_name} ($hr->{$_}{date}) </OPTION>\n^;
		
	}
	
	print qq^</select>^;
	
	print qq^<input type='hidden' name='r_type' value='$args->{r_type}'>\n^;
	print qq^<input type='hidden' name='DBID' value='$brand_id'>\n^;

	print $company;
	
	print qq^<p><input type=submit name="Go" value="Submit"></p>^;
...
sub display_info {
	...
	#get specific company if user chooses one
	my $company_clause;
	if ($args->{company} ne 'all'){
		$company_clause = "and company = '$args->{company}'";
	} #end if
...
#			 .qq|AND cbi.exclude_thirdparty='N' GROUP BY company,profile_name,brand_name ORDER BY company ASC|;
	...
	my $query=qq|SELECT 3c.client_id AS cid, 3c.brand_id AS bid, company, lp.profile_name AS profile_name, brand_name, url, |
			 ...
			 .qq|cbi.third_party_id='$infoHR->{thirdID}' AND 3c.third_party_id='$infoHR->{thirdID}' AND cbi.status='A' AND |
			 .qq|exclude_thirdparty='N' AND bui.url_type='O' AND 3c.advertiser_id='$args->{aID}' AND |
			 .qq|date(3c.scheduled_datetime)='$args->{sch_date}' $company_clause|;
	...
	while (my $info=$sth->fetchrow_hashref) {
		...
		#if linkID is empty (used to adv link only)
		if ($linkID eq ''){
			$status = 'INACTIVE';	
		}
...
#			$status = qq^http://$info->{url}/cgi-bin/redir1.cgi?eid=$infoHR->{emailid_replace}&cid=$campID&^
#					.qq^id=$linkID&f=&s=&c=^;
			$status = qq^http://$info->{url}/cgi-bin/redir1.cgi?eid=$infoHR->{emailid_replace}&cid=$args->{'campID'}&^
					 .qq^id=$linkID&f=&s=&c=^;		
		}
		
		my $adv_link=qq^$status^;
		
		my $adv_unsub_link=qq^http://$info->{url}/cgi-bin/redir1.cgi?eid=$infoHR->{emailid_replace}&cid=0&^
						  .qq^id=$Ainfo->{ulinkID}^;
		my $client_unsub_link=qq^http://$info->{url}/cgi-bin/redir1.cgi?eid=$infoHR->{emailid_replace}&^
							 .qq^cid=$campID&id=42&nid=$info->{cid}^;
		my $privacy_unsub_link=qq^http://$info->{url}/cgi-bin/redir1.cgi?eid=$infoHR->{emailid_replace}&^
		...
		if ($cnt==0) {
			print qq^<table border=0 width=100%>
			  <tr>
				<td>
				  <table border=1 bgcolor=#E3FAD1 width=100%>
...
sub getCreativeIDs {
	...
	#so table does not get overwritten by another call
	#get PID and append to temp table name
  	my $pid = $$;
  	
  	my $temp_tbl = "tmp_creative_id" . $$;
	
	my $create_table=qq^CREATE TEMPORARY TABLE $temp_tbl (advertiser_id int, creative_id int)^;
	my $sth=$DBHQ->prepare($create_table);
	$sth->execute or die "Couldn't execute statement: " . $sth->errstr;
	
	my $creative_id;
	
	#loop though 15 times (once for each creative)
	for my $i(1..15){
		
		$creative_id = "creative" . $i . "_id";
		
		my $get_creative_query = "SELECT advertiser_id, $creative_id
									FROM advertiser_setup a
									WHERE advertiser_id = $aID 
									AND class_id = 3
									AND $creative_id != 0";
		my $cid_sth=$DBHQ->prepare($get_creative_query);
		$cid_sth->execute;	
		my ($adv_id, $crt_id) = $cid_sth->fetchrow();
		
		if ($adv_id ne '' && $crt_id ne ''){			
			my $insert_query = "insert into $temp_tbl values ($adv_id, $crt_id)";
			my $insert_sth   = $DBHQ->prepare($insert_query);
			$insert_sth->execute or die "Couldn't execute statement: " . $sth->errstr;	
		}
		
	} #end for loop
	
	
#	my $get_data = "SELECT * FROM $temp_tbl";
#	my $data_sth=$DBHQ->prepare($get_data);
#	$data_sth->execute;	
#	my $results = $data_sth->fetchall_arrayref();
...
sub display_creative {
	...
	#builds a temp table for given advertiser 
	#because the table cannot be easily joined to get yahoo creatives
	#returns temp table name so we can join 
	my $temp_table = getCreativeIDs($args->{aID});

	my $sql=qq|select $temp_table.creative_id,creative.creative_name,((sum(el.click_cnt)/sum(el.sent_cnt))*100000) 
			from email_log el, creative, $temp_table,campaign c
			where el.creative_id = creative.creative_id 
			and creative.creative_id = $temp_table.creative_id
			and trigger_flag='N' 
			and creative.status='A' 
			and creative.advertiser_id='$args->{aID}' 
			and el.campaign_id=c.campaign_id and c.scheduled_date >= date_sub(curdate(),interval 60 day) and c.deleted_date is null and c.advertiser_id=$args->{aID}
			group by 1,2 
			order by 3 desc 
			limit $infoHR->{num_creative}|;
	...
	while (my ($cid,$creative_name,$temp_val)=$sth->fetchrow_array()) {
		...
		my $tracking_str = "<IMG SRC=\"http://$infoHR->{redir_domain}/cgi-bin/open_email1.cgi?id={{EMAIL_USER_ID}}&amp;cid={{CID}}&amp;f={{FID}}&amp;s={{SID}}&amp;c={{CRID}}&amp;\" border=0 height=1 width=1 alt=\"open\">";
...
{{UNSUBSCRIBE}}
        ...
        if ($Ainfo->{unsub_image} ne "") {
            ...
            if ( /\// )
            {
            	$temp_str = "<a href=\"http://{{DOMAIN}}/cgi-bin/redir1.cgi?eid=$infoHR->{emailid_replace}&amp;cid=0&amp;id=$Ainfo->{ulinkID}\" target=\"_blank\"><img src=\"http://www.{{IMG_DOMAIN}}/images/$Ainfo->{unsub_image}\" border=0 alt=\"Unsub\"></a><br><br>";
			}
			else
			{
            	$temp_str = "<a href=\"http://{{DOMAIN}}/cgi-bin/redir1.cgi?eid=$infoHR->{emailid_replace}&amp;cid=0&amp;id=$Ainfo->{ulinkID}\" target=\"_blank\"><img src=\"http://www.{{IMG_DOMAIN}}/images/unsub/$Ainfo->{unsub_image}\" border=0 alt=\"Unsub\"></a><br><br>";
			}
		}
		
#		if ($infoHR->{include_unsubscribe} eq "Y") {
#			#$html_code =~ s\{{UNSUBSCRIBE}}\$temp_str<a href="http://{{DOMAIN}}/cgi-bin/redir1.cgi?eid={{EMAIL_USER_ID}}&amp;cid={{CID}}&amp;&amp;id=42&amp;nid={{NID}}" target=_blank><img src="http://www.{{IMG_DOMAIN}}/fimg/{{FOOTER_STR}}_1.jpg" border=0 alt="end this subscription"></a><br><a href="http://{{FOOTER_SUBDOMAIN}}.{{DOMAIN}}/cgi-bin/redir1.cgi?cid=0&amp;eid={{EMAIL_USER_ID}}&amp;id=51&amp;nid={{NID}}" target=_blank><img src="http://www.{{IMG_DOMAIN}}/fimg/{{FOOTER_STR}}_2.jpg" border=0 alt="privacy policy"></a>\g;
...
##		my $footer_str = $infoHR->{DBID} . "_" . $infoHR->{sid} . "_" . $infoHR->{url_id};
		...
		if($cwc3 ne ""){
			$html_code =~ s/{{URL}}/http:\/\/$infoHR->{redir_domain}\/cgi-bin\/redir1.cgi?eid=$infoHR->{emailid_replace}&cid={{CID}}&id=$infoHR->{DBID_linkid}&f=&s=&c=$cid&cwc3=$cwc3&cwcid=$cwcid&cwprogid=$cwprogid&cr=$cr&l=$landing_page/g;
		}
		
		else{
			$html_code =~ s/{{URL}}/http:\/\/$infoHR->{redir_domain}\/cgi-bin\/redir1.cgi?eid=$infoHR->{emailid_replace}&cid={{CID}}&id=$infoHR->{DBID_linkid}&f=&s=&c=$cid&cwcid=$cwcid&cwprogid=$cwprogid&cr=$cr&l=$landing_page/g;
		}
...
#		$html_code =~ s/{{URL}}/http:\/\/$infoHR->{redir_domain}\/cgi-bin\/redir1.cgi?eid={{EMAIL_USER_ID}}&cid={{CID}}&em={{EMAIL_ADDR}}&id=$infoHR->{DBID_linkid}&f=&s=&c=$cid/g;
		$html_code =~ s/{{UNSUB_LINK}}/http:\/\/$infoHR->{redir_domain}\/cgi-bin\/redir1.cgi?eid=$infoHR->{emailid_replace}&amp;cid={{CID}}&amp;id=42/g;
            ...
            <tr>
                ...
                <table cellSpacing="0" cellPadding="0" width="100%" border="0" id="table4">
                    <tr>
						...
						<td align="right"><b>
							<a style="TEXT-DECORATION: none" href="cbs_login.cgi?action=logout&redir=cbs_deployed_view.cgi">
...
sub display_login_form {
	...
	<form method='post' action='cbs_deployed_view.cgi'>
	  <tr>
		<td>
...
sub print_nav_form {
  ...
  <tr height=20>
    ...
    <select name=mailer>
  ^;
  my $quer=qq|select third_party_id,mailer_name from third_party_defaults where third_party_id in ('1') order by mailer_name|;
  my $sth=$DBHQ->prepare($quer);
  $sth->execute;
