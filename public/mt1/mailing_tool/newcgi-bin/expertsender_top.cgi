...
if ($oldtemplateid eq "")
{
	...
	if (($espLabel eq "AlphaS") or ($espLabel eq "AlphaD"))
	{
		$oldtemplateid=2749;
	}
...
function addFrom(value,text)
{
    ...
    campform.cfrom.add(newOpt);
		...
		<FORM action="expertsender_creative.cgi" method=post name=campform>
		...
		<TR>
		<TD><b>Advertiser</b></td><td><select name="aid" id=aid onChange="upd_creative(0);">
end_of_html
...
while (($aid,$aname) = $sth->fetchrow_array())
{
	if ($aid == $oldaid)
	{
		print "<option selected value=$aid>$aname - $aid</option>\n";
	}
	else
	{
...
print<<"end_of_html";
		...
		<tr><td><b>Footer:</b></td><td><select name=footer_id><option selected value=0>None</option>
end_of_html
...
if (($esp eq "ALP001") or ($esp eq "GotClick") or ($esp eq "ALP002"))
{
	...
	$sql = "select subAffiliateID,name from AlphaSSubAffiliate where espID=$tespID order by name"; 
	$sth = $dbhq->prepare($sql);
	$sth->execute();
	my $sid;
	my $sname;
...
print qq^<tr><td><b>URL Format:</b></td><td><select name=newurl><option value=Y>New</option><option value=N selected>Old</option><option value=G>Gmail</option></select></td></tr>^;
print qq^<tr><td><b>One Image:</b></td><td><input type=radio value=Y name=one_image>Yes&nbsp;&nbsp;<input type=radio value=N checked name=one_image>No</td></tr>^;
if (($esp ne "PACK1") and ($esp ne "ALP001") and ($esp ne "GotClick") and ($esp ne "ALP002"))
{
	print qq^<tr><td><b>Cake AffiliateID </b></td><td><select name=AffiliateID><option value=309 checked>309</option>
