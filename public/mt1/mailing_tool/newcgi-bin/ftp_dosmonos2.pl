...
$sql="select mailer_ftp,ftp_username,ftp_password,record_path from third_party_defaults where mailer_name='Dos Monos 2'";
...
{    
	...
	if ($ftp)
	{
		...
		while ($i <= $#dots)
		{
			...
			if (($tstr ne "tz_") && ($tstr ne "rz_"))
			{
				$tname =~ s/_$cdate.txt//;
				my $rest_str;
				($tname,$rest_str)=split('_',$tname);
			$sql="select replace(company,' ','') from user where user_id in (18,16,22,8,11,5,2,9,10,29,6,33,27,3,39,19,17,21,26,4,24,25,15) and replace(company,' ','')='$tname'";
			$sth = $dbhq->prepare($sql);
			$sth->execute();
