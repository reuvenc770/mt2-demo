...
<TD vAlign=top align=left bgColor=#999999>
	...
	<TD vAlign=top align=left bgColor=#ffffff colSpan=10>
		...
		<TBODY>
		<tr>
		<td><form method=POST action=link_edit.cgi><input type=hidden name="mode" value="EDIT">Enter old ID to lookup: <input type=text name=link_id size=8 maxlength=8>&nbsp;&nbsp;&nbsp;<input type=submit value="Lookup"></form>
		</td>
<td width=20% align="center" valign="top">
		...
		<TD bgcolor="#EBFAD1" align="left" width="40%">
			<FONT face="verdana,arial,helvetica,sans serif" color="$table_text_color" size=2> 
			<b>URL</b></font></td>
		<TD bgcolor="#EBFAD1" align="left" width="18%">
...
#$sql = "select link_id,refurl from links where date_added >= date_sub(curdate(),interval 3 day) order by link_id desc";
$sql = "select link_id,refurl from links order by link_id desc limit 75";
$sth = $dbhq->prepare($sql);
$sth->execute();
	...
	print qq { <TR bgColor=$bgcolor> 
		...
		<TD align=left><font color="#509C10" 
			face="verdana,arial,helvetica,sans serif" size="2"> 
 			$refurl</font></TD> 
		<TD align=left><font color="#509C10" 
			face="verdana,arial,helvetica,sans serif" size="2">http://{{DOMAIN}}/cgi-bin/redir1.cgi?cid={{CID}}&eid={{EMAIL_USER_ID}}&id=$link_id</font></TD> 
 		<TD>$aname</TD> 
 		</TR> \n };
}
