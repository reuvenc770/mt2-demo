...
# ******************************************************************************
# hitpath_creative_deploy_it.cgi
#
# this page displays the Export Creative page 
#
# History
# Jim Sobeck, 05/25/07, Creation
# Jim Sobeck, 01/28/08,	Modified to display all approved creative
# Jim Sobeck, 02/14/08,	Modified to display all creative
# ******************************************************************************
...
<body>
<form method=post action="/cgi-bin/save_hitpath_creative.cgi">
...
end_of_html
$sql="select advertiser_subject from advertiser_subject where advertiser_subject.status='A' and advertiser_subject.advertiser_id=$aid order by advertiser_subject desc"; 
$sth = $dbhq->prepare($sql);
$sth->execute();
...
print<<"end_of_html";
					...
					<b><u>From Line(s):</u></b>
end_of_html
$sql="select advertiser_from from advertiser_from where advertiser_from.advertiser_id=$aid and advertiser_from != '{{FOOTER_SUBDOMAIN}}' and status='A' order by advertiser_from";
$sth = $dbhq->prepare($sql);
$sth->execute();
while (($subject_str)=$sth->fetchrow_array())
{
...
$got_it=0;
my $reccnt;
$sql="select count(*) from creative where trigger_flag='N' and creative.status='A' and creative.advertiser_id=$aid"; 
$sth =$dbhq->prepare($sql);
$sth->execute();
...
if ($reccnt > 0)
{
	$sql="select creative_id from creative where trigger_flag='N' and creative.status='A' and advertiser_id=$aid order by creative_id";
}
else
{
	$sql="select creative_id from creative where trigger_flag='N' and creative.status='A' and advertiser_id=$aid order by creative_id";
...
{
	$sql="select creative.creative_name,thumbnail,approved_flag,original_flag,html_code,hitpath_flag from creative where creative_id=?";
	...
	print "<b><u>Creative $camp_cnt: $creative_name ($copen% Open - $cclick% Click - $cindex Index - $temp_str</u></b>&nbsp;&nbsp;&nbsp;<a href=\"/cgi-bin/export_images.cgi?cid=$cid\" target=_blank>Image Export</a>&nbsp;&nbsp;<a href=\"/cgi-bin/export_images.cgi?cid=$cid&html=1\" target=_blank>HTML Export</a><br>\n";
	$got_it=1;
print<<"end_of_html";
	<font face="Tahoma">
<input type="checkbox" name="mflag" $mflag_str size="40" maxlength="90" value="$cid" style="font-weight: 700"></font><b><u>In HitPath</u></b><br>
...
sub cb 
{
	...
	my $ext;
     ...
     if (($tag eq "img") or ($tag eq "background") or ($url1 eq "background") or (($tag eq "input") and ($url1 eq "src")))
     {
		...
		open(LOG,">>/tmp/j.k");
		print LOG "<$name> <$scheme> <$auth> <$frag>\n";
		close(LOG);
		if ($scheme ne "")
		{
			my $repl_url=$scheme."://".$auth.$frag;
			$global_text =~ s/$repl_url/xxipathxx\//g;
            ...
            if ($ext eq "")
            {
				...
				{
					$global_text =~ s/$temp_str/$name/g;
					open(LOG,">>/tmp/j.k");
					print LOG "replacing <$temp_str> with <$name>\n";
					close(LOG);
					$IMG->{$name}=1;
				}
            }	
