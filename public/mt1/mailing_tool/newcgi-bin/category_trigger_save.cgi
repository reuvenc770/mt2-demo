...
my $aid3=$query->param('advertiser_id3');
my $calc_trigger1=$query->param('calc_trigger1');
if ($calc_trigger1 eq "")
{
	$calc_trigger1="N";
}
my $calc_trigger2=$query->param('calc_trigger2');
if ($calc_trigger2 eq "")
{
	$calc_trigger2="N";
}
my $calc_alt=$query->param('calc_alt');
if ($calc_alt eq "")
{
	$calc_alt="N";
}
...
$sql="delete from category_trigger where client_id=$client_id and category_id=$cid and trigger_type='$trigger_type'"; 
$rows = $dbhu->do($sql);
if ($client_id == 0)
{
	$sql="delete from category_trigger where client_id > 0 and category_id=$cid and trigger_type='$trigger_type'"; 
	$rows = $dbhu->do($sql);
}
#
	$sql = "insert into category_trigger(client_id,category_id,trigger1,trigger2,alt_trigger,trigger1_aid,trigger2_aid,alt_trigger_aid,calc_trigger1,calc_trigger2,calc_alt,trigger_type) values($client_id,$cid,$trigger_creative,$trigger_creative2,$alt_trigger,$aid1,$aid2,$aid3,'$calc_trigger1','$calc_trigger2','$calc_alt','$trigger_type')"; 
	$rows = $dbhu->do($sql);
	if ($dbhu->err() != 0)
	{
		$errmsg = $dbhu->errstr();
...
#
	my $temp_str = $aname;
	if ($trigger_type eq "OPEN")
	{
		$temp_str="OPEN-".$temp_str;
	}
...
if ($aid3 > 0)
{
	$sql = "select advertiser_name from advertiser_info where advertiser_id=$aid3"; 
    $sth = $dbhq->prepare($sql);
    $sth->execute();
...
#
	...
	{
		$sth->finish();
		$sql = "insert into campaign(campaign_name,user_id,status,created_datetime,advertiser_id,creative1_id,campaign_type) values('$temp_str',1,'T',now(),$aid3,$alt_trigger,'TRIGGER')";
		$rows=$dbhu->do($sql);
	}
}
...
document.location="/cgi-bin/category_trigger_list.cgi?userid=$client_id&trigger_type=$trigger_type";
</script>
</body>
</html>
end_of_html
