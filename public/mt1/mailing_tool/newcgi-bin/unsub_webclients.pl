...
#		$email_addr =~ s/\s.*$// ;   # remove from 1st white space at end of addr thru end of line
		...
		$total_cnt++;
		&uns_upd_list_member($email_addr, $clientID);
	} 
...
sub uns_upd_list_member
{
	...
	$email_addr =~ s/'/''/g;
	$email_exists = 0 ;
	$sql = "select email_user_id,el.status,'email_list' from email_list el,list l where email_addr= '$email_addr' and el.list_id=l.list_id and l.user_id=$clientID union select email_user_id,el.status,'email_list_new' from email_list_new el,list l where email_addr= '$email_addr' and el.list_id=l.list_id and l.user_id=$clientID";
	$sth2 = $dbh->prepare($sql) ;
	$sth2->execute();
	while(($emailid,$cstatus,$ctable) = $sth2->fetchrow_array())
	{
		if ($cstatus eq "A") 
		{
			$sql = "update $ctable set status = 'U', unsubscribe_date= curdate(),unsubscribe_time=curtime() where email_user_id= $emailid and status='A'"; 
			$rows = $dbh->do($sql) ;
			$sql = "insert into unsub_log(email_addr,unsub_date,client_id) values('$email_addr',curdate(),$clientID)";
			$rows = $dbh->do($sql);
			$total_unscnt++;
