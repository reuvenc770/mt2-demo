...
sub send_powermail
{
	...
	{
		$sql = "select min(client_brand_info.brand_id) from client_brand_info,brand_url_info where client_id=$client_id and client_brand_info.brand_id=brand_url_info.brand_id and url_type='C'"; 
		...
		#
		$sarr_cnt = 0;
		$sql = "select server_name,rand() from brand_host,client_brand_info where brand_host.brand_id=client_brand_info.brand_id and client_id=? and server_type='C' and brand_host.brand_id=$bid and server_name != 'inv1' order by 2";
		$sth = $dbhq->prepare($sql);
		print "Sql <$sql>\n";
...
[9]) = $sth->fetchrow_array())
	...
	my $i;
		...
		$ysubject_ind = 0;
		$yfrom_ind = 0;
##		$sql = "select url,url_id from brand_url_info,client_brand_info where brand_url_info.brand_id=client_brand_info.brand_id and url_type='C' and client_id=$client_id"; 
		$sql = "select url,url_id from brand_url_info,client_brand_info where brand_url_info.brand_id=client_brand_info.brand_id and url_type='C' and client_id=?"; 
		$sth3 = $dbhq->prepare($sql);
		$sth3->execute($client_id);
		($redir_domain,$turl_id) = $sth3->fetchrow_array();
		$sth3->finish();
        $redir_domain=~ tr/[A-Z]/[a-z]/;
##		$sql = "select url from brand_url_info,client_brand_info where brand_url_info.brand_id=client_brand_info.brand_id and url_type='CI' and client_id=$client_id"; 
		$sql = "select url from brand_url_info,client_brand_info where brand_url_info.brand_id=client_brand_info.brand_id and url_type='CI' and client_id=?"; 
		$sth3 = $dbhq->prepare($sql);
		$sth3->execute($client_id);
...
##		$sql = "select advertiser_subject from advertiser_subject where subject_id=$subject_arr[$subject_ind]";
		...
		$sth3->finish();
		open (OUTFILE, "> /var/www/util/new_tmpmailfiles1/list_fa_$sarry[$cnt2]_${camp_id}_$filecnt.txt");
...
sub process_list()
{
	...
	{
		$sth3->finish();
    	$sql = "select rec_id,email_addr,email_user_id,state,first_name,last_name,city,zip from daily_records1 where client_id=$client_id and send_day=$cday and rec_id between $fid and $lid";
	}
	else
	{
		$sth3->finish();
		# Now get a list of all the members and start processing
    	$sql = "select rec_id,email_addr,email_user_id,state,first_name,last_name,city,zip from daily_records1 where client_id=$client_id and send_day=$cday order by rec_id limit $max_client_emails";
	}
		$email_type="H";
		$sth = $dbhq->prepare($sql);
		print "$camp_id - <$sql>\n";
...
my $cdomain;
				...
				if ($addrec == 1)
				{
					...
					if ($cnt > $records_per_file)
					{
        				...
        				if ($list_id > 0)
        				{
							open (OUTFILE, "> /var/www/util/new_tmpmailfiles1/list_fa_$sarry1[$cnt2a]_${camp_id}_$filecnt.txt");
						}
						else
						{
							open (OUTFILE, "> /var/www/util/new_tmpmailfiles1/list_fa_$sarry[$cnt2]_${camp_id}_$filecnt.txt");
				...
				{
					$sql="delete from daily_records1 where email_addr='$cemail' and client_id=$client_id";
					$rows = $dbhu->do($sql);
				}
	}
	$sth->finish();
