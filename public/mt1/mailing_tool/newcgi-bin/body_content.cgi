...
if ($user_id == 0) {
	...
	if ($args->{new} || $args->{edit}) {
        ...
        my $q=qq|SELECT id AS bodyID, body_name AS name, date_add, inactive_date, body_content,modified_date |
			 .qq|FROM body_content WHERE id='$args->{bodyID}'|;
...
sub add_edit_body {
	...
	if ($args->{name} && $args->{body_content}) {
		my $name=$dbh->quote($args->{name});
		my $content=$dbh->quote($args->{body_content});
		my $quer;
		if ($args->{edit}) {
			$quer=qq|UPDATE body_content SET body_name=$name, body_content=$content, date_add='$args->{date_add}'|
				 .qq|, inactive_date='$args->{inactive_date}', modified_date=NOW() WHERE id='$args->{bodyID}'|;
		}
		else {
			$quer=qq|INSERT INTO body_content (body_name,date_add,modified_date,body_content) VALUES ($name,'$args->{date_add}',NOW(),$content)|;
		}
		my $rv=$dbh->do($quer);
		if ($rv) {
			my $msg=$args->{edit} ? "Body Content edited" : "New Body Content Added";
			display_confirmation('/newcgi-bin/body_content_list.cgi','2',$msg);
		}
		else {
		}
	}
	else {
		my $msg="<font color=red>Missing Body Content Name or Content.</font>";
		display_body();
...
sub delete_body {
	...
	if ($args->{bodyID}) {
		my $quer=qq|DELETE FROM body_content WHERE id='$args->{bodyID}'|;
		my $rv=$dbh->do($quer);
		if ($rv) {
			display_confirmation('/newcgi-bin/body_content_list.cgi','2',"Body Content has been deleted!");
		}
		else {
			display_confirmation('/newcgi-bin/body_content_list.cgi','2',"Problem delete the selected Body Content, try again later!");
		}
	}
}
...
sub display_form {
	...
	my $modified_date=$args->{bodyID} ? qq^<b>Modified Date: </b>$args->{modified_date}<br><br>^ : "";
	print qq^
...
rif" color="#509c10" size="3"><b>Body Content</b> </font></td>
					</tr>
					<tr>
				...
				<form method="post" action="body_content.cgi">
					$hidden
					<input type=hidden name='bodyID' value='$args->{bodyID}'>
					<b>Content Name:</b><br>
					...
					<br>
					<b><u>HTML Code:</u><br>
					<textarea name="body_content" rows="15" cols="100">$args->{body_content}</textarea> 
...
sub preview_body {
	...
	my $quer=qq|SELECT body_content FROM body_content WHERE id='$args->{bodyID}'|;
	...
	if ($content) {
		print "Content-type: text/html\n\n";
		print qq^<html><head><title>Body Content Preview</title></head><body>^;
		print "$content\n";
		print qq^</body></html>^;
	}
}
