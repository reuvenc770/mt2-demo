...
my $ftp_file = $query->param('ftp_file');
...
{
	$add_sub_dir = "${BASE_DIR}supplist/"; 
}
else
{
	$add_sub_dir = "${BASE_DIR}new_supplist/"; 
}

# get this users email address
$email_addr = "jsobeck\@lead-dog.net"; 

if ($ftp_file ne "")
{
	my $out_file = ${add_sub_dir} . ${list_id} . "_" . ${ftp_file}; 
	my $in_file = "/home/supp/" . $ftp_file;
	copy("$in_file","$out_file");
    if ($!)
    {
        print "Error moving file $out_file to  $in_file: $!\n";
    }
	unlink($in_file);
}
else
...
if ($filedate ne "")
{
	$sql = "update vendor_supp_list_info set temp_filedate='$filedate',unique_id=$unique_id where list_id=$list_id";
}
else
{
	$sql = "update vendor_supp_list_info set temp_filedate=null,filedate=null,unique_id=$unique_id where list_id=$list_id";
}
my $rows=$dbhu->do($sql);
...
<TD vAlign=top align=left bgColor=#999999>
	...
	<TD vAlign=top align=left bgColor=#FFFFFF colSpan=4>
		...
		<td align="center" width="100%">
			...
			<td width="50%" align="center">
				<a href="supplist_addnames.cgi?tid=$list_id">
