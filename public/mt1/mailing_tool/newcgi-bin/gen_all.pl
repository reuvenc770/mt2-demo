...
$sth->finish();
#
$sql="select distinct advertiser_id from advertiser_tracking where daily_deal = 'N' and advertiser_id not in (select distinct advertiser_id from advertiser_tracking where daily_deal='$ctype') order by advertiser_id desc";
