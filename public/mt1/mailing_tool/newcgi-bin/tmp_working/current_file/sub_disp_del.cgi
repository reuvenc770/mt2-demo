...
end_of_html
	...
	if ($user_id == 1)
	{
		$sth = $dbh->prepare("select list_id, list_name, status from list where status = 'A' order by list_name") ;
	}
	else
	{
		$sth = $dbh->prepare("select list_id, list_name, status from list where user_id = $user_id and status = 'A' order by list_name") ;
	}
	$sth->execute();
	$reccnt = 0 ;
