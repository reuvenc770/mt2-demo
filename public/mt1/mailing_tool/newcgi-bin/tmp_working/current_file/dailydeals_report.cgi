...
util::header("Advertiser Daily Deals Report - Current Month");
...
if ($networkopt > 0)
{
$sql = "select advertiser_info.advertiser_id,category_name,company,sum(unsubscribe_cnt),sum(bounce_cnt),sum(fullmbx_cnt),format(sum(unsubscribe_cnt),0),format(sum(bounce_cnt),0),format(sum(fullmbx_cnt),0),sum(notdelivered_cnt),format(sum(notdelivered_cnt),0) from campaign,advertiser_info,category_info,user where sent_datetime >= '$cdate' and campaign.advertiser_id=advertiser_info.advertiser_id and advertiser_info.category_id=category_info.category_id and campaign.campaign_id in (select campaign_id from daily_deals where client_id=$networkopt) and user.user_id=$networkopt group by advertiser_info.advertiser_id,category_name,company order by advertiser_name"; 
$sth = $dbh->prepare($sql);
$sth->execute();
while (($aid,$category_name,$company,$uns_cnt,$bounce_cnt,$fullmbx_cnt,$uns_cnt_str,$bounce_cnt_str,$fullmbx_cnt_str,$notdelivered_cnt,$notdelivered_cnt_str) = $sth->fetchrow_array())
{
...
else
{
	...
	while ($i <= $max_client)
	{
$sql = "select advertiser_info.advertiser_id,category_name,company,sum(unsubscribe_cnt),sum(bounce_cnt),sum(fullmbx_cnt),format(sum(unsubscribe_cnt),0),format(sum(bounce_cnt),0),format(sum(fullmbx_cnt),0),sum(notdelivered_cnt),format(sum(notdelivered_cnt),0) from campaign,advertiser_info,category_info,user where sent_datetime >= '$cdate' and campaign.advertiser_id=advertiser_info.advertiser_id and advertiser_info.category_id=category_info.category_id and campaign.campaign_id in (select campaign_id from daily_deals where client_id=$i) and user.user_id=$i group by advertiser_info.advertiser_id,category_name,company order by advertiser_name"; 
$sth = $dbh->prepare($sql);
$sth->execute();
while (($aid,$category_name,$company,$uns_cnt,$bounce_cnt,$fullmbx_cnt,$uns_cnt_str,$bounce_cnt_str,$fullmbx_cnt_str,$notdelivered_cnt,$notdelivered_cnt_str) = $sth->fetchrow_array())
{
