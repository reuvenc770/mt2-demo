...
$sth->finish();
	...
	while ($i <= $camp_cnt)
	{
		...
		while ($j < 7)
		{
			...
			$sql = "select campaign_name,advertiser_id,campaign.campaign_id from campaign,camp_schedule_info where client_id=$clientid and camp_schedule_info.slot_id=$i and slot_type='$stype' and camp_schedule_info.schedule_date=date_add('$cday1',interval $tday1+$j day) and campaign.campaign_id=camp_schedule_info.campaign_id";
			$sth = $dbh->prepare($sql);
			$sth->execute();
