...
{
	$old_id = 0;
	$sql="insert into 3rdparty_campaign(third_party_id,client_id,brand_id,catid,advertiser_id,deploy_name,scheduled_datetime,campaign_id) values($id,$client_id,$bid,$catid,$aid,'$cname','$sdate',0)";
}
my $rows=$dbhu->do($sql);
if ($old_id == 0)
{
...
if ($old_id > 0)
{
	...
	$sth->finish();
	$sql="update campaign set brand_id=$bid,scheduled_datetime='$sdate',campaign_name='$cname',advertiser_id=$aid,sent_datetime='$sdate' where campaign_id=$temp_cid"; 
	$rows=$dbhu->do($sql);
}
else
{
	$sql="select profile_id from list_profile where client_id=$client_id and (profile_name='All' or (yahoo_flag='Y' and other_flag='Y')) and status='A'"; 
	$sth = $dbhq->prepare($sql);
	$sth->execute();
...
#
	...
	($temp_cid)=$sth->fetchrow_array();
	$sth->finish();
	$sql = "update 3rdparty_campaign set campaign_id=$temp_cid where id=$new_camp_id";
	$rows=$dbhu->do($sql);
	#
...
$sth->finish();
#
$sql = "select list_name,pexicom_last_upload from vendor_supp_list_info where list_id=$vid";
$sth = $dbhq->prepare($sql);
$sth->execute();
($supp_name,$pexicom_last_upload)=$sth->fetchrow_array();
$sth->finish();
$supp_name =~ s/ //g;
$supp_name =~ s/\)//g;
$supp_name =~ s/\(//g;
...
#           system(@args) == 0 or die "system @args failed: $?";
            ...
            my $mesg=ftp_file($ftp_ip,$ftp_username,$ftp_password,$suppression_path,"/data3/supp/temp1/",$tfilename,"A",$new_name);
            $tfilename="/data3/supp/temp1/" . $tfilename;
            unlink($tfilename);
