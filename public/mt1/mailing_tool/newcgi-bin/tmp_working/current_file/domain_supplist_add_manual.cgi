#!/usr/bin/perl
#===============================================================================
# File   : domain_supplist_add_manual.cgi
#
...
$log_file = $html_dir_unix . "domain_supplist_add_${user_id}.error.log";
...
sub process_list 
{
	...
	foreach $list_email_addr (@email_array)
	{
		...
		$sql = "select email_addr from vendor_domain_supp_list where list_id=$list_id and domain='$email_addr'";
		$sth = $dbh->prepare($sql) ;
		$sth->execute();
...
sub add_upd_list_member
{
	...
	$sql = "insert into vendor_domain_supp_list(list_id,domain) values($list_id,'$email_addr')"; 
	$rows = $dbh->do($sql);
   	if ($dbh->err() != 0)
   	{
   		$errmsg = $dbh->errstr();
   		util::logerror("Updating vendor_domain_supp_list record: $sql : $errmsg");
   		exit(0);
	}
} 
...
sub print_summary
{
	...
	util::header("Add Domains To Suppression List Status");
...
<TD vAlign=top align=left bgColor=#999999>
	...
	<TD vAlign=top align=left bgColor=#FFFFFF colSpan=4>
		...
		<TD colspan="4" align=middle width="100%" height=15>
			...
			<b>Summary of Domains Added</b></font></TD>
		</TR>
		<TR bgColor=#509C10 height=15>
		<td colspan=4 >&nbsp;</td>
		</tr>
...
end_of_html
	...
	print << "end_of_html" ;
		...
		<td colspan=4 align="center" width="100%">
			...
			<td width="50%" align="center">
				<a href="/cgi-bin/domain_supplist_list.cgi">
