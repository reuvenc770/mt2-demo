...
# get some directories from sysparm
	...
	$sql = "delete from advertiser_approval where advertiser_id=$aid";
	$rows = $dbh->do($sql);
...
sub add_upd_email_user
{
	...
	$sql = "insert into advertiser_approval(advertiser_id,email_addr) values($aid,'$email_addr')";
	$rows = $dbh->do($sql);
   	if ($dbh->err() != 0)
   	{
       	$errmsg = $dbh->errstr();
       	util::logerror("Updating advertiser_approval record: $sql: $errmsg");
       	exit(0);
   	}
