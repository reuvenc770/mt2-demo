...
# get some directories from sysparm
	...
	$sql = "delete from campaign_seedlist where campaign_id=$campaign_id";
	$rows = $dbh->do($sql);
...
sub add_upd_email_user
{
	...
	$sql = "insert into campaign_seedlist(campaign_id,email_addr) values($campaign_id,'$email_addr')";
	$rows = $dbh->do($sql);
   	if ($dbh->err() != 0)
   	{
       	$errmsg = $dbh->errstr();
       	util::logerror("Updating campaign_seedlist record: $sql: $errmsg");
       	exit(0);
   	}
