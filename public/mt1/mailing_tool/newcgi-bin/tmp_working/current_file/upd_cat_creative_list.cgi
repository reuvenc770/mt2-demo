...
if ($client_id > 0)
{
	$sql = "select trigger1,trigger2,alt_trigger from category_trigger where category_id=$cid and client_id in (0,$client_id) order by client_id desc"; 
}
else
{
	$sql = "select trigger1,trigger2,alt_trigger from category_trigger where category_id=$cid and client_id = $client_id order by client_id desc"; 
}
	$sth = $dbh->prepare($sql) ;
	$sth->execute();
	($trigger1,$trigger2,$alt_trigger) = $sth->fetchrow_array();
	$sth->finish();
#
...
$sth->finish();
	...
	if ($alt_trigger > 0)
	{
		$sql = "select creative.advertiser_id,category_id from creative,advertiser_info where creative.advertiser_id=advertiser_info.advertiser_id and creative_id=$alt_trigger";
		$sth = $dbh->prepare($sql) ;
		$sth->execute();
		($advertiser_id3,$catid3) = $sth->fetchrow_array();
		$sth->finish();
		$sql="select creative_id,creative_name from creative where advertiser_id=(select advertiser_id from creative where creative_id=$alt_trigger) and status='A' and trigger_flag='Y' order by creative_name";
		$sth = $dbh->prepare($sql) ;
		$sth->execute();
		while (($cid,$aname) = $sth->fetchrow_array())
		{
    		print "parent.main.addTriggerOption3($cid,\"$aname\");\n";
		}
		$sth->finish();
	}
	print "parent.main.set_fields('$trigger1','$trigger2','$alt_trigger','$catid1','$catid2','$catid3','$advertiser_id1','$advertiser_id2','$advertiser_id3');\n";
print "</script>\n";
print "</body>\n";
print "</html>\n";
$util->clean_up();
