...
<TD vAlign=top align=left>
    ...
    <TD vAlign=top align=left bgColor=#ffffff colSpan=10>
        ...
        <TD vAlign=center align=left><font face="verdana,arial,helvetica,sans serif" 
			color="#509C10" size="3"><b>Trigger Report - $company</b></font></TD>
		</TR>
        <TR>
...
$sth->finish();
$sql = "select advertiser_info.advertiser_id,category_name,sum(unsubscribe_cnt),sum(bounce_cnt),sum(fullmbx_cnt),format(sum(unsubscribe_cnt),0),format(sum(bounce_cnt),0),format(sum(fullmbx_cnt),0),sum(notdelivered_cnt),format(sum(notdelivered_cnt),0) from campaign,advertiser_info,category_info where sent_datetime >= '$cdate' and campaign.advertiser_id=advertiser_info.advertiser_id and advertiser_info.category_id=category_info.category_id and campaign.status='T' group by advertiser_info.advertiser_id,category_name order by advertiser_name"; 
$sth = $dbh->prepare($sql);
$sth->execute();
while (($aid,$category_name,$uns_cnt,$bounce_cnt,$fullmbx_cnt,$uns_cnt_str,$bounce_cnt_str,$fullmbx_cnt_str,$notdelivered_cnt,$notdelivered_cnt_str) = $sth->fetchrow_array())
{
