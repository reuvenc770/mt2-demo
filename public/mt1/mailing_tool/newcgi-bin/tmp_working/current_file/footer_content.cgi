...
{
	$sql="select content_name,date_format(content_date,'%m/%d/%y'),date_format(inactive_date,'%m/%d/%y'),content_html from footer_content where content_id=$content_id";
	$sth = $dbh->prepare($sql) ;
	$sth->execute();
	($content_name,$content_date,$inactive_date,$content_html) = $sth->fetchrow_array();
	$sth->finish();
