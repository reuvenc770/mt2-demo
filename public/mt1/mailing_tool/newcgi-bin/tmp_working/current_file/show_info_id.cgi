...
end_of_html
$sql = "select email_list.email_addr,email_list.first_name,email_list.last_name,email_list.address,email_list.city,email_list.state,email_list.zip,source_url,capture_date,member_source,unsubscribe_date,list_name,dob,gender,email_list.status,subscribe_date,company from email_list,list,user where email_list.list_id=list.list_id and email_user_id=$tid and list.user_id=user.user_id";
$sth = $dbh->prepare($sql);
$sth->execute();
...
print<<"end_of_html";
			<td align="center"> <A HREF="unsub.cgi?eid=$tid&global=Y">Add to Global Suppression</A></td>
