...
util::header("Advertiser Campaign Totals Report - Last Month");
...
{
$sql = "select advertiser_info.advertiser_id,advertiser_name,category_name,sum(unsubscribe_cnt),sum(bounce_cnt),sum(fullmbx_cnt),format(sum(unsubscribe_cnt),0),format(sum(bounce_cnt),0),format(sum(fullmbx_cnt),0),sum(notdelivered_cnt),format(sum(notdelivered_cnt),0) from campaign,advertiser_info,category_info where sent_datetime >= '$cdate1' and sent_datetime < '$cdate' and campaign.advertiser_id=advertiser_info.advertiser_id and advertiser_info.category_id=category_info.category_id and campaign.campaign_id not in (select campaign_id from daily_deals) group by advertiser_info.advertiser_id,category_name order by advertiser_name"; 
}
$sth = $dbh->prepare($sql);
$sth->execute();
my $tname;
...
{
		$sql = "select sum(sent_cnt),format(sum(sent_cnt),0),sum(open_cnt),format(sum(open_cnt),0),sum(click_cnt),format(sum(click_cnt),0),sum(sent_cnt)-$bounce_cnt-$fullmbx_cnt-$notdelivered_cnt,format(sum(sent_cnt)-$bounce_cnt-$fullmbx_cnt-$notdelivered_cnt,0) from campaign_log where campaign_id in (select campaign_id from campaign where advertiser_id=$aid and sent_datetime >= '$cdate1' and sent_datetime < '$cdate')";
}
		$sth1 = $dbh->prepare($sql);
		$sth1->execute();
