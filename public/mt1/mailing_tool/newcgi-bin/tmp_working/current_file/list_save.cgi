...
$list_name = $dbh->quote($list_name);
my $status = $dbh->quote($query->param('status'));
my $optin_flag = $dbh->quote($query->param('optin_flag'));
my $double_mail_template = $dbh->quote($query->param('double_mail_template'));
my $thankyou_mail_template = $dbh->quote($query->param('thankyou_mail_template'));
...
elsif ($mode eq "ADD")
{
	...
	$sql = "insert into list (list_name, user_id, status, optin_flag, thankyou_mail_template,
		double_mail_template) values ($list_name, $user_id, $status, $optin_flag, 
