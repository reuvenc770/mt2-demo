...
<TD vAlign=top align=left>
    ...
    <TD vAlign=top align=left bgColor=#ffffff colSpan=10>
        ...
        <TD align=middle>
            ...
            <TABLE cellSpacing=0 cellPadding=0 width="1100px" border=1>
...
$cdate = $year_str . "-" . $month_str . "-01";
    if ($networkopt > 0)
    {
$sql = "select campaign_id,payout,campaign_name,category_name,date_format(sent_datetime,'%m/%d/%Y'),unsubscribe_cnt,bounce_cnt,fullmbx_cnt,format(unsubscribe_cnt,0),format(bounce_cnt,0),format(fullmbx_cnt,0),id,notdelivered_cnt,format(notdelivered_cnt,0),aol_flag,hotmail_flag,yahoo_flag,other_flag from campaign,advertiser_info,category_info where sent_datetime >= '$cdate' and campaign.advertiser_id=advertiser_info.advertiser_id and advertiser_info.category_id=category_info.category_id and campaign.campaign_id in (select distinct campaign_list.campaign_id from campaign_list,list where campaign_list.list_id = list.list_id and list.user_id=$networkopt) order by sent_datetime";
    }
	else
	{
$sql = "select campaign_id,payout,campaign_name,category_name,date_format(sent_datetime,'%m/%d/%Y'),unsubscribe_cnt,bounce_cnt,fullmbx_cnt,format(unsubscribe_cnt,0),format(bounce_cnt,0),format(fullmbx_cnt,0),id,notdelivered_cnt,format(notdelivered_cnt,0),aol_flag,hotmail_flag,yahoo_flag,other_flag from campaign,advertiser_info,category_info where sent_datetime >= '$cdate' and campaign.advertiser_id=advertiser_info.advertiser_id and advertiser_info.category_id=category_info.category_id order by sent_datetime"; 
	}
$sth = $dbh->prepare($sql);
$sth->execute();
...
print <<end_of_html;
	<tr><td width=100>$sent_datetime</td><td width=100>$company</td><td width=100>$list_str</td><td>$sendto_str</td><td width=230>$cname</td><td width=230>$category_name</td><td align=middle>$sent_cnt_str</td><td align=middle>$bounce_cnt_str</td><td align=middle>
end_of_html
...
print<<"end_of_html";
        ...
        <TR>
        <TD><IMG height="2" src="$images/spacer.gif" border=0></TD>
		</TR>
        <TR>
        <TD align="center">
			<a href="mainmenu.cgi">
