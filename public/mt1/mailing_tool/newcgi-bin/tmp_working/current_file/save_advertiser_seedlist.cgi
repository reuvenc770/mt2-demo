...
# get some directories from sysparm
	...
	$sql = "delete from advertiser_seedlist where advertiser_id=$aid";
	$rows = $dbh->do($sql);
	...
	print << "end_of_html" ;
		...
		<td colspan=4 align="center" width="100%">
			...
			<td width="50%" align="center">
				...
				<img src="$images/advertiserinfo.gif" border="0"></a></td>
			</tr>
			</table>
...
sub add_upd_email_user
{
	...
	$sql = "insert into advertiser_seedlist(advertiser_id,email_addr) values($aid,'$email_addr')";
	$rows = $dbh->do($sql);
   	if ($dbh->err() != 0)
   	{
       	$errmsg = $dbh->errstr();
       	util::logerror("Updating advertiser_seedlist record: $sql: $errmsg");
       	exit(0);
   	}
