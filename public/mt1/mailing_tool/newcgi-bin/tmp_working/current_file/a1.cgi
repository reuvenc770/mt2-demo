...
<center>
<h3>Open Log Report - Last Month</h3>
end_of_html
#
...
while ($client_id <= $set_id)
{
	...
	$sth->finish();
	$sql = "select distinct date_format(date_processed,'%m/%d/%Y'),date_processed from move_open_log where client_id=$client_id and date_processed >= '$cdate1' and date_processed < '$cdate' order by 1"; 
	$sth = $dbh->prepare($sql);
	$sth->execute();
	while (($sdate,$sdate1) = $sth->fetchrow_array())
	{
		...
		$sql = "select sum(sent_cnt) from profile_log where campaign_id in (select campaign_id from campaign where date(sent_datetime)= '$sdate1') and list_id in (select list_id from list where user_id=$client_id)";
		$sth1 = $dbh->prepare($sql);
		$sth1->execute();
	...
	$list_cnt = 0;
	$sql = "select distinct move_open_log.list_id,list_name from move_open_log,list where client_id=$client_id and date_processed >= '$cdate1' and date_processed < '$cdate' and move_open_log.list_id=list.list_id order by 1"; 
	$sth = $dbh->prepare($sql);
	$sth->execute();
...
while ($i < $cidcnt)
{
	...
	{
		$sth->finish();
		$sql = "select sum(sent_cnt) from profile_log where list_id=$LID[$j] and campaign_id in (select campaign_id from campaign where date(sent_datetime)= '$CDATEARR1[$i]')";
		$sth = $dbh->prepare($sql);
		$sth->execute();
		print LOG "<$sql>\n";
