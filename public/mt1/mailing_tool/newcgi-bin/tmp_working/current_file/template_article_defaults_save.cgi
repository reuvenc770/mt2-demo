...
for ($k=1 ; $k<=$num_articles ; $k++)
{
	...
	$article_title = $dbh->quote($query->param("article${k}_title"));
	$article_text = $dbh->quote($query->param("article${k}_text"));
	$article_link_name = $dbh->quote($query->param("article${k}_link_name"));
	$article_link_id = $dbh->quote($query->param("article${k}_link_id"));
	$article_image_url = $dbh->quote($query->param("article${k}_image_url"));
