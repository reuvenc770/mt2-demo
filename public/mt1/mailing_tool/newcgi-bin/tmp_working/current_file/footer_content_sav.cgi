...
if ($content_id == 0)
{
	$sql = "insert into footer_content(content_name,content_date,inactive_date,content_html) values('$content_name','$tdate','$idate','$html_code')";
	$sth = $dbh->do($sql);
...
{
	$sql = "update footer_content set content_name='$content_name',content_date='$tdate',inactive_date='$idate',content_html='$html_code' where content_id=$content_id";
	$sth = $dbh->do($sql);
}
