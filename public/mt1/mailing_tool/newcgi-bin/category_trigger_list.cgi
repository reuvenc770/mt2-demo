...
<TD vAlign=top align=left bgColor=#999999>
	...
	<TD vAlign=top align=left bgColor=#ffffff colSpan=10>
		<form method=post action=category_trigger_list.cgi>
...
$sql = "select category_info.category_id,category_name from category_info where status='A' order by category_name";
$sth = $dbhq->prepare($sql);
$sth->execute();
my $catcnt;
while (($cat_id,$category_name) = $sth->fetchrow_array())
{
	...
	if ($userid > 0)
	{
		$sql="select client_id,trigger1_aid,trigger2_aid,alt_trigger_aid from category_trigger where client_id in (0,$userid) and category_id=$cat_id and trigger_type=? order by client_id desc";
	}
	else
	{
		$sql="select client_id,trigger1_aid,trigger2_aid,alt_trigger_aid from category_trigger where client_id =$userid and category_id=$cat_id and trigger_type=? order by client_id desc";
	}
	$sth1 = $dbhq->prepare($sql);
	$sth1->execute($trigger_type);
	$trigger1_str="";	
	$trigger2_str="";	
	$alt_trigger_str="";	
	if (($tclient,$trigger1,$trigger2,$alt_trigger) = $sth1->fetchrow_array())
	{
		...
		if ($trigger1 > 0)
		{
			...
			$sth1->finish();
			my $cid="";
    		$sql="select campaign_id from campaign where campaign_name='$trigger1_str' and status='T' and deleted_date is null";
    		$sth1 = $dbhq->prepare($sql);
    		$sth1->execute();
    		($cid) = $sth1->fetchrow_array();
			$sth1->finish();
			if ($cid ne "")
			{
				$trigger1_str = $trigger1_str . " " . $cid;
			}
		}
		if ($trigger2 > 0)
		{
			...
			$sth1->finish();
			my $cid="";
    		$sql="select campaign_id from campaign where campaign_name='$trigger2_str' and status='T' and deleted_date is null";
    		$sth1 = $dbhq->prepare($sql);
    		$sth1->execute();
    		($cid) = $sth1->fetchrow_array();
			$sth1->finish();
		...
		if ($alt_trigger > 0)
		{
			$sql = "select advertiser_name from advertiser_info where advertiser_id = ?"; 
			$sth1 = $dbhq->prepare($sql);
			$sth1->execute($alt_trigger);
			($alt_trigger_str) = $sth1->fetchrow_array();
			$sth1->finish();
			my $cid="";
    		$sql="select campaign_id from campaign where campaign_name='$alt_trigger_str' and status='T' and deleted_date is null";
    		$sth1 = $dbhq->prepare($sql);
    		$sth1->execute();
    		($cid) = $sth1->fetchrow_array();
			$sth1->finish();
			if ($cid ne "")
			{
				$alt_trigger_str = $alt_trigger_str . " " . $cid;
			}
		}
	}
	else
	...
	if ($tclient > 0)
	{
		print qq { <TR bgColor=$bgcolor><TD align=left><font color="#509C10" face="verdana,arial,helvetica,sans serif" size="2"> <A HREF="category_trigger.cgi?cid=$cat_id&userid=$userid&trigger_type=$trigger_type">$category_name</a> (Advertisers - $catcnt) </font></TD> <TD><font color=red>$trigger1_str</font></TD> <TD><font color=red>$trigger2_str</font></TD> <TD><font color=red>$alt_trigger_str<font></TD> </TR> \n };
	}
	else
	{
		print qq { <TR bgColor=$bgcolor><TD align=left><font color="#509C10" face="verdana,arial,helvetica,sans serif" size="2"> <A HREF="category_trigger.cgi?cid=$cat_id&userid=$userid&trigger_type=$trigger_type">$category_name</a> (Advertisers - $catcnt)</font></TD> <TD>$trigger1_str</TD> <TD>$trigger2_str</TD> <TD>$alt_trigger_str</TD> </TR> \n };
	}
}
