...
my $format="H";
my $cdeploy="N";
my ($clientid,$bid,$template_id,$include_wiki,$headerid,$wikiID);
my $header_id;
my $footer_id;
...
$sql="select client_id,brand_id,creative_id,mailing_template,include_wiki,mailingHeaderID,wikiTemplateID,subject_id,advertiser_id,header_id,footer_id from test_campaign where test_id=$tid";
my $sth=$dbhu->prepare($sql);
$sth->execute();
($clientid,$bid,$creative_id,$template_id,$include_wiki,$headerid,$wikiID,$subject_id,$aid,$header_id,$footer_id)=$sth->fetchrow_array();
$sth->finish();

# Get the email template and substitute all the field data
my $the_email =  get_template($dbhu,$bid,$creative_id,$template_id,$clientid,0,0,"",'Y',$header_id,$footer_id);
$the_email=~s/{{DOMAIN}}/www.credithelpadvisor.com/g;
$the_email=~s/{{IMG_DOMAIN}}/www.credithelpadvisor.com/g;
my $subject;
if ($subject_id == 999999999)
{
	$sql="select advertiser_subject from advertiser_subject where advertiser_id=$aid and status='A' order by rand() limit 1";
...
if ($wikiID > 0)
{
	...
	$wiki_text=$mtaRandom->wikiText({'wikiID' => $wikiID});
}
}
$the_email=~s/{{WIKI}}/$wiki_text/g;
...
sub get_template
{
	my ($dbh,$bid,$creative_id,$template_id,$client_id,$mailing_domain_id,$binding_id, $fcode,$encrypt_link,$header_id,$footer_id)=@_;
	my $sql;
	my $sth;
	my $master_str;
	my $brand_unsub_img;
	...
	if ($header_id > 0)
	{
		$sql="select header_code from Headers where header_id=?"; 
		$sth=$dbh->prepare($sql);
		$sth->execute($header_id);
		($header_str)=$sth->fetchrow_array();
		$sth->finish();
	}
	$master_str=~s/{{HEADER}}/$header_str/g;
	my $footer_str="";
	if ($footer_id > 0)
	{
		$sql="select footer_code from Footers where footer_id=?"; 
		$sth=$dbh->prepare($sql);
		$sth->execute($footer_id);
		($footer_str)=$sth->fetchrow_array();
		$sth->finish();
	}
	$master_str=~s/{{FOOTER}}/$footer_str/g;
