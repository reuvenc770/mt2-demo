#!/usr/bin/perl
#===============================================================================
# Purpose: Displays a List of Article Sources 
# File   : article_source_list.cgi
#
#===============================================================================
...
sub disp_body
{
	...
	<TD bgcolor="#EBFAD1" align="left" width="10%">
		...
		<b>Name</b></font></td>
	<TD bgcolor="#EBFAD1" align="left" width=40%><FONT face="verdana,arial,helvetica,sans serif" color="#509C10" size=2><b>Image</b></font></td>
...
end_of_html
	...
	$sql = "select source_id,source_name,source_url,image_url from article_source order by source_name"; 
	$sth = $dbhq->prepare($sql) ;
	$sth->execute();
	$reccnt = 0 ;
	while (($sid,$name,$source_url,$image_url) = $sth->fetchrow_array())
	{
		$reccnt++;
		if ( ($reccnt % 2) == 0 ) 
		{
		...
		print qq{		<A HREF="article_source_edit.cgi?sid=$sid">$name</a></font></TD> \n } ;
		print qq{	<TD align=left><font color="#509C10" face="Arial" size="2"><a href="http://www.affiliateimages.com/$image_url">$image_url</a></font></TD> \n } ;
        print qq{	<TD align=left><font color="#509C10" face="Arial" size="2"><a href="/cgi-bin/article_source_del.cgi?sid=$sid">Delete</a></font>&nbsp;\n } ;
		print qq{</TR> \n} ;
