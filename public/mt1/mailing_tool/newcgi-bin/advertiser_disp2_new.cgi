...
my $dbhq1 = DBI->connect("DBI:mysql:new_mail:slavedb.routename.com", "db_user", "sp1r3V");
...
function delete_tracking(ctype)
{
	if (ProcessForm('A'))
	{
		...
		if (ctype == 'I')
		{
			document.edit_advertiser.backto.value="/cgi-bin/disp_tracking.cgi?aid=$puserid&tid="+document.edit_advertiser.iadvertiser_url.value;
		}
		if (ctype == 'R')
		{
			document.edit_advertiser.backto.value="/cgi-bin/disp_tracking.cgi?aid=$puserid&tid="+document.edit_advertiser.radvertiser_url.value;
		}
...
function edit_creative()
{
	if (ProcessForm('A'))
	{
	document.edit_advertiser.backto.value="/cgi-bin/edit_creative_new.cgi?aid=$puserid&cid="+document.edit_advertiser.creative.value;
	document.edit_advertiser.submit();
	}
}
...
print <<"end_of_html";
        ...
        <FORM name=edit_advertiser action="advertiser_upd2_new.cgi" method=post onsubmit="return ProcessForm('A');" ENCTYPE="multipart/form-data">
            ...
            <TD align=middle>
                ...
                <TD align=middle>
                    ...
                    <TD vAlign=center align=left>
						...
						</FONT>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="/cgi-bin/replace_url.cgi?aid=$puserid">Find/Replace URLs</a>&nbsp;&nbsp;<a href="/cgi-bin/rep_adv_subject_creative.cgi?aid=$puserid" target=_blank>Subject/Creative Stats</a>&nbsp;&nbsp;<a href="/cgi-bin/replace_advertiser_frame.cgi?aid=$puserid">Replace Advertiser</a></td>
					</TR>
                    <TR> <!-- -------- Email Addr -------------- -->
                    <TD vAlign=center noWrap align=right width="20%">
						<FONT face="verdana,arial,helvetica,sans serif" color=#509C10 size=2>
                    ...
                    <TD vAlign=center align=left>
						<FONT face="verdana,arial,helvetica,sans serif" color=#509C10 size=2>
						<INPUT size=50 maxlength=255 value="$email_addr" name=email_addr></FONT>&nbsp;&nbsp;<a href="/cgi-bin/advertiser_setup_new.cgi?aid=$puserid">Setup Creative/Subject/From Rotation</a>&nbsp;&nbsp;<a href="/cgi-bin/creative_deploy_it.cgi?aid=$puserid">Deploy Creative</a></TD>
					</TR>
					...
					<TD vAlign=center align=left>
						<FONT face="verdana,arial,helvetica,sans serif" color=#509C10 size=2>
						<INPUT size=50 maxlength=255 value="$internal_email_addr" name=internal_email_addr></FONT>&nbsp;&nbsp;<a href="/cgi-bin/emailreach_creative.cgi?aid=$puserid" target=_blank>EmailReach - Test All Creative</a>&nbsp;&nbsp;<a href="/cgi-bin/del_mon_tag.cgi?aid=$puserid&cytpe=D" target=_blank>Delivery Monitor - Test All Creative</a></TD>
					</TR>
...
end_of_html
	if ($cstatus eq "A")
	{
		print "<td align=left><input type=radio name=cstatus value=\"S\">Setup&nbsp;&nbsp;&nbsp;<input type=radio name=cstatus checked value=\"A\">Active&nbsp;&nbsp;&nbsp;<input type=radio name=cstatus value=\"I\" onClick=\"set_inactive();\">Inactive&nbsp;&nbsp;<input type=radio name=cstatus value=\"D\">Deleted<br><br></td>\n";
	}
	elsif ($cstatus eq "S")
	{
		print "<td align=left><input type=radio name=cstatus checked value=\"S\">Setup&nbsp;&nbsp;&nbsp;<input type=radio name=cstatus value=\"A\">Active&nbsp;&nbsp;&nbsp;<input type=radio name=cstatus value=\"I\" onClick=\"set_inactive();\">Inactive&nbsp;&nbsp;<input type=radio name=cstatus value=\"D\">Deleted<br><br></td>\n";
	}
	elsif ($cstatus eq "I")
	{
	...
	{
		print "<td align=left><input type=radio name=cstatus value=\"S\">Setup&nbsp;&nbsp;&nbsp;<input type=radio name=cstatus value=\"A\">Active&nbsp;&nbsp;&nbsp;<input type=radio name=cstatus value=\"I\" onClick=\"set_inactive();\">Inactive&nbsp;&nbsp;<input type=radio name=cstatus checked value=\"D\">Deleted<br><br></td>\n";
	}
print<<"end_of_html";
<tr><td><b>Inactive Date(yyyy-mm-dd):</b><br></td><td><input type=text name=idate value="$idate" maxlength=10 size=10></td></tr>
<tr><td> <b>Type:</b></td>
<td>
end_of_html
...
end_of_html
$sql = "select tracking_id,url,code,date_added,date_format(date_added,\"%m/%d/%y\"),company,daily_deal from advertiser_tracking, user where advertiser_id=$puserid and client_id=user.user_id and daily_deal != 'I' and daily_deal != 'R' order by mediactivate_id";
$sth = $dbhq->prepare($sql);
$sth->execute();
...
$sth->finish();
print<<"end_of_html";
</select><input type="button" value="Delete" name="B22" onClick=delete_tracking('N');>&nbsp;&nbsp;<input type="button" value="Gen URLs" name="B22" onClick=gen_tracking('N');></br></br></td></tr>
<tr><td valign=top><b><a href="Javascript:tracking('I');">Intela Redirect URL</a>: (in parens = listname)</b><br></td>
<td><select name=iadvertiser_url>
end_of_html
$sql = "select tracking_id,url,code,date_added,date_format(date_added,\"%m/%d/%y\"),company,daily_deal from advertiser_tracking, user where advertiser_id=$puserid and client_id=user.user_id and daily_deal = 'I' order by mediactivate_id";
$sth = $dbhq->prepare($sql);
$sth->execute();
my ($url,$code,$date_added,$fdate,$company,$tracking_id,$daily_deal);
while (($tracking_id,$url,$code,$date_added,$fdate,$company,$daily_deal) = $sth->fetchrow_array())
...
$sth->finish();
print<<"end_of_html";
</select><input type="button" value="Delete" name="B22" onClick=delete_tracking('I');>&nbsp;&nbsp;<input type="button" value="Gen URLs" name="B22" onClick=gen_tracking('I');></br></br></td></tr>
<tr><td valign=top><b><a href="Javascript:tracking('R');">Raut Media Redirect URL</a>: (in parens = listname)</b><br></td>
<td><select name=radvertiser_url>
end_of_html
$sql = "select tracking_id,url,code,date_added,date_format(date_added,\"%m/%d/%y\"),company,daily_deal from advertiser_tracking, user where advertiser_id=$puserid and client_id=user.user_id and daily_deal = 'R' order by mediactivate_id";
$sth = $dbhq->prepare($sql);
$sth->execute();
my ($url,$code,$date_added,$fdate,$company,$tracking_id,$daily_deal);
while (($tracking_id,$url,$code,$date_added,$fdate,$company,$daily_deal) = $sth->fetchrow_array())
...
$sth->finish();
print<<"end_of_html";
</select><input type="button" value="Delete" name="B22" onClick=delete_tracking('R');>&nbsp;&nbsp;<input type="button" value="Gen URLs" name="B22" onClick=gen_tracking('R');></br></br></td></tr>
...
#   Get creative 
#
$sql = "select creative_id, creative_name,original_flag, trigger_flag, approved_flag,mediactivate_flag from creative where status='A' and advertiser_id=$puserid order by creative_name";
$sth = $dbhq->prepare($sql);
$sth->execute();
my $cid;
my $cname;
...
while (($cid,$cname,$oflag,$tflag,$aflag,$mflag) = $sth->fetchrow_array())
{
	$temp_str = $cname . " (";
	if ($tflag eq "Y")
	{
