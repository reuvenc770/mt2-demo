...
if ($args->{submit}) {
	...
	if ($err_flag==1) {
		...
		if ($hrInfo->{user_id} eq 'intela') {
			$args->{redir}||='intela_deployed_view.cgi';
		}
		else {
			$args->{redir}||='intela_deployed_view_new.cgi';
		}
		my $cookie = "intelalogin=$hrInfo->{user_id}; path=/;";
	...
	clear_cookie();
	print "Location: intela_login.cgi?redir=$args->{redir}\n\n";
}
else {
	display_header();
...
sub validate_data {
        ...
        if (!$hrArgs->{userID}) {
			...
			my $dataHR={'intela'=>'creative1', 'dave'=>'dave'};
...
sub display_login_form {
	...
	<form method="post" action="intela_login.cgi">
