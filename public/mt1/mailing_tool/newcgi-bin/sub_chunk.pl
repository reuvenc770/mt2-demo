...
sub uns_upd_list_member
{
	...
	if (($emailid) = $sth2->fetchrow_array())
	{
		...
		my $sth2b = $dbhq->prepare($sql);
		$sth2b->execute();
		while (($user_id) = $sth2b->fetchrow_array())	
		{
			$sql="update email_chunk_list_${user_id} set status='U',unsubscribe_date=curdate(),unsubscribe_time=curtime() where email_user_id=$emailid and status='A'";
			$rows = $dbh2->do($sql);
		}
		$sth2b->finish();
	}
