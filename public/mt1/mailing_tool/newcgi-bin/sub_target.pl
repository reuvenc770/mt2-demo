...
sub uns_upd_list_member
{
	...
	$email_addr =~ s/'/''/g;
	$email_exists = 0 ;
	$sql = "select email_user_id,status from email_list where email_addr= '$email_addr' and list_id in (select list_id from list where user_id=23)";
	...
	if (($emailid,$cstatus) = $sth2->fetchrow_array())
	{
		if (($cstatus eq "A") || ($cstatus eq "P") || ($cstatus eq "G") || ($cstatus eq "L"))
		{
			...
			$rows = $dbh->do($sql) ;
			$sql = "insert into unsub_log(email_addr,unsub_date,client_id) values('$email_addr',curdate(),23)";
			$rows = $dbh->do($sql);
			$total_unscnt++;
    ...
    $sth2->execute();
    if (($emailid) = $sth2->fetchrow_array())
    {
        $sql="update email_chunk_list_23 set status='U',unsubscribe_date=curdate(),unsubscribe_time=curtime() where email_user_id=$emailid and status='A'";
        $rows = $dbh2->do($sql);
    }
    $sth2->finish();
} # end of sub
