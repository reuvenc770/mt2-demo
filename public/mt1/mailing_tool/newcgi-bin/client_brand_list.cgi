...
sub disp_header
{
    ...
    function Delete(bid,bname)
    {
        if (confirm("Are you sure you want to delete "+bname+"?"))
        {
            document.location.href="del_client_brand.cgi?bid="+bid;
        }
    }
	</script>
end_of_html
...
end_of_html
	...
	while (($bid,$bname) = $sth->fetchrow_array())
	{
		...
		print "<td><A HREF=\"javascript:Delete($bid,'$bname');\">Delete</a>&nbsp;&nbsp;<a href=\"move_brand.cgi?bid=$bid&cid=$cid\">Move</a></td>\n";
