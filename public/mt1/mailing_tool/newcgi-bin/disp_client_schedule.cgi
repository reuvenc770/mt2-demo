#!/usr/bin/perl
#===============================================================================
# Purpose: Bottom frame of view_schedule.cgi page 
# Name   : disp_client_schedule.cgi 
#
#--Change Control---------------------------------------------------------------
...
<body>
<form method=post action="/cgi-bin/save_client_schedule.cgi">
...
<table border="0" width="100%" id="table8">
	<tr>
		<td>&nbsp;</td>
		<td width="1130">
		<p align="right"><b><font face="Verdana">&nbsp;<a href="/cgi-bin/disp_client_schedule.cgi?clientid=$clientid&sdate=$startdate1">BACK</a>&nbsp;
		<a href="/cgi-bin/disp_client_schedule.cgi?clientid=$clientid&sdate=$startdate">FWD</a></font></b></td>
	</tr>
</table>
end_of_html
#
...
if ($stype eq "A")
{
	$sql = "select client_id,company,aol_cnt from network_schedule, user where client_id=user_id and client_id=$clientid order by client_id";
}
elsif ($stype eq "W")
{
	$sql = "select client_id,company,chunk_cnt from network_schedule, user where client_id=user_id and client_id=$clientid order by client_id";
}
else
{
	$sql = "select client_id,company,campaign_cnt from network_schedule, user where client_id=user_id and client_id=$clientid order by client_id";
}
$sth1 = $dbhq->prepare($sql);
$sth1->execute();
while (($client_id,$company,$camp_cnt) = $sth1->fetchrow_array())
{
$cday=$cday1;
$tday=$cday1;
...
$sth->finish();
print<<"end_of_html";
Network: <a href="/cgi-bin/upd_client_schedule.cgi?client_id=$clientid" target="_blank">$company<a/><br>
...
print<<"end_of_html";
	<tr>
		<td width="100"><font face="Verdana" size="1" style="font-weight: 700">Profile</font></td>
...
$sth->finish();
	...
	while ($i <= $camp_cnt)
	{
		...
		while ($j < 7)
		{
			if ($j == 0)
			{
				$sql="select profile_name from list_profile,schedule_info where list_profile.profile_id=schedule_info.profile_id and slot_type='$stype' and slot_id=$i and schedule_info.client_id=$clientid";
				$sth = $dbhq->prepare($sql);
				$sth->execute();
			...
			$sql = "select campaign_name,advertiser_id,campaign.campaign_id from campaign,camp_schedule_info where client_id=$clientid and camp_schedule_info.slot_id=$i and slot_type='$stype' and camp_schedule_info.schedule_date=date_add('$cday1',interval $tday1+$j day) and campaign.campaign_id=camp_schedule_info.campaign_id and camp_schedule_info.status='A'";
			$sth = $dbhq->prepare($sql);
			$sth->execute();
			if (($cname,$temp_aid,$camp_id) = $sth->fetchrow_array())
			{
				...
				{
					print "<td width=171>\n";
					print "<font face=\"Verdana\" size=1><a href=\"/cgi-bin/advertiser_disp2.cgi?pmode=U&puserid=$temp_aid\" target=\"_blank\">$cname</a>&nbsp;<font color=\"#FF00000\" face=\"Verdana\" size=\"1\"><b><a href=\"/cgi-bin/camp_history.cgi?campaign_id=$camp_id\" target=_blank>H</a></b>";
				}
				print "</font>\n";
			    $sql = "select vendor_supp_list_id from advertiser_info where advertiser_info.advertiser_id=$temp_aid"; 
    			$sth = $dbhq->prepare($sql) ;
    			$sth->execute();
				($sid)=$sth->fetchrow_array();
				$sth->finish();
			    $sql = "select list_name,last_updated,filedate,datediff(curdate(),last_updated) from vendor_supp_list_info where list_id=$sid"; 
    			$sth = $dbhq->prepare($sql) ;
    			$sth->execute();
    			if (($supp_name,$last_updated,$filedate,$day_cnt) = $sth->fetchrow_array())
    			{
