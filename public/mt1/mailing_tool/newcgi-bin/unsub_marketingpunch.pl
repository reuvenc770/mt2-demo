...
$sql="select date_format(date_sub(curdate(),interval 1 day),'%m_%d'),date_format(curdate(),'%Y%m%d')";
$sth=$dbhu->prepare($sql);
$sth->execute();
($cdate,$cdate1)=$sth->fetchrow_array();
$sth->finish();
my $filename="XLUnsub_".$cdate.".txt";
$legacy_filename="MP_Legacy_Unsub_".$cdate1.".txt";
$rtf_filename="MP_RTF_Unsub_".$cdate1.".txt";
open(LOG,">XLUnsub_$cdate.txt");
$sql="select email_addr from unsub_log where client_id in (1206,1184) and unsub_date >= date_sub(curdate(),interval 1 day) and unsub_date < curdate()";
$sth=$dbhu->prepare($sql);
$sth->execute();
...
if ($ftp)
{
    $ftp->login('marketingpunchunsub','n4b5LtRQ') or print "Cannot login ", $ftp->message;
    $ftp->ascii();
	$ftp->cwd("XLUnsubs");
    $ftp->put($filename) or print "put failed ", $ftp->message;
    print "Sent $filename\n";
	$ftp->cwd("../MPUnsubs");
	$ftp->get($legacy_filename);
	$ftp->get($rtf_filename);
    $ftp->quit;
}
process_file($legacy_filename,1206);
process_file($rtf_filename,1184);
...
#		$email_addr =~ s/\s.*$// ;   # remove from 1st white space at end of addr thru end of line
		...
		$total_cnt++;
		&uns_upd_list_member($email_addr,$client_id);
	} 
...
sub uns_upd_list_member
{
	...
	$email_addr =~ s/'/''/g;
	$email_exists = 0 ;
	$sql = "select email_user_id,email_list.status,user_id from email_list where email_addr= '$email_addr' and client_id=$client_id";
