...
end_of_html
	...
	if ( $user_type eq "A" )
	{
		print << "end_of_html";
			...
			<TR>
			<TD colSpan=3 height=12>&nbsp;&nbsp;&nbsp;&nbsp;
				<a href="client/client_disp.cgi?pmode=A">
				<FONT face="verdana,arial,helvetica,sans serif" color=#000000 size=2>
...
if ($user_type ne "R")
{
	...
	-->
			...
			<TR>
    		<TD colSpan=3 height=12>&nbsp;&nbsp;&nbsp;&nbsp; 
				<a href="client/client_disp.cgi?pmode=U&puserid=$user_id">
				<FONT face="verdana,arial,helvetica,sans serif" color=#000000 size=2>
...
#	}
	...
	$sth = $dbhq->prepare($sql);
	$sth2 = $dbhq->prepare("select member_cnt,aol_cnt,hotmail_cnt,msn_cnt,yahoo_cnt,foreign_cnt from list where list_id = ? and status = 'A' ") ;
	$sth->execute();
	$reccnt = 0 ;
