...
my ($list_id, $list_name, $email_addr, @email_array, $email_id);
...
sub process_file 
{
	...
	while (<SAVED>) 
	{
		...
		$line =~ s/\t/|/g ;
		$line =~ s/,/|/g ;
		($email_id, @rest_of_line) = split('\|', $line) ;
		
		$total_cnt++;
		$sql = "delete from member_list1 where list_id=105 and email_user_id=$email_id";
		$rows = $dbhu->do($sql) ;
	} 
