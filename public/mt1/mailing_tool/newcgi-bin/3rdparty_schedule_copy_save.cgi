...
# ******************************************************************************
# 3rdparty_schedule_copy_save.cgi
#
...
foreach my $network (@networks) 
{
	foreach my $tparty (@tpartys)
	{
		...
		while (($camp_id,$slot_id,$temp_date) = $sth->fetchrow_array())
		{
			...
			print LOG "<$sql>\n";
			$rows = $dbhu->do($sql);
			$sql = "update camp_schedule_info set status='D' where client_id=$network and campaign_id=$camp_id and slot_id=$slot_id and schedule_date='$temp_date' and slot_type='3' and status='A'";
			print LOG "<$sql>\n";
			$rows = $dbhu->do($sql);
		}
		$sth->finish();
#
		$sql="select camp_schedule_info.client_id,camp_schedule_info.campaign_id,camp_schedule_info.slot_id,schedule_date,campaign.advertiser_id,hour(schedule_info.schedule_time),advertiser_info.category_id,advertiser_info.advertiser_name,advertiser_info.exclude_from_brands_w_articles,schedule_info.brand_id,schedule_info.performance,campaign.campaign_name,advertiser_info.vendor_supp_list_id,schedule_info.log_campaign from camp_schedule_info,campaign,advertiser_info,schedule_info where camp_schedule_info.client_id=$network and camp_schedule_info.slot_type='3' and schedule_date >= '$sdate' and schedule_date <= '$edate' and camp_schedule_info.slot_id=schedule_info.slot_id and schedule_info.client_id=camp_schedule_info.client_id and schedule_info.slot_type='3' and schedule_info.status='A' and camp_schedule_info.status='A' and schedule_info.third_party_id=$tparty and camp_schedule_info.campaign_id=campaign.campaign_id and campaign.advertiser_id=advertiser_info.advertiser_id and advertiser_info.status='A' and deleted_date is null"; 
		$sth = $dbhu->prepare($sql);
		$sth->execute();
print LOG "<$sql>\n";
		while (($client_id,$camp_id,$slot_id,$temp_date,$adv_id,$stime,$catid,$aname,$exclude_from_brands_w_articles,$brand_id,$priority,$campaign_name,$suppid,$log_camp) = $sth->fetchrow_array())
		{
			if ($priority == 0)
			{
					$priority=5;
...
#
				...
				($cname,$sdate1) = $sth1->fetchrow_array();
				$sth1->finish();
	        	$sql="insert into 3rdparty_campaign(third_party_id,client_id,brand_id,advertiser_id,deploy_name,vsgID, campaign_id,scheduled_datetime,mta_id,priority,log_campaign) values($tparty,$network,$bid,$adv_id,'$cname','$vsgID', $new_camp_id,'$sdate1',$mta_id,$priority,'$log_camp')";
	        	$rows=$dbhu->do($sql);
				...
				if ($tparty != 10)
				{
					thirdparty::deploy_it($dbhu,$tparty,$new_camp_id,$bid,$adv_id,$network);
				}
				}
				else
				{
