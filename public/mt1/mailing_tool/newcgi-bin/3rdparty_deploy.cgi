...
# 3rdparty_deploy.cgi
#
# this page displays the 3rd party deploy page 
#
# History
...
if ($old_id > 0)
{
	$sql="select third_party_id,client_id,brand_id,catid,advertiser_id,vsgID, deploy_name,date_format(scheduled_datetime,'%m/%d/%Y'),hour(scheduled_datetime) from 3rdparty_campaign where id=$old_id";
	$sth = $dbhq->prepare($sql);
	$sth->execute();
	($old_mailer_id,$old_client_id,$old_brand_id,$old_catid,$old_aid,$old_vsgID, $old_name,$sdate,$hour) = $sth->fetchrow_array();
	$sth->finish();
}
if ($mode eq "Copy")
{
...
function update_brand()
{
    ...
    var selLength = campform.catid.length;
    while (selLength>0)
    {
        campform.catid.remove(selLength-1);
        selLength--;
    }
    campform.catid.length=0;
    parent.frames[1].location="/newcgi-bin/3rd_upd_brand.cgi?cid="+selObj.options[selIndex].value;
}
...
print<<"end_of_html";
	...
	else
	{
		...
		campform.campaign_name.value = argname + " (" + selObj.options[selIndex].text + ")";
	}		
end_of_html
}
print<<"end_of_html";
...
function update_advertiser()
{
    ...
    var selIndex = selObj.selectedIndex;
    var selObj1 = document.getElementById('client_id');
    var selIndex1 = selObj1.selectedIndex;
    ...
    parent.frames[1].location="/newcgi-bin/3rd_upd_advertiser.cgi?cid="+selObj.options[selIndex].value+"&client="+selObj1.options[selIndex1].value;
}
</script>
</head>
	...
	<tr>
		...
		<table cellSpacing="0" cellPadding="10" width="100%" bgColor="#ffffff" border="0" id="table5">
			<tr>
				...
				<form name="campform" onsubmit="return ProcessForm('A');" method="post" action="/cgi-bin/3rdparty_deploy_it.cgi" target=_top>
										...
										<tr bgColor="#e3fad1">
											...
											<table cellSpacing="0" cellPadding="0" width="100%" border="0" id="table12">
												...
												<tr>
													...
													<td>
													<select size="1" name="mailer_id" onChange="update_name();">
end_of_html
$sql = "select third_party_id,mailer_name,num_subject,num_from,num_creative from third_party_defaults where status='A' order by mailer_name";
$sth = $dbhq->prepare($sql) ;
$sth->execute();
while (($id,$mailer_name,$num_subject,$num_from,$num_creative) = $sth->fetchrow_array())
{
	if ($id == $old_mailer_id)
	{
		print "<option selected value=$id>$mailer_name</option>\n";
	}
	else
	{
		print "<option value=$id>$mailer_name</option>\n";
	}
}
$sth->finish();
print<<"end_of_html";
...
&nbsp;</td>
												...
												<tr>
													...
													<select name="client_id" onChange="update_brand();">
end_of_html
$sql = "select user_id,company from user where status='A' order by company";
$sth = $dbhq->prepare($sql) ;
$sth->execute();
while (($id,$mailer_name) = $sth->fetchrow_array())
{
	if ($id == $old_client_id)
	{
		print "<option selected value=$id>$mailer_name</option>\n";
...
if ($old_client_id > 0)
{
	$sql="select brand_id,brand_name from client_brand_info where client_id=$old_client_id and status='A' order by brand_name";
	$sth = $dbhq->prepare($sql) ;
	$sth->execute();
	while (($id,$mailer_name) = $sth->fetchrow_array())
	{
...
end_of_html
my $prev_vsgID='';
$sql="SELECT vsgID, ip FROM server_config sc, server_ip_config sic WHERE sc.id=sic.id AND sv.inService=1 AND sc.type='strmail' ORDER BY vsgID ASC, ip ASC";
$sth = $dbhq->prepare($sql);
$sth->execute();
while (my ($vsgID, $ip)=$sth->fetchrow_array) {
	my $selected=($old_vsgID eq $vsgID) ? 'SELECTED' : '';
	print qq^<option value='$vsgID' $selected>$vsgID [all IPs]</option>\n^ if $vsgID ne $prev_vsgID;
	print qq^<option value='$vsgID-$ip' $selected>$vsgID-$ip</option>\n^;
	$prev_vsgID=$vsgID;
}
$sth->finish;
print<<"end_of_html";
