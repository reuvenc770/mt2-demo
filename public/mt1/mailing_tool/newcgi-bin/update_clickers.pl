	...
	while (($client_id,$tab) = $sth1->fetchrow_array())
	{
		...
		while (($list_id,$list_name) = $sth2->fetchrow_array())
		{
			...
			while ($i > 0)
			{
				...
				$sql = "select email_user_id from daily_click_log where list_id = $list_id and date_clicked = '$cday'"; 
				$sth = $dbh2->prepare($sql);
				$sth->execute();
				while (($email_user_id) = $sth->fetchrow_array())
				{
					...
					$rows = $dbh->do($sql);
					$sql = "update OpenClicks set last_click='$cday' where email_user_id=$email_user_id";
					$rows = $dbh->do($sql);
...
#					}
				...
				$sth->finish();
				$sql = "delete from daily_click_log where date_clicked ='$cday' and list_id=$list_id";			
				$rows = $dbh2->do($sql);
				$i--;
			}
#			if ($move_cnt > 0)
