...
# sm2_preview.cgi
#
# this page presents the user with a preview of the strongmail test deal 
#
# History
...
$sql="select mailing_template,advertiser_id,brand_id,mailing_domain,creative_id,client_id,header_id,footer_id from test_campaign where test_id=$tid";
$sth=$dbhu->prepare($sql);
$sth->execute();
($template_id,$aid,$brand_id,$mailing_domain,$creative_id,$client_id,$header_id,$footer_id)=$sth->fetchrow_array();
$sth->finish();
#
...
$sql="select privacy_img,unsub_img,client_id,brand_name,mailing_addr1,mailing_addr2,header_text from client_brand_info where brand_id=$brand_id";
$sth = $dbhq->prepare($sql);
$sth->execute();
($privacy_img,$brand_unsub,$nid,$bname,$addr1,$addr2,$header_text) = $sth->fetchrow_array();
$sth->finish();

$content_html=~s/{{HEADER_TEXT}}/$header_text/g;
...
<body>
<center><h2>Test Campaign Preview</h2>
<hr width=80%>
<br>
</center>
end_of_html
