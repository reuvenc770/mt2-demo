...
if (!$cookieID) {
	print "Location: cbs_login.cgi?redir=cbs_deployed_view_new.cgi\n\n";
}
else {
	my $infoHR={};
	$infoHR->{cookie}=$cookieID;
	if ($infoHR->{cookie} eq 'cbs') {
		my $sql3ID=qq|SELECT third_party_id FROM third_party_defaults WHERE mailer_name='CBS'|;
		my $sth3ID=$DBHQ->prepare($sql3ID);
		$sth3ID->execute();
		$infoHR->{thirdID}=$sth3ID->fetchrow;
		$sth3ID->finish();
	...
	my $Ainfo={};
	if ($args->{aID}) {
		$infoHR->{uid}=$args->{cID}; ### client_id(user_id)
		$infoHR->{DBID}=$args->{bID}; ### brand_id
	...
	display_header($args,$infoHR);
	display_available_deployed_campaign($infoHR,$args);
	if ($args->{aID}) {
		if ($args->{thirdID}==1) {
			display_dosmonos_info($args,$infoHR,$Ainfo);
		}
		else {
...
sub display_available_deployed_campaign {
	...
	my $client_clause=my $brand_clause="";
#	my $hr={};
	my @info;

	if ($infoHR->{cookie} eq 'cbs') {
		$client_clause=qq^AND client_id='$infoHR->{uid}'^;
		$brand_clause=qq^AND brand_id='$infoHR->{DBID}'^;
	}
	my $quer=qq|SELECT 3c.id, 3c.third_party_id, company, brand_name, advertiser_name, profile_name, 3c.client_id, |
			.qq|3c.brand_id as brand_id, a.category_id as catid, 3c.advertiser_id, deploy_name, 3c.campaign_id, |
			.qq|date(3c.scheduled_datetime) as date, date(3c.scheduled_datetime) AS sch_date FROM campaign c, |
			.qq|3rdparty_campaign 3c, advertiser_info a, |
			.qq|client_brand_info cbi, user u, list_profile lp WHERE 3c.campaign_id=c.campaign_id AND 3c.client_id=u.user_id |
			.qq|AND 3c.brand_id=cbi.brand_id AND 3c.advertiser_id=a.advertiser_id AND c.profile_id=lp.profile_id AND |
			.qq|c.deleted_date IS NULL AND 3c.third_party_id='$args->{mailer}' AND $date_clause |
			.qq|ORDER BY company ASC, profile_name ASC, advertiser_name ASC|;
...
##	foreach (sort {$hr->{$a}{deploy_name} cmp $hr->{$b}{deploy_name}} keys %$hr) {
	foreach my $info (@info) {
		my $mailer=$infoHR->{mailer_name};
		$mailer=~s/\s//g;
		my $client=$info->{company};
		$client=~s/\s//g;
		my $profile=$info->{profile_name};
		$profile=~s/\s//g;
		my $adv_name=$info->{advertiser_name};
		$adv_name=~s/\s//g;
		my $dp_name=$mailer.$client.$profile.$adv_name.$info->{date};
		print qq^
		<tr>
		  <td><a href="cbs_deployed_view_new.cgi?$href&id=$info->{id}&thirdID=$info->{third_party_id}&cID=$info->{client_id}&bID=$info->{brand_id}&catID=$info->{catid}&aID=$info->{advertiser_id}&campID=$info->{campaign_id}&mailer=$args->{mailer}&sch_date=$info->{sch_date}#info">$dp_name</a></td>
		</tr>
		^;
	}
...
# my $quer=qq|SELECT 3c.id, c.campaign_id, c.profile_id,substr(profile_name,1,8) AS profile_name, advertiser_name, brand_name, company, url, |
#			.qq|date_format(date(3c.scheduled_datetime), "%m-%d-%Y") AS sch_date FROM 3rdparty_campaign 3c, campaign c, |
#			.qq|list_profile lp, advertiser_info ai, schedule_info si,client_brand_info cbi, user u, brand_url_info bui |
#			.qq|WHERE 3c.campaign_id=c.campaign_id AND c.profile_id=lp.profile_id AND lp.profile_id=si.profile_id AND |
#			.qq|3c.advertiser_id=ai.advertiser_id AND si.brand_id=3c.brand_id AND si.brand_id=cbi.brand_id AND |
#			.qq|cbi.brand_id=3c.brand_id AND si.brand_id=bui.brand_id AND bui.brand_id=cbi.brand_id AND bui.brand_id=3c.brand_id |
#			.qq|AND si.client_id=u.user_id AND 3c.client_id=si.client_id AND 3c.client_id=u.user_id AND bui.url_type='O' |
#			.qq|AND 3c.id='$args->{id}'|;
	my $quer=qq|SELECT 3c.id, c.campaign_id, c.profile_id, substr(profile_name,1,8) AS profile_name, advertiser_name, brand_name, |
			.qq|company, url, date_format(date(3c.scheduled_datetime), "%m-%d-%Y") AS sch_date FROM user u, campaign c, |
			.qq|list_profile lp, 3rdparty_campaign 3c, client_brand_info cbi, advertiser_info ai,  brand_url_info bui WHERE |
			.qq|c.campaign_id=3c.campaign_id AND 3c.advertiser_id=ai.advertiser_id AND 3c.brand_id=cbi.brand_id AND |
			.qq|3c.brand_id=bui.brand_id AND c.profile_id=lp.profile_id AND lp.client_id=3c.client_id AND u.user_id=3c.client_id |
			.qq|AND 3c.id=$args->{id} AND bui.brand_id=$args->{bID} AND 3c.client_id=$args->{cID} AND url_type='O'|;

	my $sth=$DBHQ->prepare($quer);
	$sth->execute;
	my $info=$sth->fetchrow_hashref;
        my $domQ=qq|SELECT subdomain_name FROM category_brand_info cbi, brandsubdomain_info bi WHERE category_id='$args->{catID}' |
                .qq|AND bi.subdomain_id=cbi.subdomain_id AND cbi.brand_id='$args->{bID}'|;
        ...
        $sthD->finish;
        my $lIDq=qq|SELECT link_id  FROM advertiser_tracking WHERE advertiser_id='$args->{aID}' AND client_id='$args->{cID}' |
        ...
        my $adv_link=qq^http://$info->{url}/cgi-bin/redir1.cgi?eid=$infoHR->{emailid_replace}&cid=$info->{campaign_id}&^
                    .qq^id=$linkID&f=&s=&c=^;
        ...
        my $client_unsub_link=qq^http://${footer_subdomain}.$info->{url}/cgi-bin/redir1.cgi?eid=$infoHR->{emailid_replace}&^
                             .qq^cid=$info->{campaign_id}&id=42&nid=$args->{cID}^;
        my $privacy_unsub_link=qq^http://${footer_subdomain}.$info->{url}/cgi-bin/redir1.cgi?eid=$infoHR->{emailid_replace}&^
                              .qq^cid=0&id=51&nid=$args->{cID}^;
        my $img_link=qq^http://$info->{url}/cgi-bin/open_email1.cgi?id=$infoHR->{emailid_replace}&cid=$info->{campaign_id}&f=&s=&c=&^;
		$adv_link=~s/\#//g;
		$adv_unsub_link=~s/\#//g;
		$client_unsub_link=~s/\#//g;
##        my $color=($count%2==0) ? "#E9E9E9" : "#FFFFFF";
		...
		my $adv_name=$info->{advertiser_name};
		$adv_name=~s/\s//g;
		my $sch_name=$mailer.$client.$profile.$adv_name.$info->{sch_date};
		my $sch_name2=$mailer.$client.$adv_name.$info->{sch_date};
		$sch_name=~s/\(//;
		$sch_name=~s/\)//;
		$sch_name=~s/\://;

        my $report_purpose="";
        if ($args->{mailer}==5 || $args->{mailer}==9) {
            my $report_name=$sch_name;
            my $cid_str="&cid=".$info->{campaign_id};
            my $str_len=64- (length $cid_str);
            $report_name=~s/$mailer//;
            $report_name=substr($report_name,0,$str_len);
            $report_name.=$cid_str;
            $report_purpose=qq^
                <tr bgcolor=#FFFFFF>
                    <td colspan=2 width=100%>
                      <font type="Arial" size=4>$report_name</font>
                </td></tr>
            ^;
        }
		else {
			$report_purpose=qq^
                <tr bgcolor=#FFFFFF>
                    <td colspan=2 width=100%>
                      <font type="Arial" size=4>$sch_name2</font>
                </td></tr>
                <tr bgcolor=#FFFFFF>
                    <td colspan=2 width=100%>
                      <font type="Arial" size=4>$sch_name</font>
                </td></tr>
			^;
		}
        print qq^
          ...
          <tr>
            <td>
              ...
              <table border=0 bgcolor=#E3FAD1 width=100%>
                ...
                <tr bgcolor=$color><td width=15%><font type="Arial" size=-1><b>IMG URL</b></font></td>
                    <td width=85%><a href="$adv_link" target=_blank>$img_link</a></td>
                </tr>
...
#			 .qq|AND cbi.exclude_thirdparty='N'|;
	...
	while (my $info=$sth->fetchrow_hashref) {
		...
		my $lIDq=qq|SELECT link_id  FROM advertiser_tracking WHERE advertiser_id='$args->{aID}' AND client_id='$info->{cid}' |
				.qq|AND daily_deal='$args->{thirdID}'|;
		my $sthLID=$DBHQ->prepare($lIDq);
		$sthLID->execute;
		my $linkID=$sthLID->fetchrow;
		$sthLID->finish;
		...
		my $adv_link=qq^http://$info->{url}/cgi-bin/redir1.cgi?eid=$infoHR->{emailid_replace}&cid=$campID&^
					.qq^em=$infoHR->{email_replace}&id=$linkID&f=&s=&c=^;
		my $adv_unsub_link=qq^http://$footer_subdomain.$info->{url}/cgi-bin/redir1.cgi?eid=$infoHR->{emailid_replace}&cid=0&^
						  .qq^em=$infoHR->{email_replace}&id=$Ainfo->{ulinkID}^;
		my $client_unsub_link=qq^http://${footer_subdomain}.$info->{url}/cgi-bin/redir1.cgi?eid=$infoHR->{emailid_replace}&^
							 .qq^cid=$campID&em=$infoHR->{email_replace}&id=42&nid=$info->{cid}^;
		my $privacy_unsub_link=qq^http://${footer_subdomain}.$info->{url}/cgi-bin/redir1.cgi?eid=$infoHR->{emailid_replace}&^
							  .qq^cid=0&em=$infoHR->{email_replace}&id=51&nid=$info->{cid}^;
...
sub display_creative {
	...
	my $sql=qq|select el.creative_id,creative.creative_name,((sum(el.click_cnt)/sum(el.sent_cnt))*100000) from email_log el, |
		   .qq|creative,campaign c where el.creative_id=creative.creative_id and trigger_flag='N' and creative.status='A' and |
		   .qq|creative.advertiser_id='$args->{aID}' and el.campaign_id=c.campaign_id and c.scheduled_date >= date_sub(curdate(),interval 60 day) and c.deleted_date is null and c.advertiser_id=$args->{aID} group by 1,2 order by 3 desc limit $infoHR->{num_creative}|;
	my $sth=$DBHQ->prepare($sql);
	$sth->execute;
	my $camp_cnt = 1;
	while (my ($cid,$creative_name,$temp_val)=$sth->fetchrow_array()) {
		my $querNum=qq|select (sum(el.open_cnt)/sum(el.sent_cnt)*100),(sum(el.click_cnt)/sum(el.open_cnt)*100),|
				   .qq|((sum(el.click_cnt)/sum(el.sent_cnt))*100000) from email_log el,campaign c where creative_id=? and el.campaign_id=c.campaign_id and c.scheduled_date >= date_sub(curdate(),interval 60 day) and c.deleted_date is null and c.advertiser_id=$args->{aID}|;
		my $sthNum=$DBHQ->prepare($querNum);
		$sthNum->execute($cid);
		my ($copen,$cclick,$cindex) = $sthNum->fetchrow_array();
		$sthNum->finish();

		my $sqlC=qq|select approved_flag,original_flag,html_code from creative where creative_id=$cid|;
		my $sthC=$DBHQ->prepare($sqlC);
		$sthC->execute();
		my ($aflag,$oflag,$html_code) = $sthC->fetchrow_array();
		$sthC->finish();
...
{{UNSUBSCRIBE}}
        ...
        if ($Ainfo->{unsub_image} ne "") {
            ...
            if ( /\// )
            {
            	$temp_str = "<a href=\"http://{{FOOTER_SUBDOMAIN}}.{{DOMAIN}}/cgi-bin/redir1.cgi?eid={{EMAIL_USER_ID}}&amp;cid=0&amp;em={{EMAIL_ADDR}}&amp;id=$Ainfo->{ulinkID}\" target=\"_blank\"><img src=\"http://www.{{IMG_DOMAIN}}/images/$Ainfo->{unsub_image}\" border=0 alt=\"Unsub\"></a><br><br>";
			}
			else
			{
            	$temp_str = "<a href=\"http://{{FOOTER_SUBDOMAIN}}.{{DOMAIN}}/cgi-bin/redir1.cgi?eid={{EMAIL_USER_ID}}&amp;cid=0&amp;em={{EMAIL_ADDR}}&amp;id=$Ainfo->{ulinkID}\" target=\"_blank\"><img src=\"http://www.{{IMG_DOMAIN}}/images/unsub/$Ainfo->{unsub_image}\" border=0 alt=\"Unsub\"></a><br><br>";
			}
		}
...
#		$html_code =~ s/{{EMAIL_ADDR}}//g;
		$html_code =~ s/{{EMAIL_ADDR}}/$infoHR->{email_replace}/g;
		$html_code =~ s/{{EMAIL_USER_ID}}/$infoHR->{emailid_replace}/g;
...
my $date_options=print_nav_form($args,$infoHR);
            ...
            <tr>
                ...
                <table cellSpacing="0" cellPadding="0" width="100%" border="0" id="table4">
                    <tr>
						...
						<td align="right"><b>
							<a style="TEXT-DECORATION: none" href="cbs_login.cgi?action=logout&redir=cbs_deployed_view_new.cgi">
...
sub retrieve_cookie {
    ...
    if ($cookies{'cbslogin'} ne "0") {
        $login_ok = $cookies{'cbslogin'};
    }
    else {
        $login_ok = 0;
    }
...
sub print_nav_form {
  ...
  my $r_select= my $c_select= my $l_select= my $t_select=my $tm_select=my $s_select="";
  if ($args->{r_type} eq "r") { $r_select="CHECKED"; }
  if ($args->{r_type} eq "s") { $s_select="CHECKED"; }
  if ($args->{r_type} eq "t") { $t_select="CHECKED"; }
  if ($args->{r_type} eq "tm") { $tm_select="CHECKED"; }
 ...
 if ($infoHR->{cookie} ne "cbs") {
  $mailer=qq^
  <tr height=20>
	<td colspan=2><b>3rd Party Mailer:</b>
	<select name=mailer>
  ^;
  my $quer=qq|select third_party_id,mailer_name from third_party_defaults order by mailer_name|;
  my $sth=$DBHQ->prepare($quer);
  $sth->execute;
  ...
  <table align=center border=0 width=100% bgcolor=#509c10>
    ...
    <tr>
      <td align=center bgcolor=#ffffff>
        <form method=post action="cbs_deployed_view_new.cgi">
        <table border=0 width=100%>
		  $mailer
          <tr height=20>
          ...
          <tr height=20>
            <td width=5%><input type=radio name="r_type" value="tm" $tm_select></td>
            <td align=left><b>Tomorrow</b></td>
          </tr>
          <tr height=20>
...
sub build_date_clause {
		...
		elsif ($args->{r_type} eq 'tm') {
			$date_clause=qq^to_days($name)=to_days(date_add(curdate(), interval 1 day))^;
			$href=qq^r_type=tm^;
		}
        else {  
