...
if ($args->{submit}) {
	...
	if ($err_flag==1) {
		...
		my $cookie_data=CGI::Cookie->new(-name=>"deploy",
                                         -path=>"/",
                                         -value=>{id=>"$hrInfo->{id}", username=>"$hrInfo->{username}"});
		$args->{redir}='third_party_deployed.cgi';
...
sub validate_data {
        ...
        if (!$hrArgs->{userID}) {
			...
			my $quer=qq|SELECT id,username,passwd FROM third_party_login WHERE username=?|;
            ...
            elsif (lc($hrData->{passwd}) ne lc($hrArgs->{passwd})) {
                $err_flag=1;
...
sub display_login_form {
	...
	<form method="post" action="third_party_login.cgi">
