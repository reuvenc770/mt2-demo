...
$sql="select list_id,list_name from list where list_name in ('Openers','Clickers') and status='A' and user_id in (678)";
...
sub uns_upd_list_member
{
	...
	$email_addr =~ s/'/''/g;
	$email_exists = 0 ;
	$sql = "select email_user_id,status,client_id from email_list where email_addr= '$email_addr' and client_id  in (678)";
