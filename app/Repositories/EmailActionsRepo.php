...
class EmailActionsRepo {
    ...
    public function pullAggregatedListProfileActions($start, $end) {
        return DB::select("SELECT
            ...
            e.lower_case_md5,
            e.upper_case_md5,
            IFNULL(ed.id, 0) as email_domain_id,
            IFNULL(dg.id, 0) as email_domain_group_id,
            IFNULL(d.offer_id, 0) as offer_id,
            IFNULL(cv.id, 0) as cake_vertical_id,
            (ea.opens > 0) as has_esp_open,
            (ea.opens > 0) as has_open,
            (ea.clicks > 0) as has_esp_click,
            (ea.clicks > 0) as has_click,
            ea.deliveries,
            ea.opens,
            ea.clicks,
            d.party,
