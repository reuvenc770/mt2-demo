...
class Assets {
	...
	private function processAssets(array $attributes, array $assets, $extension, $source_dir, $filters, $format_link, $format_inline) {
		...
		foreach ($assets as $asset) {
			...
			if (!$this->public->has("public/".$path . '/' . $hash . $extension)) {
				if ($this->isAbsoluteUrl($asset)) {
				...
				$this->public->write("public/". $path . '/' . $hash . $extension, $data);
				$this->public->write("public/". $path . '/' . $hash . '.min' . $extension, $data);
			}
			$hashes[] = $hash;
		}
	...
	private function concatenateFiles($path, $sources, $destination, $extension) {
		if (!$this->public->has("public/".$path . '/' . $destination . $extension)) {
			$data = '';
			foreach ($sources as $source) {
				$data .= $this->public->read("public/".$path . '/' . $source . $extension);
			}
			$this->public->write("public/".$path . '/' . $destination . $extension, $data);
		}
	}
	...
	private function createGzip($path) {
		...
		if ($gzip >= 1 && $gzip <= 9 && function_exists('gzcompress') && !$this->public->has($path . '.gz')) {
			...
			$this->public->write("public/".$path . '.gz', $content_gz);
		}
	}
