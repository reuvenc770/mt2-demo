...
class Kernel extends ConsoleKernel
{
    ...
    protected $commands = [
        ...
        Commands\ReprocessFirstPartyActions::class
];
    ...
    protected function schedule(Schedule $schedule)
    {
         ...
         */
        $schedule->command('domains:expired')->dailyAt(self::REPORT_TIME); //command DomainExpirationNotification, job class domainExpirationNotifications, job name ExpiredDomains //bb
        $schedule->command('notify:scheduled')->everyMinute(); //command ScheduledNotificationsCommand, job class ScheduledNotificationQueueJob, job name ScheduledNotificationQueueJob:% //bb
        $schedule->command('monitors:runtime --mode=monitor --days-back=1 --runtime-threshold=30s')->cron('05 8,16 * * * *'); //job class: RunTimeMonitorJob bb
        ...
        $schedule->command('reports:generateEspUnsubReport --lookback=1')->dailyAt(self::REPORT_TIME); //command ESPUnsubsReport, job class GenerateEspUnsubReport, job name GenerateEspUnsubReport //bb

        $schedule->command('exportUnsubs emailsForOpensClicks --lookback=15 --runtime-threshold=1h')->dailyAt(self::REPORT_TIME); // command ExportActionsElsewhere, job class ExportActionsJob, job name <report_name>-<date> //bb
        $schedule->command('exportUnsubs ZxSprintUnsubExport --lookback=1 --runtime-threshold=1m')->dailyAt(self::REPORT_TIME); //bb
        $schedule->command('exportUnsubs ZxEsuranceUnsubExport --lookback=1 --runtime-threshold=1m')->dailyAt(self::REPORT_TIME); //bb

        #$unsubFilePath = storage_path( 'logs' ) . "/unsubJobs.log";
        $schedule->command( 'suppression:sendToMT1 3' )->cron('15 */4 * * * *'); //command and job class: SendSuppressionsToMT1, job name FTPSuppressionsToMT1 //bb
        $schedule->command('suppression:exportPublicators 1')->cron('10 */4 * * *'); //command SharePublicatorsUnsubs, job class SharePublicatorsUnsubsJob, job name ExportPublicatorsUnsubs //bb
        
        /**
         * Campaign Data Daily
         */
        $filePath = storage_path('logs')."/downloadAPI.log";
        $schedule->command('reports:downloadApi BlueHornet --daysBack=5 --runtime-threshold=60s')->hourly()->sendOutputTo($filePath); //command GrabApiEspReports, job class: RetrieveApiReports, job name: RetrieveApiEspReports //bb
        $schedule->command('reports:downloadApi Campaigner --daysBack=5 --runtime-threshold=60s')->hourly()->sendOutputTo($filePath); //bb
        ...
        $schedule->command('reports:downloadApi Maro --daysBack=5 --runtime-threshold=60s')->hourly()->sendOutputTo($filePath); //bb
        $schedule->command('reports:updateMissingMaroCampaigns')->daily()->sendOutputTo($filePath); //command UpdateMissingMaroCampaigns, job class: UpdateMissingMaroCampaignsJob, job name UpdateMissingMaroCampaignsJob //bb
         ...
         */
        $schedule->command('reports:downloadApi BlueHornet --daysBack=31 --runtime-threshold=5m')->monthly()->sendOutputTo($filePath); //bb
        $schedule->command('reports:downloadApi Campaigner --daysBack=31 --runtime-threshold=5m')->monthly()->sendOutputTo($filePath); //bb
        ...
        $schedule->command('reports:downloadApi Maro --daysBack=31')->monthly()->sendOutputTo($filePath); //bb
        //$schedule->command('reports:downloadApi Ymlp --daysBack=31')->monthly()->sendOutputTo($filePath);
        $schedule->command('reports:downloadApi Publicators --daysBack=31 --runtime-threshold=5m')->monthly()->sendOutputTo($filePath); //bb
        $schedule->command('reports:downloadApi Bronto --daysBack=31 --runtime-threshold=5m')->monthly()->sendOutputTo($filePath); //bb
        $schedule->command('reports:downloadTrackingData Cake 31 --runtime-threshold=10m')->monthly()->sendOutputTo($filePath); //bb
        ...
        $schedule->command( 'reports:downloadDeliverables Campaigner:delivered 2 Campaigner' )->dailyAt( self::EARLY_DELIVERABLE_SCHEDULE_TIME )->sendOutputTo( $deliverableFilePath ); //command GrabDeliverableReports, job class and name: RetrieveDeliverableReports //bb
        $schedule->command( 'reports:downloadDeliverables Campaigner:actions 5 Campaigner' )->cron('0 10,20 * * * *'); //bb
        $schedule->command( 'reports:downloadDeliverables BlueHornet:delivered 3 BlueHornet' )->dailyAt( self::DELIVERABLE_SCHEDULE_TIME )->sendOutputTo( $deliverableFilePath ); //bb
        $schedule->command('reports:downloadDeliverables BlueHornet:actions 5 BlueHornet')->cron('0 10,22 * * * *'); //bb
        #$schedule->command( 'reports:downloadDeliverables EmailDirect 5' )->dailyAt( self::DELIVERABLE_SCHEDULE_TIME )->sendOutputTo( $deliverableFilePath ); //bb
        $schedule->command( 'reports:downloadDeliverables Maro 2 Maro' )->cron('0 10,22 * * * *')->sendOutputTo( $deliverableFilePath ); //bb
        $schedule->command( 'reports:downloadDeliverables Maro:delivered 2 Maro' )->dailyAt( self::DELIVERABLE_SCHEDULE_TIME )->sendOutputTo( $deliverableFilePath ); //bb
        ...
        $schedule->command( 'reports:downloadDeliverables Bronto:actions 5 Bronto' )->cron('0 10,22 * * * *'); //bb
        $schedule->command( 'reports:downloadDeliverables Bronto:delivered 2 Bronto' )->dailyAt( self::DELIVERABLE_SCHEDULE_TIME )->sendOutputTo( $deliverableFilePath ); //bb
        $schedule->command( 'reports:downloadDeliverables Publicators:delivers 2 Publicators' )->dailyAt( self::DELIVERABLE_SCHEDULE_TIME )->sendOutputTo( $deliverableFilePath ); //bb
        $schedule->command( 'reports:downloadDeliverables Publicators:actions 5 Publicators' )->cron('0 10,22 * * * *'); //bb
        ...
        $schedule->command( 'reports:populateStats')->dailyAt(self::DELIVERABLE_AGGREGATION_TIME)->sendOutputTo($deliverableFilePath); // Job name like: PopulateEmailCampaignStats, PullCakeDeliverableStats //bb
        $schedule->command('process:useragents')->hourly(); // Job name like: ProcessUserAgents //bb
        $schedule->command('reports:findIncompleteDeploys --runtime-threshold=10m')->dailyAt(self::DEPLOY_CHECK_TIME); //command CheckDeployStats, job class DataProcessingJob, job name CheckDeployStats //bb
         ...
         *
         */
        $schedule->command('ftp:admin -H ' . config('ssh.servers.mt1_feed_file_server.host') . ' -U ' . config('ssh.servers.mt1_feed_file_server.username') . ' -k ' . config('ssh.servers.mt1_feed_file_server.public_key') . ' -K ' . config('ssh.servers.mt1_feed_file_server.private_key') . ' -u -s Feed')->everyFiveMinutes(); //command FtpAdmin, no job //bb
        $schedule->command( 'attribution:syncModelsWithNewFeeds' )->everyThirtyMinutes(); //command SyncModelsWithNewFeedsCommand, job class SyncModelsWithNewFeedsJob, job class and name SyncModelsWithNewFeedsJob //bb
        ...
        $schedule->command('runFilter expiration --runtime-threshold=25m')->dailyAt(self::MIDNIGHT); //command RunScheduledFilter, Job class ScheduledFilterResolver, job name: Scheduled Filter Expiration //bb
        ...
        $schedule->command('listprofile:dataEtl --all --runtime-threshold=5m')->cron('0 1 * * 7 *'); // command S3RedshiftExport, job class S3RedshiftExportJob //bb
        $schedule->command('listprofile:optimize')->weekly(); //command VacuumRedshift, job class: VacuumRedshiftJob, job class VacuumRedshiftJob, job name OptimizeRedshift //bb
        ...
        $schedule->command('listprofile:validateRedshift 1')->cron('0 6 * * * *'); // command RedshiftDataConsistencyValidation, Job names like: DataValidation & upper-case entity, job class: RedshiftDataValidationJob //bb

        /**
         * Feed File Processing
         */
        $schedule->command( 'feedRecords:processMt1BatchFiles --runtime-threshold=15m' )->everyFiveMinutes(); //command ProcessMt1BatchFeedFiles, job class ProcessMt1BatchFeedFilesJob, Job name like: ProcessMt1BatchFeedFilesJob% //bb
        $schedule->command( 'feedRecords:processMt1RealtimeFiles --runtime-threshold=15m' )->everyMinute(); //command ProcessMt1RealtimeFeedFiles, job class ProcessMt1RealtimeFeedFilesJob, Job name like: ProcessMt1RealtimeFeedFilesJob% //bb
        $schedule->command( 'feedRecords:processMt1FirstPartyFiles --feedname=unemployment --runtime-threshold=15m' )->everyMinute(); //command ProcessMt1FirstPartyFeedFiles, job class ProcessMt1%, Job name like: ProcessMt1UnemploymentFeedFilesJob% //bb next 5
        ...
        $schedule->command( 'feedRecords:updateCounts' )->dailyAt( self::EARLY_DELIVERABLE_SCHEDULE_TIME ); //command UpdateFeedCounts, job class UpdateFeedCountJob, Job name: UpdateFeedCountJob //bb
        $schedule->command( 'feedRecords:updateCounts' )->dailyAt( self::UPDATE_SOURCE_COUNTS );
        $schedule->command( 'feedRecords:checkMt1Realtime' )->everyThirtyMinutes(); //command CheckMt1RealtimeFeedProcessingCommand, job class CheckMt1RealtimeFeedProcessingJob, Job name like: CheckMt1RealtimeFeedProcessingJob% //bb
        $schedule->command( 'feedRecords:checkMt1Batch' )->everyThirtyMinutes(); //command CheckMt1BatchFeedProcessingCommand, job class and name like: CheckMt1BatchFeedProcessingJob% //bb

        // Currently commented-out. Waiting for everything going live
        // Process first party feeds, by feed id. This list is dynamic.
        #$schedule->command('feedRecords:firstParty')->cron('*/2 * * * * *'); //command GetFirstPartyRecords, no job //bb
        ...
        #$schedule->command('feedRecords:reprocessFirstParty 1')->dailyAt(self::DELIVERABLE_AGGREGATION_TIME); //command ReprocessFirstPartyActions, job class FirstPartyReprocessingJob, job name FirstPartyActionsReprocessing //bb
         ...
         */
        $schedule->command("dataValidation emails exists --runtime-threshold=1h")->dailyAt(self::MT1_SYNC_TIME); //command DataConsistencyValidation, job class DataConsistencyValidationJob, job name DataValidation-% //bb and two below
        ...
        $schedule->command("newActions:process --hoursBack=2 --runtime-threshold=20m")->cron("30 * * * * *"); //command: ProcessNewActionsCommand, job class: ProcessNewActionsJob, Job name like: ProcessNewActions% //bb
