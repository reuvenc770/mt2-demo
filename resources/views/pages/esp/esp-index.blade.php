        ...
        <table md-table>
            ...
            <tbody md-body>
                <tr md-row ng-repeat="record in esp.accounts track by $index">
                    ...
                    <td md-cell>@{{ record.field_options.open_email_id_field }} <md-icon ng-if="(record.name).toLowerCase() === 'bluehornet'" md-font-set="material-icons" class="mt2-icon-black material-icons icon-xs cmp-tooltip-marker" data-toggle="popover" data-placement="bottom" data-content="Note: The Blue Hornet EID token shown includes a dash ( - ) for technical reasons. The actual token in Blue Hornet and used in templates will not include the dash.">error</md-icon></td>
                    <td md-cell ng-bind="record.field_options.open_email_address_field"></td>
                    <td md-cell>@{{ record.field_options.email_id_field }} <md-icon ng-if="(record.name).toLowerCase() === 'bluehornet'" md-font-set="material-icons" class="mt2-icon-black material-icons icon-xs cmp-tooltip-marker" data-toggle="popover" data-placement="bottom" data-content="Note: The Blue Hornet EID token shown includes a dash ( - ) for technical reasons. The actual token in Blue Hornet and used in templates will not include the dash.">error</md-icon></td>
